...
### WOW64 下的 PEB32/TEB32

64 位 Windows 运行 32 位应用程序时，使用 **WOW64（Windows-on-Windows 64-bit）** 子系统。WOW64 进程同时维护**两套 PEB 和 TEB**：64 位的原生结构（PEB64/TEB64）以及 32 位的兼容结构（PEB32/TEB32），形成双层结构体系。

#### 为什么需要双层结构

32 位代码假设指针大小为 4 字节、地址空间为 4GB，而 64 位进程地址空间为 128TB。WOW64 必须向 32 位代码提供完整的 32 位运行时视图（32 位 PEB、32 位 `fs` 段指向 TEB32），同时在 64 位内核层维护真实的 64 位结构（供系统调用、内存管理使用）。

#### 双层结构布局

```
WOW64 进程地址空间
├── 64 位地址空间（0 – 0x7FFFFFFFFFFF）
│   ├── PEB64（64 位 PEB，正常位置 ~0x7FF...）
│   │     PEB64.Wow64                ← 指向 WOW64_PROCESS 兼容信息
│   ├── TEB64 × N（每线程一个，gs:[0x00]，正常 x64 约定）
│   │     TEB64.WowTebOffset (+0x180C) ← 指向对应 TEB32 的偏移（有符号）
│   │
│   ├── PEB32（32 位布局，通常位于低 4GB，如 0x7EFE0000 附近）
│   │     用 `fs:[0x30]`（32 位线程中）访问
│   └── TEB32 × N（每线程一个，32 位 `fs:[0x00]` 访问）
│         TEB32.Peb (+0x030)         ← 指向 PEB32（4 字节）
│         TEB32.NtTib.StackBase/Limit ← 32 位栈范围（用户态 32 位栈）
```
...
| 指针 | 位置 | 说明 |
|------|------|------|
| `EPROCESS.Peb` | 内核，8 字节 | 始终指向 **PEB64** |
| `EPROCESS.WoW64Process`（`Wow64Process` 字段） | 内核 | 非 NULL 表示该进程是 WOW64 进程，指向 WOW64 内核辅助结构 |
| `NtCurrentPeb()`（x64 `gs:[0x60]`） | TEB64 内 | 指向 **PEB64** |
| 32 位代码 `fs:[0x30]` | TEB32 内 | 指向 **PEB32**（4 字节，低 4GB 地址） |
| `TEB64.WowTebOffset`（+0x180C，LONG） | TEB64 | 有符号偏移：`(BYTE*)TEB64 + WowTebOffset = TEB32*` |

#### 访问 PEB32/TEB32（调试与逆向）

**WinDbg 内核调试**：

```windbg
; 1. 切换到 WOW64 进程上下文
.process /p <WOW64_EPROCESS_addr>

; 2. 查看 PEB64（正常路径）
dt nt!_PEB @$peb            ; PEB64

; 3. 定位 PEB32（通过 PEB64.Wow64 或直接偏移计算）
; PEB32 通常紧邻 PEB64 或由 WOW64 辅助结构记录
; 快捷方法：
!wow64exts.str              ; 如果 wow64exts.dll 可用
dt wow64!_PEB32 <PEB32_addr>

; 4. 通过 TEB64 找到 TEB32
dt nt!_TEB @$teb            ; TEB64
; TEB64.WowTebOffset 处读取偏移，计算 TEB32 地址
; r @$teb + poi(nt!_TEB WowTebOffset 实际偏移处读出的值)
```

**用户态调试（32 位调试视图）**：

```windbg
; 附加到 WOW64 进程，调试器以 32 位视角显示
0:000:x86> .peb              ; 显示 PEB32
0:000:x86> .teb              ; 显示 TEB32（fs:[0x00]）
0:000:x86> dt wow64!_PEB32 @$peb
0:000:x86> dt wow64!_TEB32 @$teb
```

**编程方式定位 PEB32（64 位进程中读取同一进程的 WOW64 信息）**：

```c
// 通过 NtQueryInformationProcess 获取 PEB32 地址（64 位进程查询 WOW64 子进程）
ULONG_PTR peb32Addr = 0;
NtQueryInformationProcess(
    hProcess,
    ProcessWow64Information,   // 返回 PEB32 地址（ULONG_PTR）
    &peb32Addr,
    sizeof(peb32Addr),
    NULL
);
// peb32Addr 即 PEB32 的 64 位地址（保证在低 4GB）
```

#### WOW64 转换层：从 32 位代码到 64 位内核

32 位代码无法直接调用 64 位内核，WOW64 通过以下机制完成转换：

```
32 位用户代码
    ↓ 调用 32 位 ntdll (SysWOW64\ntdll.dll) 中的 Nt* 存根
32 位 ntdll 存根
    mov eax, <调用号>
    call [wow64cpu!KiFastSystemCall]  ; 不执行 int 0x2e 或 sysenter
                                       ; 而是跳入 Heaven's Gate
Heaven's Gate（关键转换点）
    ; 地址 0x33:_code   ← CS = 0x33（64 位代码段选择子）
    retf               ; far return 到 64 位段（CS:EIP → CS:RIP）
                       ; 此后 CPU 进入 64 位模式执行 wow64cpu.dll 代码
wow64cpu.dll（64 位）
    ; 将 32 位参数从 32 位调用约定转换为 64 位调用约定
    ; 调用 64 位 ntdll (system32\ntdll.dll) 的 Nt* 存根
64 位 ntdll
    mov r10, rcx
    mov eax, <调用号>
    syscall            ; 真正的 64 位系统调用
    ↓ 进入内核 KiSystemCall64
```

**Heaven's Gate 详解**：在 WOW64 进程中，32 位代码段（CS=0x23）和 64 位代码段（CS=0x33）共存于同一进程地址空间。`retf`（far return）指令同时弹出 EIP 和 CS：当 CS 切换到 0x33 时，CPU 立即进入 64 位模式执行，这是 32 位→64 位的最小开销转换机制。逆向时在 `SysWOW64\ntdll.dll` 的 `Wow64Transition` 地址处（`fs:[0x0C0]` 或固定地址）即可找到 `retf` 指令。

```asm
; Heaven's Gate 处的汇编（32 位视图，示意）
; 地址 0x77XXX000 (SysWOW64 ntdll 中的 Wow64Transition 例程)
mov  edx, esp                    ; 保存 32 位 ESP
jmp  far ptr cs33_label          ; far jmp 到 CS=0x33（切换到 64 位模式）
; 以下 8 字节以 64 位模式执行：
cs33_label:
    ; 现在是 64 位代码（RIP = 此处地址，CS = 0x33）
    ; wow64cpu!BTCpuSimulate 或类似入口
```
...
**场景：在 64 位调试器中分析 WOW64 进程**：

| 目标 | 定位方法 |
|------|---------|
| PEB64 | `gs:[0x60]`（在 wow64 进程上下文中的 64 位 ntdll 代码里） |
| PEB32 | `ProcessWow64Information`（`NtQueryInformationProcess`）或 `fs:[0x30]`（切入 32 位模式后） |
| TEB64 | `gs:[0x00]`（标准 x64 路径） |
| TEB32 | `TEB64 + WowTebOffset`（TEB64 +0x180C 处的有符号偏移） |
| 32 位模块列表 | 遍历 PEB32.Ldr（`PEB_LDR_DATA32`），字段均为 4 字节指针 |
| 64 位模块列表 | 遍历 PEB64.Ldr（标准 `PEB_LDR_DATA`），字段均为 8 字节指针 |
| Heaven's Gate | SysWOW64\ntdll.dll 中 `Wow64Transition`（`fs:[0x0C0]` 指向） |

**WinDbg 切换 32/64 位视图**：

```windbg
; 在附加 WOW64 进程的 WinDbg 会话中
!wow64exts.info           ; 显示 WOW64 状态信息
!wow64exts.k              ; 显示 32 位调用栈
.effmach x86              ; 切换为 32 位分析视图（影响寄存器显示）
.effmach amd64            ; 切换回 64 位视图
```

**Volatility 取证场景**：WOW64 进程的 `dlllist` 命令会同时列出 PEB64 中的 64 位模块（`wow64.dll`、`wow64cpu.dll`、`wow64win.dll` 等 WOW64 子系统组件）和 PEB32 中的 32 位模块（`SysWOW64\ntdll.dll`、`SysWOW64\kernel32.dll` 等）；若仅看到 PEB32 模块而 WOW64 子系统组件缺失，可能是 PEB64 Ldr 链表被篡改的信号。
