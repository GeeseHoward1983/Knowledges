...
x64 以 `syscall` 指令切入内核，ARM64 Windows 则以 **`SVC #0`**（Supervisor Call，立即数固定为 0）触发 EL0 → EL1 的异常级别切换，二者在 CPU 硬件层面的机制完全不同。

#### 寄存器约定

| 角色 | ARM64 寄存器 | x64 等价 | 说明 |
|------|-------------|---------|------|
| 系统调用号 | **X8** | EAX | ntdll 存根用 `mov x8, #N` 写入 |
| 参数 1–4 | X0–X3 | RCX/RDX/R8/R9（x64 调用约定） | 遵循 ARM64 AAPCS64 |
| 参数 5–6 | X4–X5 | 栈（x64）或 R9/R11 | ARM64 在寄存器中传更多参数 |
| 返回值 | X0 | RAX | 系统调用的 `NTSTATUS` 或结果 |
| 返回地址 | **ELR_EL1**（CPU 自动保存） | RCX（CPU 自动保存） | CPU 在陷入时分别写入对应寄存器 |
| 旧 PSTATE | **SPSR_EL1**（CPU 自动保存） | R11（保存 RFLAGS） | 用于 `eret` / `sysret` 时还原 |

ARM64 ntdll 中 `NtReadFile` 存根示意：

```asm
; ARM64 Windows ntdll!NtReadFile 存根（示意，AArch64 汇编）
NtReadFile:
    mov  x8, #6          ; X8 = 系统调用号（与 x64 EAX 角色相同，值随版本变化）
    svc  #0              ; EL0 → EL1 异常，CPU 跳入异常向量表
    ret                  ; 从 ELR_EL1 返回（eret 已被内核处理，ret 回到调用方）
```

> [!warning] 示例输出为示意

与 x64 存根的核心差异：
- x64 必须先 `mov r10, rcx` 保护第 1 参（因为 `syscall` 会破坏 RCX），ARM64 无此问题——`SVC` 不覆盖 X0-X7；
- x64 调用号在 EAX，ARM64 调用号在 **X8**（参数寄存器 X0-X7 全部留给参数）；
- x64 `syscall` 将返回地址写入 RCX、RFLAGS 写入 R11，ARM64 CPU 将返回地址写入 **ELR_EL1**、处理器状态写入 **SPSR_EL1**，这两个寄存器在 EL1 中不可被用户态直接读写。

#### 异常向量表 VBAR_EL1

ARM64 的中断/异常入口通过 **`VBAR_EL1`（Vector Base Address Register, EL1）** 指向一个 2KB 对齐的向量表，共 16 个入口（每个 0x80 字节），按异常类型和来源 EL 分组：

```
VBAR_EL1 + 0x000  同步异常，来自 EL1（SP_EL0，通常不用）
VBAR_EL1 + 0x080  IRQ，来自 EL1（SP_EL0）
VBAR_EL1 + 0x100  FIQ，来自 EL1
VBAR_EL1 + 0x180  SError，来自 EL1

VBAR_EL1 + 0x200  同步异常，来自 EL1（SP_EL1）— 内核到内核
...
VBAR_EL1 + 0x400  同步异常，来自 EL0（64 位）— ← SVC #0 命中此入口
VBAR_EL1 + 0x480  IRQ，来自 EL0（64 位）
...
VBAR_EL1 + 0x600  同步异常，来自 EL0（32 位 AArch32，WOW64 32位）
```

`SVC #0` 从 EL0（64 位）发出，命中 `VBAR_EL1 + 0x400` 处的入口，内核在此处执行 `swapgs` 等价操作（ARM64 Windows 内核在此切换到内核栈、保存 EL0 寄存器）后跳入 ARM64 版 `KiSystemCall`，以 X8 为索引查 SSDT 分发。

ARM64 Windows 内核在启动时将内核异常向量表地址写入 `VBAR_EL1`（MSR 写操作，仅 EL1/EL2 可执行），对等于 x64 的 `wrmsr LSTAR, KiSystemCall64`。

#### x64 与 ARM64 系统调用机制对照

| 对比维度 | x64 Windows | ARM64 Windows |
|---------|------------|--------------|
| 陷入指令 | `syscall` | `SVC #0` |
| 调用号寄存器 | **EAX**（RAX 低 32 位） | **X8** |
| 参数寄存器 | RCX/RDX/R8/R9（+ 栈） | X0–X5（更多寄存器参数） |
| 第 1 参保护 | `mov r10, rcx`（syscall 破坏 RCX） | 无需额外保护（SVC 不覆盖 X0-X7） |
| 内核入口定位 | MSR `LSTAR`（0xC0000082）→ `KiSystemCall64` | 寄存器 `VBAR_EL1` → 异常向量表 `+0x400` |
| 返回地址保存 | CPU → RCX | CPU → **ELR_EL1** |
| 标志/状态保存 | CPU → R11（RFLAGS） | CPU → **SPSR_EL1** |
| 返回指令 | `sysret`（恢复 RIP←RCX, RFLAGS←R11） | `eret`（恢复 PC←ELR_EL1, PSTATE←SPSR_EL1） |
| 用户态保护（内核入口） | `swapgs`（切换 GS.base 到 KPCR） | 等价的 `mrs`/`msr` 操作切换内核栈 |

逆向 ARM64 Windows ntdll 时，识别系统调用存根的特征模式为 `mov x8, #N; svc #0; ret`（3 条指令），比 x64 的 `mov r10,rcx; mov eax,N; syscall; ret`（4 条指令）少一条，因为无需保护第 1 参。在 IDA / Binary Ninja 中搜索 `SVC #0` 即可快速定位所有存根。
