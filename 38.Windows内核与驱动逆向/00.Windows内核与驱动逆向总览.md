...
> 作者原创系列，共 **30** 篇（21–30 为扩展专题）。系统拆解 **Windows 内核机制与驱动逆向**：从 **内核架构**、**用户态↔内核态切换**、**WDM/KMDF 驱动模型（`DRIVER_OBJECT`/IRP）**、**内核对象与句柄**、**内核内存**、**WinDbg 内核调试**，到 **SSDT/系统调用表**、**内核回调**、**内核 Hook**、**Rootkit 原理与检测**、**PatchGuard/DSE/HVCI**、**内核漏洞与利用基础**，再到 **驱动逆向实战**、**IRQL 与内核同步**、**文件系统微过滤驱动**、**网络驱动与 WFP**、**ETW 内核遥测**、**PEB/TEB/KPCR 进程线程结构**，最后落到 **Windows 安全机制与对抗全景**。基准平台 **Windows 10/11 x64 + ntoskrnl.exe**，逆向工具为 IDA/Ghidra/WinDbg/x64dbg。驱动 `.sys` 即一个内核态 PE——本系列与 [[13.PE文件详解/00-合集总览-PE文件详解|PE 文件详解]]、[[44.调试器原理与实战/10.WinDbg与x64dbg|WinDbg 调试]]、[[34.现代二进制防护机制/10.Windows防护体系|Windows 防护体系]]、[[33.二进制漏洞与利用基础/00.二进制漏洞与利用基础总览|二进制漏洞]]、[[32.hooks/00.总览|Hook 技术]] 深度互补，并与 [[15.操作系统加载与启动/10.系统调用机制|Linux 系统调用]] 形成双平台对照。
...
| # | 章节 | 说明 |
| --- | --- | --- |
| 21 | [[21.APC机制深度]] | KAPC、三类 APC、APC 队列、线程注入(QueueUserAPC/Early Bird) |
| 22 | [[22.对象管理器深度]] | OBJECT_HEADER、句柄表、命名空间、ObRegisterCallbacks |
| 23 | [[23.Job对象与Silo容器隔离]] | EJOB、Silo、Windows 容器、AppContainer、逃逸边界 |
| 24 | [[24.注册表内核实现CM]] | Hive 格式、KCB、CmRegisterCallback、注册表 rootkit（防御）|
| 25 | [[25.HyperV架构与VBS]] | Hyper-V、VTL0/1、Secure Kernel、HVCI、Credential Guard |

## 九、内核攻防进阶（扩展，防御视角）

| # | 章节 | 说明 |
| --- | --- | --- |
| 26 | [[26.内核池风水与利用]] | 内核池、Segment Heap、pool grooming、缓解（防御/CTF）|
| 27 | [[27.BYOVD攻击链与驱动供应链]] | 脆弱驱动滥用、EDR 摘除、blocklist、检测（防御）|
| 28 | [[28.GDI与Win32k内核攻击面]] | win32k、GDI 对象喷射、user callback、类型隔离（防御）|
| 29 | [[29.CI.dll与代码完整性WDAC]] | Authenticode 内核验证、DSE、WDAC 策略、HVCI |
| 30 | [[30.内核Fuzzing]] | IOCTL fuzzing、kAFL/Nyx、Driver Verifier、CVD（负责任披露）|
