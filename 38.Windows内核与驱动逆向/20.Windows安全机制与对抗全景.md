...
### CET 与内核影子栈（Kernel Shadow Stack）

#### 用户态 CET：Shadow Stack + IBT

CET（Control-flow Enforcement Technology）是 Intel Tiger Lake（11th Gen）/ AMD Zen 3 起的硬件 CFI（Control Flow Integrity）机制，Windows 10 20H1+ 支持用户态 CET，分两个子特性：

**1. Shadow Stack（影子栈，SS）**

处理器维护一个独立的**只读**影子栈（由 `CR4.CET` 和 `IA32_U_CET MSR` 启用），其地址存于 `SSP`（Shadow Stack Pointer）寄存器：

```
普通栈（RW，用户可写）       影子栈（RO，硬件强制）
  RSP ──►  ┌──────────┐      SSP ──►  ┌──────────┐
            │ 返回地址  │               │ 返回地址  │  ← CALL 时同步压入
            │ 局部变量  │               │（仅返回地址，无局部变量）
            │ ...       │               └──────────┘
            └──────────┘
                 ↑                           ↑
...
`RET` 指令执行时，处理器同时弹出普通栈返回地址和影子栈返回地址，**若两者不一致则触发 `#CP`（Control Protection Exception）**，进程被终止。效果：ROP（Return-Oriented Programming）链中任何篡改返回地址的 gadget 均被检测并阻断。

**2. IBT（Indirect Branch Tracking，间接分支追踪）**

IBT 要求所有间接跳转目标（`JMP [r/m]`、`CALL [r/m]`）必须以 **`ENDBR64`**（`F3 0F 1E FA`）指令开头，否则触发 `#CP`：

```asm
; 合法的间接调用目标（须以 ENDBR64 开头）
target_func:
    endbr64          ; 标记为有效间接跳转目标
    push   rbp
    ...

; ROP/JOP gadget：无 ENDBR64，间接跳入此处 → #CP
internal_gadget:     ; 攻击者构造的 gadget，跳入函数中段
    pop    rdi
    ret
```

**Windows 用户态 CET 进程保护级别（Process Mitigation）：**

通过 `SetProcessMitigationPolicy(ProcessUserShadowStackPolicy, ...)` 或 `IFEO`（Image File Execution Options）注册表启用。微软 Edge、Win32k 等关键进程已默认启用。CIG 配合 CET：CIG 确保只有签名 DLL 加载，CET 确保 DLL 中的控制流无法被劫持。

#### Kernel Shadow Stack（Win11 22H2+）

Windows 11 22H2（KB5019980，2022年10月）将 CET Shadow Stack 扩展至**内核态**：

```
MSR：IA32_S_CET（0x6A2）  ── 内核 CET 控制寄存器
     ├── SH_STK_EN：启用内核影子栈
     └── ENDBR_EN：启用内核 IBT

内核影子栈存储在：
  PerCPU 的 Shadow Stack 内存（由 `ntoskrnl` 在 CPU 初始化阶段分配）
  ← 通过 EPT/SLAT 标记为 Supervisor Shadow Stack（不可执行，只可 shadow-stack 写入）
```
...
| 机制 | 保护对象 | 原理 | 硬件依赖 |
|---|---|---|---|
| kCFG（Kernel CFG） | 间接调用目标 | 位图校验，软件实现 | 无（Win10 RS2+）|
| Kernel IBT（CET 子集） | 间接跳转目标 | 硬件 `ENDBR64` 标记 | Intel 11th+/AMD Zen3+ |
| Kernel Shadow Stack | 返回地址完整性 | 硬件影子栈，`RET` 双重验证 | 同上 |
| PatchGuard（KPP） | 内核关键结构 | 定期校验 SSDT/IDT 等 | 无 |
| HVCI | 代码页完整性 | VTL1 EPT 只读内核代码 | VT-x/AMD-V + SLAT |

**内核 Shadow Stack 与 HVCI 的配合：**

- HVCI 确保内核代码页只读，无法注入 shellcode；
- 内核 Shadow Stack 确保即使攻击者通过内核漏洞覆盖了栈上的返回地址，`RET` 时会与影子栈的副本不一致，触发 `#CP` → BSOD（`KERNEL_SECURITY_CHECK_FAILURE`，Bug Check Code `0x139`）；
- 两者互补：HVCI 防止写入新可执行代码，Shadow Stack 防止劫持已有代码的返回链（ROP）。

**内核 IBT 实现细节（`ntoskrnl` 逆向视角）：**

Win11 22H2+ 的 `ntoskrnl.exe` 中，所有对外暴露的间接调用入口（如 KiInterruptDispatch、回调函数入口）均以 `ENDBR64` 开头。驱动程序若要注册内核回调（`PsSetCreateProcessNotifyRoutineEx` 等），其回调函数指针指向的代码也须以 `ENDBR64` 开头，否则内核 IBT 触发异常：

```bash
# 验证内核 Shadow Stack 状态（WinDbg 内核调试）
kd> rdmsr 0x6a2    # 读取 IA32_S_CET MSR
# bit 0 (SH_STK_EN) = 1：内核影子栈已启用
# bit 2 (ENDBR_EN)  = 1：内核 IBT 已启用
```

> [!warning] 示例输出为示意
> 具体 MSR 值依 CPU 型号与 Windows 版本而定。

**防御实战：驱动开发与 CET 兼容性**

新开发或更新的内核驱动若要在 Win11 22H2+ 的 CET 环境下稳定运行：
- 编译器须支持 `/CETCOMPAT`（MSVC）或 GCC 的 `-mmanual-endbr`；
- 所有被间接调用的函数入口自动生成 `ENDBR64`；
- 严禁使用"蹦床"（trampoline shellcode）修改返回地址——该手法在 Shadow Stack 下直接 BSOD；
- HVCI + Kernel Shadow Stack 组合下，内联 Hook（修改函数入口前几字节）仍被 HVCI 阻止，间接 Hook（篡改函数指针）则被 IBT 阻止，栈上 ROP 被 Shadow Stack 阻止，三重覆盖使传统 Ring 0 Hook 几乎全面失效。
...
⬅️ 上一篇 [[19.PEB与TEB与KPCR]] ｜ 🏠 [[00.Windows内核与驱动逆向总览]] ｜ ➡️ 下一篇 [[21.APC机制深度]]
