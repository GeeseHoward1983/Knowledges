...
### VMProtect 3.x Handler 表指针加密

VMProtect 2.x 的 Handler 表以**明文指针**存储各 Handler 的实际地址：Dispatcher 直接用操作码作索引读取表项，得到的就是可直接跳转的 Handler RVA。这使得静态枚举 Handler 表非常简单——找到表基址后，按固定步长扫描每一个 QWORD/DWORD 指针，即可批量提取所有 Handler 地址。

**3.x 的关键变化：指针加密存储**

VMProtect 3.x 对 Handler 表中的每个指针施加了加密变换后再写入表中，最常见的形式是 XOR 一个编译期确定的常量 key（部分样本采用 `ADD/SUB` 或 `ROR/ROL` 变换，极少数样本使用 key 本身也随位置变化的多 key 方案）：

```
存储值 = 真实 Handler 地址 XOR encryption_key
                   ↑
            编译期嵌入，同一保护实例所有表项共享同一 key（最常见）
```

枚举时若不先解密直接跳转，控制流会落到完全错误的地址，导致程序崩溃或触发保护检测。

**从 Dispatcher 逆出解密算法**

Dispatcher 在取出表项后、执行 `jmp` 之前，必然存在一段解密序列。定位方法：

1. 用 PIN Tracer 找到 Dispatcher 的高频代码块（见第 3 节）
2. 在 Dispatcher 汇编中寻找形如 `xor rax, 0xXXXXXXXX` 或 `add rax, 0xXXXXXXXX` 的常量操作——这个常量即 encryption_key；`rax` 通常是刚从 Handler 表读出的值

典型 3.x Dispatcher 片段（x64，XOR 型）：

```asm
; RBP = VMContext，RBX = 操作码 offset
mov    rax, [handler_table + rbx*8]   ; 读取加密后的表项
xor    rax, 0x12345678ABCDEF00        ; 解密：XOR 常量 key
jmp    rax                             ; 跳转到真实 Handler
```

对于运行时 key（少数样本 key 本身由初始化阶段计算），需在 `VM_INIT` 阶段设断，观察 key 被写入哪个寄存器或内存位置，再追踪到 Dispatcher 使用处。

**与 2.x 明文表的对比**

| 维度 | VMProtect 2.x | VMProtect 3.x |
|------|--------------|--------------|
| Handler 表项内容 | 明文 Handler 地址 | `地址 XOR key`（或 ADD/ROL 变换）|
| 静态枚举难度 | 直接读表即可 | 必须先提取 key 再批量解密 |
| Dispatcher 复杂度 | `jmp [table + op*8]` 一步到位 | 多一条（或几条）解密指令 |
| key 来源 | N/A | 编译期常量（最常见）或 VM_INIT 运行时计算 |

**对自动化分析的影响**

`angr CFGFast` 在静态构建 CFG 时会尝试从 Dispatcher 跳转目标推断控制流边，但加密后的表项值对 angr 来说是"无效地址"——CFGFast 无法解析 `xor rax, const` 后的间接跳转，导致 Handler 分支在 CFG 中呈现为**悬空边（unresolved indirect jump）**，Handler 函数全部缺失。

应对方案：
- 用 `CFGEmulated`（符号执行模式）替代 `CFGFast`，让 angr 实际执行解密指令后再跟踪跳转目标
- 或预处理阶段先提取 encryption_key，用脚本批量解密 Handler 表，再将真实地址 patch 回二进制，供 `CFGFast` 使用
