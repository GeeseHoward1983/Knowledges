...
ARM 标准指令均为固定 32 位，在内存受限的嵌入式场景中代码体积偏大。Thumb 指令集将常用操作压缩为 16 位编码，代码密度约提升 25%～30%；Thumb-2（ARMv6T2 引入）在 16 位基础上混入部分 32 位指令，兼顾密度与表达能力，是 Cortex-M 系列的主要执行模式。

| 模式 | 指令宽度 | 典型架构 |
|------|---------|---------|
| ARM | 32 位固定 | ARMv4～ARMv7-A |
| Thumb | 16 位固定 | ARMv4T～ARMv7 |
| Thumb-2 | 16/32 位混合 | ARMv6T2、ARMv7-A/M/R |

### 16 位 Thumb 与 32 位 Thumb-2 指令混合

Thumb-2 通过前缀字识别指令宽度：若第一个半字的高 5 位为 `11101`/`11110`/`11111`，则该指令是 32 位；否则为 16 位。汇编器和反汇编器对此透明处理，程序员通常无需手动区分。

```armasm
@ 16 位 Thumb 指令（Cortex-M 常见写法）
MOVS  R0, #1          @ 16 位：目标 < R8、立即数小
ADD   R1, R1, R2      @ 16 位：两寄存器运算
LDR   R3, [R4, #8]    @ 16 位：小偏移量

@ 32 位 Thumb-2 指令（超出 16 位限制时自动选用）
MOVW  R0, #0xABCD     @ 32 位：16 位立即数
LDRD  R6, R7, [SP, #16] @ 32 位：双寄存器加载
```

### IT（If-Then）条件执行块

AArch32 Thumb 状态下，几乎所有指令默认不支持条件执行后缀，需借助 `IT` 指令构建最多 4 条指令的条件执行窗口。`IT` 后接 T（Then）/E（Else）组合，最多 `ITTTE`/`ITEEE` 等形式。

```armasm
@ IT 块示例：等价于 ARM 状态 ADDEQ / SUBNE
CMP   R0, #0
ITEEE EQ            @ 若 EQ：第1条 Then，其余3条 Else
MOVEQ R1, #1        @ EQ 时执行
SUBNE R2, R2, #1    @ NE 时执行（第1条 Else）
ADDNE R3, R3, #1    @ NE 时执行（第2条 Else）
MOVNE R4, #0        @ NE 时执行（第3条 Else）
```

> 注：AArch64 彻底移除了 IT 块，条件执行改用 `CSEL`/`CSET` 等指令。

### ARM / Thumb 状态切换：BX / BLX 与 bit0

处理器通过目标地址的 **bit0** 判断切换目标状态：bit0=1 切换到 Thumb，bit0=0 切换到 ARM（实际跳转地址自动对齐）。

```armasm
@ 从 ARM 状态调用 Thumb 函数
LDR   R0, =thumb_func + 1    @ +1 使 bit0=1
BX    R0                      @ 跳转并切换到 Thumb 状态

@ BLX：带链接的跳转并切换状态（可直接用标号）
BLX   thumb_func              @ 汇编器自动设置 bit0
```

- `BX Rn`：跳转到 Rn，根据 bit0 切换状态，LR 不保存
- `BLX Rn/label`：同上但同时将返回地址存入 LR（R14）
- CPSR 的 T bit（bit5）反映当前状态；`BX LR` 从子程序返回时可同时恢复原状态

---

## LSE 原子指令（ARMv8.1-A Large System Extensions）

### 背景：LDXR/STXR 独占访问循环的局限

ARMv8.0 原子操作依赖"独占监控"模式：`LDXR`（加载独占）→ 修改 → `STXR`（存储独占，若监控失效则返回非零并重试）。在高核数/NUMA 系统中，这种循环在竞争激烈时可能因反复失败而效率低下，且不可被中断透明地插入。

```armasm
@ ARMv8.0 原子加 1（CAS-like 循环）
retry:
    LDXR  W0, [X1]        @ 独占加载
    ADD   W0, W0, #1      @ 修改
    STXR  W2, W0, [X1]    @ 独占存储，W2=0 成功
    CBNZ  W2, retry       @ 失败则重试
```

### ARMv8.1 LSE 指令一览

LSE 引入单指令原子操作，无需循环，且对互连总线更友好（one-shot 语义，减少总线事务）。

| 指令 | 操作 | 语义 |
|------|------|------|
| `LDADD Ws, Wt, [Xn]` | 原子加 | `Wt = mem; mem += Ws` |
| `LDSET Ws, Wt, [Xn]` | 原子置位 | `Wt = mem; mem \|= Ws` |
| `LDCLR Ws, Wt, [Xn]` | 原子清位 | `Wt = mem; mem &= ~Ws` |
| `LDEOR Ws, Wt, [Xn]` | 原子异或 | `Wt = mem; mem ^= Ws` |
| `SWP  Ws, Wt, [Xn]`  | 原子交换 | `Wt = mem; mem = Ws` |
| `CAS  Ws, Wt, [Xn]`  | 比较并交换 | 若 `mem==Ws` 则 `mem=Wt`，`Ws=原值` |

每条指令均有 A（Acquire）、L（Release）、AL 变体，用于控制内存序：

```armasm
@ 原子加（无序，适合统计计数器）
LDADD  W5, W6, [X0]        @ mem[X0] += W5，W6 = 旧值

@ 带 Acquire 语义的原子加（加载端内存序）
LDADDA  W5, W6, [X0]

@ Compare-And-Swap：若 *X0 == W8 则写入 W9
CAS    W8, W9, [X0]         @ W8 = 原始值（无论是否成功）

@ 仅写入、不需要旧值时：STADD（LDADD 的只写形式）
STADD  W5, [X0]
```

### 与 LDXR/STXR 的横向对比

| 维度 | LDXR/STXR 循环 | LSE 单指令 |
|------|---------------|------------|
| 代码行数 | 3～5 行 + 分支 | 1 行 |
| 竞争下性能 | 可能饥饿 | 保证进展 |
| 中断友好性 | 需配合 CLREX | 天然原子 |
| 最低架构要求 | ARMv8.0 | ARMv8.1 |
| 编译器启用 | 默认 | `-march=armv8.1-a` 或 `+lse` |

> Linux 内核与 glibc 在 ARMv8.1+ 硬件上会通过 hwcap 探测并动态替换为 LSE 指令（ifunc 机制）。

---

## AArch64 指令编码 Bit-Field 概览

### 固定 32 位编码与类别识别

AArch64 所有指令均为固定 32 位，通过 bits[28:25]（"op0"字段）快速区分大类：
...
以 `MOV`（别名 `ORR Xd, XZR, #imm`）与 `MOVZ` 为例：

```
MOVZ Xd, #imm16, LSL #shift
┌──────┬──┬──────────┬────────────────┬────┐
│ 1 1 0│sf│  opc(2)  │  hw(2)+imm16   │ Rd │
│[31:29]│28│  [28:23] │   [22: 5]      │4:0 │
└──────┴──┴──────────┴────────────────┴────┘
sf=1 → 64 位；sf=0 → 32 位（W 寄存器）
hw 编码 LSL 量：00=0, 01=16, 10=32, 11=48
```

### 加载/存储典型编码

```
LDR Xt, [Xn, #imm12]（无符号偏移形式）
┌──────────┬────┬────────────┬─────┬──────┐
│ 1111 1001│opc │  imm12     │  Rn │  Rt  │
│ [31:24]  │[23:22]│[21:10] │[9:5]│[4:0] │
└──────────┴────┴────────────┴─────┴──────┘
size bits[31:30]=11 → 64 位；10 → 32 位；01 → 16 位；00 → 8 位
imm12 为无符号偏移，按 size 自动缩放（64 位时实际偏移 = imm12 × 8）
```
...
│[31]│             [25:0]                  │
└───┴──────────────────────────────────────┘
op=0 → B；op=1 → BL（同时写 X30）
imm26 为 PC 相对偏移 / 4，有符号扩展，范围 ±128 MB
...
│ [31:24]  │  │   [23:5]        │  │[3:0] │
└──────────┴──┴─────────────────┴──┴──────┘
范围 ±1 MB；cond 4 位对应 EQ/NE/CS/CC/…/AL
```

> 工具提示：`objdump -d --no-show-raw-insn` 显示反汇编；加 `--insn-width=4` 可同时显示 32 位原始编码，便于对照上述位域。
