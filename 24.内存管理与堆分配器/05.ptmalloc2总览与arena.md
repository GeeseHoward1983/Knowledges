...
`main_arena` 通过 `brk` 扩展连续的 `[heap]` 段，天然是单块线性内存，**不需要额外描述符**；arena 自身的 `malloc_state` 就足够管理一切。

**线程 arena** 则不同：它通过 `mmap` 以 `HEAP_MAX_SIZE`（默认 **64 MB**，x86-64）为粒度申请匿名映射块。当一块 64 MB 用尽时，arena 需再申请一块新的 mmap 区域追加上去——这时就需要一个**描述符结构**来串联同一 arena 下的多个 mmap 块。这个描述符就是 `heap_info`。

#### heap_info 结构

```c
/* glibc malloc/arena.c 精简 */
typedef struct _heap_info {
    struct malloc_state *ar_ptr;   /* ① 指回所属 arena（malloc_state *） */
    struct _heap_info   *prev;     /* ② 指向同一 arena 的前一个 heap（逆序单链） */
    size_t  size;                  /* ③ 本块通过 mmap 映射的总字节数 */
    size_t  mprotect_size;         /* ④ 已 mprotect 可读写的字节数（懒映射上限） */
    /* 后面紧跟 malloc_state（仅第一个 heap 有），再之后是 chunk 数据区 */
    char    pad[...];              /* 对齐到 MALLOC_ALIGNMENT */
} heap_info;
```

| 字段 | 类型 | 含义 |
|---|---|---|
| `ar_ptr` | `malloc_state *` | 反向指针，指向管理本 heap 的 arena（用于 `arena_for_chunk`） |
| `prev` | `heap_info *` | 同 arena 前一个 heap 的头部地址（逆向链表，最早分配的 heap 的 prev 为 NULL） |
| `size` | `size_t` | 本 mmap 块大小（通常为 `HEAP_MAX_SIZE = 64MB` 的整数倍） |
| `mprotect_size` | `size_t` | 懒惰映射已开放的范围，初始较小，按需 `mprotect` 扩大 |

#### 内存布局：第一个 heap vs 后续 heap

```
mmap 区（第一个 heap，地址对齐到 HEAP_MAX_SIZE）
┌────────────────────────────────────┐  ← heap_info 起点（HEAP_MAX_SIZE 对齐）
│  heap_info                         │  0x00 ~ sizeof(heap_info)
│    ar_ptr → malloc_state(arena)    │
│    prev   = NULL                   │
│    size   = 64 MB                  │
├────────────────────────────────────┤  ← sizeof(heap_info) 对齐后
│  malloc_state（arena 管理头）       │  仅第一个 heap 有！
│  （fastbinsY、bins、top ...）       │
├────────────────────────────────────┤
│  chunk 数据区                       │  ← arena 实际管理的 chunk 空间
│  ...                               │
│  top chunk（wilderness）            │
└────────────────────────────────────┘  ← heap_info.size 上界

mmap 区（同 arena 第二个 heap）
┌────────────────────────────────────┐  ← 新 heap_info（HEAP_MAX_SIZE 对齐）
│  heap_info                         │
│    ar_ptr → 同一个 malloc_state    │
│    prev   → 第一个 heap_info       │  ← 串联：逆序链表
│    size   = 64 MB                  │
├────────────────────────────────────┤
│  chunk 数据区（无 malloc_state）    │
│  ...                               │
│  top chunk（新的 wilderness）       │
└────────────────────────────────────┘
```

**关键设计细节**：
- **第一个 heap 的 `malloc_state` 紧接 `heap_info`**，所以 `heap_info` 的结束地址 = `malloc_state` 的起始地址。
- **后续 heap 没有 `malloc_state`**，所有 heap 共享同一个 arena（`ar_ptr` 指向第一个 heap 中的 `malloc_state`）。
- `prev` 链是**逆序**的：新 heap 的 `prev` 指向旧 heap，形成从新到旧的单向链表。

#### arena_for_chunk：从 chunk 反查 arena

非主 arena 管理的 chunk 的 `size` 低 3 位中 `NON_MAIN_ARENA`（bit 2 = 0x4）置位。当 ptmalloc 需要知道一个 chunk 属于哪个 arena 时（如 `_int_free` 需要操作正确的 bins），用**地址掩码**快速定位 `heap_info` 继而找到 arena：

```c
/* glibc arena.c 精简 */
#define HEAP_MAX_SIZE  (1024 * 1024 * 64)   /* 64 MB，必须是 2 的幂 */
...
{
    if (!chunk_non_main_arena(ptr))
        return &main_arena;                 /* main arena，直接返回 */

    /* 非主 arena：chunk 地址 & ~(HEAP_MAX_SIZE-1) 找到 heap_info 头 */
    heap_info *heap = heap_for_ptr(ptr);
    return heap->ar_ptr;                    /* ar_ptr 就是 malloc_state * */
}
```

**地址掩码原理**：每个 `mmap` 的 heap 块都按 `HEAP_MAX_SIZE`（64 MB = 0x4000000）**对齐申请**（`mmap` 时 hint 地址按 `HEAP_MAX_SIZE` 对齐，实际通过申请 2× 大小再裁剪实现）。因此任意 chunk 地址 `& ~(HEAP_MAX_SIZE-1)` 即得到本 heap 的 `heap_info` 起始地址，`O(1)` 无需遍历。

```
chunk 地址   = 0x7fff_f400_0ab0
掩码        = ~(0x4000000 - 1) = 0xFFFF_FFC000_0000
heap_info   = 0x7fff_f400_0ab0 & 0xFFFF_FFC000_0000
            = 0x7fff_f400_0000   ← heap_info 结构体起点
heap_info.ar_ptr → malloc_state（第一个 heap 中）
```

#### main_arena 的特殊性：无 heap_info

| 特征 | main_arena | 线程 arena |
|---|---|---|
| 扩堆方式 | `brk`（连续，非 mmap） | `mmap`（HEAP_MAX_SIZE 对齐） |
| heap_info 描述符 | **无** | **有**（每个 mmap heap 块都有） |
| 多堆串联 | 不存在（`[heap]` 始终是一块连续段） | `heap_info.prev` 逆序链表 |
| arena_for_chunk | `NON_MAIN_ARENA=0`，直接返回 `&main_arena` | 掩码找 heap_info → `ar_ptr` |
| chunk NON_MAIN_ARENA 位 | 0（bit 2 = 0） | 1（bit 2 = 1） |

#### 多线程堆逆向/调试实践

```bash
# pwndbg：遍历线程 arena 的所有 heap 块
pwndbg> arenas           # 列出所有 arena
pwndbg> heap             # 当前 arena 的 chunk 布局
pwndbg> p *(heap_info*)0x7ffff0000000   # 手动查看 heap_info 结构
```

```python
# pwntools 离线脚本：从 chunk 地址反查 arena
HEAP_MAX_SIZE = 0x4000000  # 64 MB
chunk_addr = 0x7ffff4000ab0

# 检查 NON_MAIN_ARENA 标志（chunk size 低 3 位中 bit 2）
chunk_size = leaked_size_field  # 从堆泄漏读取
if chunk_size & 0x4:            # NON_MAIN_ARENA
    heap_info_addr = chunk_addr & ~(HEAP_MAX_SIZE - 1)
    ar_ptr = read_ptr(heap_info_addr + 0)       # heap_info.ar_ptr（偏移 0）
    prev   = read_ptr(heap_info_addr + 8)       # heap_info.prev（偏移 8）
    # ar_ptr 即 malloc_state 地址，可在其上操作 bins/top
else:
    ar_ptr = main_arena_addr    # 直接是 main_arena
```

**调试意义**：
- 堆逆向时，只要读到一个线程 arena chunk，就能通过掩码找到 `heap_info`，再找到 `malloc_state`（arena 管理头），进而访问 `top`、`bins`、`fastbinsY` 等全部分配状态。
- 多个 heap 之间通过 `prev` 链可完整遍历同一 arena 所管理的全部 mmap 区域，是调试器插件（pwndbg `arenas` 命令）实现堆全景的内部机制。
