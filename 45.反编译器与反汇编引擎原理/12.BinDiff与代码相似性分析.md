...
本章介绍的是**传统图匹配路线**：归一化指令哈希、CFG 结构度量（节点/边/圈复杂度）与调用图传播。这套方法在工程上成熟稳定，但对编译器差异（x86 vs ARM、O0 vs O2）和代码变形（inline 展开、循环展开）的鲁棒性有限。

当前工程活跃分支是**基于嵌入向量的深度学习方法**：Asm2Vec 将函数表示为低维向量后用余弦相似度度量，jTrans 以 Transformer 编码汇编 token，GNN 方案直接在 CFG/ACFG 图结构上做图神经网络嵌入，可跨架构、跨编译器进行语义相似性检索。这类方法的原理、训练细节与工程实践详见 [[20.深度学习二进制相似性]]。
