...
### dlmopen 与链接器命名空间：搜索顺序的"平行宇宙"

普通的 `dlopen` 把新加载的 DSO 插入**同一个全局符号命名空间**（`LM_ID_BASE`）——它的符号与已加载的所有库共享同一张查找作用域表。这意味着两件事：一，该 DSO 能"看到"进程里已有的所有全局符号（如 `malloc`）；二，它自身导出的符号也可能被其他模块的符号解析所截获（符号插入，见 [[06.符号解析与符号查找]]）。有时候这正是我们不想要的，比如：

- 在同一进程中同时加载一个库的**两个不兼容版本**（v1 与 v2 的 `libssl`）
- 将一个安全敏感的插件与主进程完全隔离，让它无法访问主进程的函数指针或全局状态
- 在测试框架中隔离被测模块，使其内部的 `malloc`/`free` 不与测试框架的覆盖版本冲突

**`dlmopen` 提供了"平行命名空间"解决方案**。
...
| `lmid` 值 | 含义 |
|---|---|
| `LM_ID_BASE`（0） | 主命名空间（等同 `dlopen`） |
| `LM_ID_NEWLM`（-1） | 创建一个全新的隔离命名空间 |
| 已有的 `Lmid_t` 值 | 加载进指定的已有命名空间（通过 `dlinfo(handle, RTLD_DI_LMID, &lmid)` 获取） |

```c
// 示例：在隔离命名空间中加载 libssl v1 与 libssl v3
void *ns1 = dlmopen(LM_ID_NEWLM, "libssl.so.1.1", RTLD_NOW | RTLD_LOCAL);
void *ns2 = dlmopen(LM_ID_NEWLM, "libssl.so.3",   RTLD_NOW | RTLD_LOCAL);
// 两者的内部全局变量、函数指针、初始化代码完全隔离，互不干扰
```

#### 命名空间隔离对搜索顺序的影响

命名空间隔离改变了依赖解析与符号查找的若干基本假设，与普通 `dlopen` 相比存在以下关键差异：

**差异一：依赖库的归属**

普通 `dlopen` 加载 `libfoo.so` 时，若 `libfoo` 的 `DT_NEEDED` 里有 `libc.so.6`，`ld.so` 会直接复用主命名空间里已有的 `libc.so.6`（SONAME 去重）。

`dlmopen(LM_ID_NEWLM, ...)` 则在新命名空间内**重新加载所有依赖**——新命名空间里会有自己私有的 `libc.so.6` 副本（独立的实例，独立的全局变量），即使进程里主命名空间已有一份。这正是"完全隔离"的代价：内存占用会更高（多份 libc 副本），也意味着不同命名空间的 `malloc`/`free` 分配的内存**不能跨命名空间传递**（指针地址虽然数值上可见，但来自不同的堆分配器实例，跨命名空间释放是未定义行为）。

**差异二：符号查找作用域**

普通搜索顺序（加载序 BFS 的全局作用域）在命名空间隔离下**仅在同一命名空间内有效**。命名空间 A 里的 `libfoo` 无法解析到命名空间 B 里的 `libbar` 导出的符号，即使两个命名空间在同一进程地址空间里。命名空间边界是符号查找的硬边界。

```text
主命名空间（LM_ID_BASE）:  libc.so.6、libstdc++、主程序 → 共享同一全局作用域
隔离命名空间 #1:           libssl.so.1.1（私有）、libc.so.6（私有副本）→ 独立作用域
隔离命名空间 #2:           libssl.so.3（私有）、libc.so.6（私有副本）→ 独立作用域

命名空间 #1 的 libssl 调用 malloc → 只能解析到命名空间 #1 自己的 libc malloc
命名空间 #2 的 libssl 调用 malloc → 只能解析到命名空间 #2 自己的 libc malloc
```

**差异三：`RTLD_GLOBAL` 在命名空间中的含义**

在普通 `dlopen` 中，`RTLD_GLOBAL` 让加载的库的符号进入全局查找作用域，可被后续加载的库看到。在 `dlmopen` 新建的命名空间中，`RTLD_GLOBAL` 的作用域仍被限制在**该命名空间内部**——它不会把符号"泄漏"到主命名空间或其他命名空间。

**差异四：库搜索路径策略本身不受影响**

需要区分"搜索路径策略"与"符号作用域"两个概念。命名空间隔离影响的是**符号可见性与依赖归属**，而不改变库文件定位的搜索策略——在隔离命名空间中，`ld.so` 仍然按 `DT_RPATH` → `LD_LIBRARY_PATH` → `DT_RUNPATH` → `/etc/ld.so.cache` → 默认路径的优先顺序去**文件系统**上找库文件，这与主命名空间完全一致。

#### 典型使用场景

- **插件隔离**：安全沙箱中加载不可信插件，让它无法通过符号查找获取主进程的内部函数指针
- **多版本并存**：同一进程加载两个 ABI 不兼容的同名库版本（如加密库的新旧版本），各自处理不同协议的连接
- **测试框架隔离**：测试框架替换了 `malloc`（通过 `LD_PRELOAD` 或主命名空间符号插入），但需要被测模块在隔离环境中使用原始分配器，以准确测量其内存行为

> [!note] 深入阅读
> `dlmopen` 涉及的链接器命名空间（Linker Namespace）完整机制——包括 `dlinfo(RTLD_DI_LMID)` 获取命名空间 ID、`RTLD_DEEPBIND` 优先本地符号、多命名空间下 `link_map` 链表的组织方式——详见 [[17.dlmopen与链接器命名空间]]。
