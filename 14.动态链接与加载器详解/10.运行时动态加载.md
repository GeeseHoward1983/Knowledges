...
`dlmopen` 是 glibc 提供的 `dlopen` 扩展，在**独立的链接器命名空间（linker namespace）**中加载共享库，彻底隔离符号解析范围：
...
`lmid` 参数决定目标命名空间：
- `LM_ID_BASE`（`0`）：主命名空间（与 `dlopen` 等效）。
- `LM_ID_NEWLM`（`-1`）：**创建全新的独立命名空间**并在其中加载库——这是 `dlmopen` 的核心用途。
- 已有命名空间的 ID（从前次 `dlmopen` + `dlinfo(handle, RTLD_DI_LMID, &id)` 获取）：在指定命名空间内追加加载库。

**与 `dlopen` 的本质区别**：

`dlopen` 加载的库与主程序共享同一个符号命名空间（若用 `RTLD_GLOBAL`），或与主程序同在默认命名空间但局部可见（`RTLD_LOCAL`）——无论哪种，库的符号解析都在同一个链接器全局状态（`ld.so` 内部的链接图）中进行，库的 `DT_NEEDED` 依赖会与主程序的依赖合并。

`dlmopen(LM_ID_NEWLM, ...)` 在**全新的、完全独立的链接图**中加载目标库：目标库及其所有传递依赖（`DT_NEEDED`）在新命名空间里单独解析、单独 mmap。新命名空间有自己的一份 `libc.so`、`libpthread.so` 等基础库副本，与主程序的 `libc` 完全隔离——函数指针、全局状态（如 `malloc` arena、`errno` TLS）互不干扰。

```c
// 在独立命名空间加载插件（与主程序 libc 隔离）
void *ns_handle = dlmopen(LM_ID_NEWLM, "./plugin.so", RTLD_NOW | RTLD_LOCAL);
if (!ns_handle) {
    fprintf(stderr, "dlmopen failed: %s\n", dlerror());
    return;
}

// 获取该命名空间的 lmid，以便后续在同命名空间追加加载
Lmid_t lmid;
dlinfo(ns_handle, RTLD_DI_LMID, &lmid);

// 在同一命名空间加载另一个库（与 plugin.so 共享 libc 副本）
void *extra = dlmopen(lmid, "./helper.so", RTLD_NOW | RTLD_LOCAL);
```

**典型使用场景**：
- **插件隔离**：多个插件各自加载同名但不同版本的依赖（如各自携带一个私有版本的 `libssl`），互不冲突。
- **安全沙箱**：在隔离命名空间加载不受信任的代码，避免其 `LD_PRELOAD` 风格的 hook 污染主程序符号空间。
- **多版本共存**：加载同一 DSO 的不同版本（`libfoo.so.1` 和 `libfoo.so.2`），但需各自独立运行。

**与 `dlopen` 的限制对比**：

| 维度 | `dlopen` | `dlmopen(LM_ID_NEWLM, ...)` |
|---|---|---|
| 符号命名空间 | 主命名空间或局部（RTLD_LOCAL） | **全新独立命名空间** |
| `libc` 共享 | 与主程序共享同一份 `libc` | **新命名空间有自己的 `libc` 副本** |
| `errno`/`malloc` 等全局状态 | 与主程序共享 | **完全隔离** |
| 内存开销 | 低（复用已加载的 libc） | 高（需额外加载 libc 及其他基础库） |
| 与主程序数据交换 | 直接指针传递即可 | 需要跨命名空间边界，只能传 POD 数据或通过 `void *` 传入/出 |
| glibc 最低版本 | 早期 glibc | glibc 2.3.4+ |

> [!note] 交叉引用
> `dlmopen` 的命名空间机制与链接器内部的链接图（link map）管理直接相关。更深入的命名空间语义、与 `LD_AUDIT` 的交互、以及 glibc 实现中 `Lmid_t` 的管理细节见 [[17.dlmopen与链接器命名空间]]。

### RTLD_NODELETE 的引用计数语义

`RTLD_NODELETE` 是传给 `dlopen` 的标志之一，但它的语义经常被误解：许多人以为"不删除"只是推迟了卸载，实际上它**改变了引用计数归零时的行为**，而不是禁止 `dlclose` 调用本身。

**精确语义**：加载时带 `RTLD_NODELETE` 标志，ld.so 内部在该库的 `link_map` 结构上设置一个持久化标记（glibc 内部为 `l_nodelete` 位）。此后，即使对该库句柄调用 `dlclose` 使引用计数降到 0，ld.so 也**不会**：
1. 调用库的析构函数链（`.fini_array` 中的函数、`__attribute__((destructor))` 标记的函数）。
2. 执行 `munmap` 把库的各段从地址空间移除。
3. 释放 `link_map` 结构（符号表等元数据仍占用内存）。

换言之，`RTLD_NODELETE` 的效果是"**此库一旦加载即永驻内存**，引用计数只是形式，归零也不触发真正的卸载流程"。

```c
// 加载时标记为不可卸载
void *handle = dlopen("./stateful_plugin.so", RTLD_NOW | RTLD_LOCAL | RTLD_NODELETE);
// ...使用插件...

dlclose(handle);
// handle 变为无效（不可再传给 dlsym），但库仍在内存中！
// 库的 destructor 未被调用，静态变量保持原始状态
// 符号地址（之前从 dlsym 拿到的函数指针）仍然有效！
```

**`dlclose` 不调用 destructor 的场景与陷阱**：

若库中存在 C++ 静态对象（带析构函数的全局/静态变量），`RTLD_NODELETE` 下这些对象的析构函数**永远不会被调用**（因为 `.fini_array` 不被执行）。这意味着：

```cpp
// plugin.cpp（被 dlopen 的库）
static std::shared_ptr<Connection> global_conn = make_connection(); // 有析构函数

// 如果宿主程序用 RTLD_NODELETE 加载此库，
// 程序退出时（或 dlclose 后）global_conn 的析构不会被调用，
// 可能导致连接未关闭、文件句柄未释放、网络状态不一致等问题
```

**合理使用 `RTLD_NODELETE` 的场景**：

1. **含不可安全卸载的全局状态的库**：某些库（如某些数据库驱动、密码学库）在全局初始化后无法安全地完全清理（内核资源、硬件状态、不可逆的进程级注册），`RTLD_NODELETE` 避免了危险的局部析构。

2. **函数指针缓存**：若调用方持有从 `dlsym` 拿到的函数指针，并且在 `dlclose` 之后仍可能通过回调被调用，`RTLD_NODELETE` 保证函数指针不成为悬空指针。

3. **TLS 析构依赖**：若库含有线程局部析构（通过 `__cxa_thread_atexit` 注册），而线程退出时机不可控，`RTLD_NODELETE` 可防止库在线程析构前被卸载导致崩溃。

**与多次 `dlopen` 的交互**：若一个库被 `dlopen` 了 N 次（N > 1），其中一次使用了 `RTLD_NODELETE`，则即使后续 N 次 `dlclose` 全部完成，库也不会被卸载。反之，若 N 次 `dlopen` 都不带 `RTLD_NODELETE`，引用计数归零后正常卸载。`RTLD_NODELETE` 是**粘性的**（sticky）：一旦有任何一次 `dlopen` 带了此标志，该库就进入"永驻"状态，无法通过后续 `dlclose` 消除这一标记（没有对应的"取消 NODELETE"接口）。

```c
// 第一次：普通加载
void *h1 = dlopen("lib.so", RTLD_NOW);
// 第二次：加 RTLD_NODELETE（此时库已在内存，但标记被追加）
void *h2 = dlopen("lib.so", RTLD_NOW | RTLD_NODELETE);

dlclose(h2);  // 引用计数 2 → 1，不卸载
dlclose(h1);  // 引用计数 1 → 0，但 RTLD_NODELETE 标记存在，仍然不卸载！
// 库永驻内存，无法再通过 dlclose 卸载
```
