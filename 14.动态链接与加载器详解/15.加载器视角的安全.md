...
### Shadow Stack / CET（`-fcf-protection=return`）实现原理

**Intel CET（Control-flow Enforcement Technology）**的影子栈（Shadow Stack）是 x86 硬件层面对"返回地址完整性"的保证，目标是从根本上使 ROP（Return-Oriented Programming）失效。与 Stack Canary 的"在溢出发生后检测"不同，Shadow Stack 是"返回时强制比对"，两者是互补而非替代的关系。

#### 硬件基础：Shadow Stack 页与特殊权限位

影子栈的存储区是一段专用内存，其 PTE（页表项）的 `Dirty` 位被用作 Shadow Stack 标记位（在 CET 语义下该位称为 `SSP` 标记）。这段内存：

- **不可被普通 `WRSS` 以外的写指令修改**：普通 `mov [addr], val` 向影子栈页写入会触发 `#GP`（General Protection Fault）。
- **只有专用指令可操作**：`WRSS`（Write to Shadow Stack）写单条返回地址；`RSTORSSP`（Restore SSP）恢复影子栈指针；`INCSSP`（Increment SSP）移动指针（用于跳过已弹出项）。
- **CR4.CET / MSR IA32_U_CET / IA32_PL3_SSP**：分别控制 CET 启用状态和用户态影子栈基址寄存器（Shadow Stack Pointer，SSP）。

```
普通栈 RSP ──→ [ ret addr(N) ]   ←── call 压入的正常返回地址
                [ ret addr(N-1) ]
                [ ...           ]

影子栈 SSP ──→ [ ret addr(N) ]   ←── 硬件在 CALL 时同步压入（只读副本）
                [ ret addr(N-1) ]
                [ ...           ]
```

每次 `CALL` 指令执行时，CPU 同时向两处压返回地址：普通栈（RSP 方向）和影子栈（SSP 方向）。每次 `RET` 时，CPU 比对普通栈顶（`[RSP]`）与影子栈顶（`[SSP]`），若不匹配则触发 `#CP`（Control Protection Exception）。

#### `XSAVE` 扩展联动：线程上下文保存

影子栈寄存器（SSP）作为 CET 状态，被纳入 `XSAVE` 扩展状态区域（`XSTATE_CET_U` 组件，component bit 11）。这意味着：

- **线程切换**：内核在上下文切换时通过 `XSAVE`/`XRSTOR` 保存/恢复每个线程独立的 SSP，影子栈与线程绑定，无需额外代码。
- **`clone()`/`pthread_create()`**：新线程需要**独立的影子栈页**。内核（或 glibc 线程库）在创建线程时分配并初始化新的影子栈，通过 `WRSS` 压入初始返回地址（线程入口），然后用 `RSTORSSP` 激活。

#### 加载器/内核的建立流程

加载器（`ld.so`）和内核如何为进程/线程建立影子栈：

```
① 内核 execve() 阶段：
   - 若 ELF 的 GNU_PROPERTY 段含 GNU_PROPERTY_X86_FEATURE_1_SHSTK 标记，
     内核为主线程分配影子栈页（大小 = 普通栈的一定比例）
   - 写入 CR4.CET=1，MSR IA32_U_CET.SHSTK_EN=1
   - MSR IA32_PL3_SSP = 影子栈基址 + 大小

② ld.so 自举阶段：
   - ld.so 自身若需要，通过 arch_prctl(ARCH_SHSTK_ENABLE, ARCH_SHSTK_SHSTK)
     确认或重新激活影子栈（Linux 6.6+ 接口）
   - 可通过 arch_prctl(ARCH_SHSTK_LOCK, ARCH_SHSTK_SHSTK) 锁定，防止后续代码关闭

③ pthread_create() / clone() 阶段：
   - glibc 的 __pthread_create_2_1() 分配新线程影子栈
   - 通过 map_shadow_stack(addr, size, flags) syscall（Linux 6.6+）
     创建影子栈页（PTE 标记 Shadow Stack）
   - WRSS 压入线程入口的返回地址
   - 新线程的 XSAVE 区域中写入新 SSP 地址
```

#### 编译器层面：`-fcf-protection=return`

GCC/Clang 的 `-fcf-protection=return` 选项在目标文件的 `.note.gnu.property` 中设置 `GNU_PROPERTY_X86_FEATURE_1_SHSTK` 标记，通知内核/加载器该二进制支持影子栈。完整 CET 使用 `-fcf-protection=full`（同时包含 IBT 分支保护）。

```bash
# 检查二进制是否启用了 CET Shadow Stack
readelf -n ./app | grep -A2 "GNU_PROPERTY"
```
...
Displaying notes found in: .note.gnu.property
  ...
  Properties: x86 feature: IBT, SHSTK   ← SHSTK = Shadow Stack 已启用
```

> [!note] CET 的 CPU 要求与现状
> Intel Ice Lake（2019 年）及以后的 CPU 支持 CET Shadow Stack；AMD Zen 3（2020 年）也支持。Linux 内核从 6.6 起提供完整的 Shadow Stack 系统调用接口（`map_shadow_stack`、`arch_prctl` SHSTK 子命令）。glibc 2.39 起在 `pthread_create` 中自动为新线程分配影子栈（若内核/CPU 支持）。

#### 为什么 ROP 被影子栈终结

经典 ROP 的核心是**覆写普通栈上的返回地址**，让 `RET` 跳到 gadget 而非合法调用方。Shadow Stack 的比对发生在 `RET` 执行时：

```
普通栈被溢出覆写：[ 0xdeadbeef (攻击者控制的 gadget) ]
影子栈（只读）：  [ 0x401234   (真实调用方的返回地址) ]

RET 执行 → CPU 比对：0xdeadbeef ≠ 0x401234 → #CP 异常 → 进程终止
```

唯一绕过 Shadow Stack 的方式是同时修改影子栈——而影子栈页的写保护使普通写指令无法做到这一点。攻击者必须找到影子栈页的 `WRSS` gadget 或利用内核漏洞，门槛大幅抬高。
...
### `__malloc_hook` / `__free_hook`（glibc < 2.34）案例详解

在第 3 点提到的 `__malloc_hook`/`__free_hook` 值得单独展开，因为它是"Full RELRO 保护了 GOT、但 `.bss`/`.data` 中的函数指针仍是攻击面"这一命题最经典的历史案例。

> 本节关注的是堆利用技术中如何**将任意写原语转化为代码执行**的这一环节，完整的堆漏洞利用链（how to get arbitrary write in the first place）见 [[33.二进制漏洞与利用基础/10.堆漏洞入门]]。

#### 什么是 `__malloc_hook`

`__malloc_hook` 是 glibc `malloc` 实现中内置的一个全局函数指针（声明在 `<malloc.h>`）：

```c
/* glibc malloc/malloc.h（简化）*/
...
glibc 的 `malloc()` 在每次被调用时，首先检查 `__malloc_hook` 是否非空，若非空则**直接调用该指针**而非执行正常的堆分配逻辑：

```c
/* glibc malloc/malloc.c（高度简化的逻辑示意）*/
void *malloc(size_t sz) {
    void *(*hook)(size_t, const void *) = atomic_load_relaxed(&__malloc_hook);
    if (__builtin_expect(hook != NULL, 0))
        return (*hook)(sz, RETURN_ADDRESS(0));  // 直接调用 hook，传入 caller 地址
    /* ... 正常堆分配逻辑 ... */
}
```

`__malloc_hook` 位于 glibc 的 `.bss` 段（或 libc 的可写数据段）——它**不受 Full RELRO 保护**（Full RELRO 只锁 `.got` 和 `.got.plt`）。

#### 利用方式：one_gadget / system

经典的 `__malloc_hook` 利用模式分两步：

**第一步：将 `__malloc_hook` 改写为 `one_gadget` 或 `system` 的地址**

攻击者先通过信息泄露获得 libc 基址（或通过格式化字符串/堆泄露），计算 `__malloc_hook` 在进程内存中的实际地址，然后用任意写原语把它改写为目标函数地址：
...
# 信息泄露：通过某个 use-after-free 或格式化字符串漏洞泄露 libc 地址
...
# 方式一：改为 one_gadget（执行条件满足时直接 execve("/bin/sh", NULL, NULL)）
one_gadget_addr = libc_base + 0x4527a   # one_gadget 工具找到的偏移（示意）
write_target(p, malloc_hook, one_gadget_addr)

# 触发：下一次 malloc() 调用即执行 one_gadget
trigger_malloc(p)
```

```python
# 方式二：改为 system，同时构造 malloc("/bin/sh") 调用
system_addr = libc.symbols['system']
write_target(p, malloc_hook, system_addr)
# 此后调用 malloc(addr_of_binsh_string) 等价于 system("/bin/sh")
```

**第二步：触发 `malloc()` 调用**

几乎任何触发 `malloc` 的路径都能触发 hook——`printf`（内部 buffer 扩展时）、`scanf`、`strdup`、C++ `new` 运算符……这使得触发十分容易。

#### 为什么 glibc 2.34 移除了这些钩子

`__malloc_hook` 系列钩子的设计初衷是"调试辅助"（应用层替换自定义分配器、内存追踪），但长期以来成为**全平台、跨漏洞类型通用的 exploitation primitive**：只要有任意写，就能把堆操作转化为任意代码执行，门槛极低。

2021 年发布的 **glibc 2.34** 彻底移除了 `__malloc_hook`、`__free_hook`、`__realloc_hook`、`__memalign_hook`（[glibc commit](https://sourceware.org/git/?p=glibc.git;a=commit;h=6c40b9ee0e34e0e5a7d8d4c18dd8d92f1ab36e66)）。原因直接：

> "These hooks have been deprecated since at least glibc 2.25, and now we are removing them entirely. They are a security hazard and do not meet the needs of modern programs."

移除后的影响：
- glibc 2.34+ 的 libc 中不再有 `__malloc_hook` 符号，该地址不再有效。
- 旧的基于 `__malloc_hook` 的利用脚本在新版 glibc 上直接失效，需要改用 tcache poisoning、`_IO_FILE` 结构体改写等新型原语（堆利用的演进详见 [[33.二进制漏洞与利用基础/10.堆漏洞入门]]）。
- 这是 glibc 对"不安全的历史遗留特性"做出的主动安全决策，与同期 glibc 2.34 移除 `LD_AUDIT` 部分接口的加固方向一致。

#### 残余攻击面的启示

`__malloc_hook` 被移除后，`.bss`/`.data` 中还有多种全局函数指针可成为攻击面（如 `_IO_FILE` 结构体的跳表 `_IO_jump_t`、C++ 虚表、`atexit` 回调链、glibc 内部的 `__exit_funcs` 数组等）。Full RELRO + CET/CFI 是从"写保护"和"调用落点限制"两个维度系统缩窄这类攻击面的组合拳，而非单靠移除个别钩子。
