...
### AArch64 与 RISC-V 的 PIC 代码生成

x86-64 用 `RIP`-relative 寻址解决了"代码段无绝对地址"的问题，但这是 x86-64 ISA 的专属特性。AArch64 和 RISC-V 采用不同但等价的方案，理解这两套机制有助于把 PIC 原理从"x86-64 的实现细节"提升到"架构无关的通用规律"。

#### AArch64：`ADRP + ADD` 与 `:got:` GOT-relative 寻址

AArch64 没有通用的"PC + 任意 32 位偏移"寻址，但提供了 **`ADRP`（Address of Page，PC-relative 页对齐地址）**指令：它把当前 PC 的高位（低 12 位清零，即对齐到 4KB 页）加上 21 位有符号偏移（以页为单位），得到一个 4KB 对齐的"页基址"，存入目标寄存器。紧跟其后的 `ADD` 或 `LDR` 再加上页内 12 位偏移，完成最终寻址。

**访问本模块 `.rodata`（直接 PC-relative）**：

```asm
// AArch64：访问本模块的只读字符串（.rodata）
adrp    x0, .LC0@PAGE         // x0 = (.LC0 所在页的地址)，PC-relative，21 位页偏移
add     x0, x0, .LC0@PAGEOFF  // x0 += (.LC0 在页内的偏移，12 位)
                               // 最终 x0 = &"hello\n"，全程无绝对地址
```

与 x86-64 `lea rdi, [rip + 0x1234]` 对应：x86-64 用"PC + 一个连续的 32 位偏移"，AArch64 用"PC 对齐到页 + 页内偏移"，本质都是"编码为与加载基址无关的相对量"。

**访问外部符号（经 GOT 间接访问）**：

当目标符号在其他 DSO 中，AArch64 编译器生成 `:got:` / `:got_lo12:` 修饰词，对应 `R_AARCH64_ADR_GOT_PAGE` + `R_AARCH64_LD64_GOT_LO12_NC` 两条重定位，将 GOT 表项的地址分两步取出：

```asm
// AArch64：通过 GOT 访问外部变量 `errno`
adrp    x0, :got:errno          // x0 = GOT[errno] 所在页地址（R_AARCH64_ADR_GOT_PAGE）
ldr     x0, [x0, :got_lo12:errno]  // x0 = GOT[errno] 的值（= errno 真实地址，R_AARCH64_LD64_GOT_LO12_NC）
ldr     w1, [x0]                // w1 = *errno（读取 errno 值）
```

与 x86-64 的 `movq errno@GOTPCREL(%rip), %rax; movl (%rax), %edi` 完全对应：第一步取 GOT 表项的地址，第二步解引用得符号地址，第三步读取值。

**与 x86-64 `RIP`-relative 的对比**：

| 维度 | x86-64 | AArch64 |
|---|---|---|
| 直接 PC-relative 寻址 | `lea rdi, [rip+disp32]`（最大 ±2GB 偏移） | `adrp + add`（最大 ±4GB，以页为单位） |
| GOT 间接 | `movq sym@GOTPCREL(%rip), %rax` | `adrp x0, :got:sym; ldr x0, [x0, :got_lo12:sym]` |
| 编码宽度 | 32 位偏移，单条指令 | 21 位页偏移 + 12 位页内偏移，两条指令 |
| 固定指令长度 | 否（变长 ISA） | 是（4 字节定长），故需两条指令拆分寻址 |

#### RISC-V：`auipc + addi` 与经 GOT 的 `la` 伪指令

RISC-V 的 PC-relative 寻址方式与 AArch64 高度相似，但使用 **`auipc`（Add Upper Immediate to PC）+ `addi`/`lw`/`ld`** 的组合：

- `auipc rd, imm20`：`rd = PC + (imm20 << 12)`，即把 20 位立即数左移 12 位加到当前 PC，得到目标符号所在 4KB 页的基址（page-aligned PC-relative 地址）。
- `addi rd, rd, imm12`：再加 12 位有符号偏移，得到目标符号的精确地址。

**直接 PC-relative 访问本模块符号**：

```asm
// RISC-V：访问本模块的 .rodata 字符串（%pcrel_hi / %pcrel_lo 重定位对）
auipc   a0, %pcrel_hi(.LC0)     // a0 = PC + (%pcrel_hi(.LC0) << 12)（R_RISCV_PCREL_HI20）
addi    a0, a0, %pcrel_lo(.LC0)  // a0 += %pcrel_lo(.LC0)（R_RISCV_PCREL_LO12_I）
                                  // 最终 a0 = &.LC0
```

RISC-V 汇编器要求 `%pcrel_lo` 标注的指令必须引用同一 `auipc` 指令的标号，链接器在处理 `R_RISCV_PCREL_LO12_I` 时会找到配对的 `R_RISCV_PCREL_HI20` 取其重定位结果作为 base，这是 RISC-V 重定位对（relocation pair）的标准用法。

**经 GOT 访问外部符号**：

RISC-V 汇编器提供 `la`（Load Address）伪指令，在 PIC/PIE 上下文中会展开为经 GOT 的两条指令：

```asm
// RISC-V：通过 GOT 访问外部符号 `errno`（la 伪指令展开）
auipc   a0, %got_pcrel_hi(errno)   // a0 = GOT[errno] 所在页（R_RISCV_GOT_HI20）
ld      a0, %pcrel_lo(1b)(a0)      // a0 = GOT[errno] 的值（= errno 真实地址，R_RISCV_PCREL_LO12_I）
lw      a1, 0(a0)                   // a1 = *errno
```

`%got_pcrel_hi` 对应 `R_RISCV_GOT_HI20` 重定位，告诉链接器在 GOT 中为 `errno` 分配一个槽位，并把"GOT 槽位地址相对当前 PC 的高 20 位"编码进 `auipc`。

**与 x86-64 和 AArch64 的横向对比**：

| 维度 | x86-64 | AArch64 | RISC-V |
|---|---|---|---|
| "取 PC"方式 | `RIP` 隐式作为基址 | `ADRP`（页对齐） | `auipc`（页对齐） |
| 直接 PC-relative | `lea rdi, [rip+d32]` | `adrp + add` | `auipc + addi` |
| GOT 间接 | `movq @GOTPCREL(%rip); deref` | `adrp :got:; ldr :got_lo12:` | `auipc %got_pcrel_hi; ld %pcrel_lo` |
| 重定位对 | 否（单条指令单重定位） | 否（两条指令各有重定位） | 是（`HI20` + `LO12` 成对） |
| GOT 松弛 | `REX_GOTPCRELX → leaq` | `ADR_GOT_PAGE → ADR` | `GOT_HI20 → PCREL_HI20`（可松弛） |

三架构的核心策略完全一致：**把符号地址拆分为"编译期可知的相对量（链接时确定偏移）"和"运行期填入的绝对量（GOT 槽位）"**，代码段只含相对量，绝对量统一放可写 GOT，由动态链接器加载时填入。差异仅在于 ISA 提供的 PC-relative 指令形态和编码宽度。
...
### THP（透明巨页）对 ASLR 熵的影响

**透明巨页（Transparent HugePage，THP）** 是 Linux 内核对 2MB（x86-64）或 512MB（AArch64，64 KB 页模式下的 512×64 KB）大页的自动使用机制：内核在满足对齐条件时，会把一段连续的 4KB 小页"晋升"为一个巨页，减少 TLB 缺失，提升内存访问吞吐量。THP 通过 `/sys/kernel/mm/transparent_hugepage/enabled` 控制（`always`/`madvise`/`never`）。

**THP 对 ASLR 熵的影响**：ASLR 在选择 mmap 基址时，会把地址**对齐到 2MB 边界**（当 THP 可能适用时），这意味着基址的低 21 bit 都被固定为 0，而不是仅低 12 bit 为 0。直接的结果是：**可随机的有效比特数减少了 9 bit**（从"低 12 bit 固定"变为"低 21 bit 固定"）。

以 x86-64 共享库加载为例：
- 正常 4KB 对齐：低 12 bit 固定，有效熵 ~28–30 bit（48 bit 空间 - 固定位 - 内核保留区）。
- 2MB THP 对齐：低 21 bit 固定，有效熵**减少约 9 bit**，降至约 19–21 bit。
- 若内核选择 1GB 巨页（Gigabyte Page，`pdpe1gb` CPU 特性，某些场景下使用），低 30 bit 固定，有效熵进一步骤降至约 10–12 bit，已进入暴力可猜的范围。

```bash
# 查看当前 THP 状态
cat /sys/kernel/mm/transparent_hugepage/enabled
# 输出示例：always [madvise] never
# [madvise] 表示仅在程序明确 madvise(MADV_HUGEPAGE) 的区域使用 THP

# 强制禁用 THP（调试/安全强化场景）
echo never > /sys/kernel/mm/transparent_hugepage/enabled
```

**AArch64 上问题更突出**：AArch64 支持 4 KB / 16 KB / 64 KB 三种页粒度（编译内核时选定），使用 64 KB 页的系统（某些嵌入式 / 企业 Linux）基础对齐已是 64 KB，对应 `1 << 16`，基础熵就比 x86-64 少 4 bit；若再启用 512 MB 巨页，熵损失非常严重。

**安全含义**：在对 ASLR 熵有严格要求的场景（如需防范本地暴力探测攻击的多租户系统），应禁用 THP（或设为 `madvise` 仅按需启用），在性能与安全之间权衡。反过来，如果希望测试程序在 THP 关闭和开启时的地址布局差异，可用 `pmap <pid>` 观察映射是否出现 2MB 对齐的区间。
...
### prelink 工具的安全含义与检测方法

prelink 对安全性的破坏体现在以下几个维度：

**ASLR 直接失效**：prelink 把库的虚拟地址写进文件后，该文件每次 mmap 都会请求固定虚拟地址。ld.so 在加载时检测到 prelink 元数据（存储在库文件的 ELF NOTE 节或独立的 `/etc/prelink.cache` 文件里），会尝试 mmap 到预先分配的地址。即使内核 ASLR 给出随机地址，ld.so 检测到不匹配时会选择**重新全量重定位**（放弃 prelink 加速效果，但此时 ASLR 随机地址才有效）——或者在更激进的 prelink 实现里，直接 `MAP_FIXED` 强制映射到固定地址，**ASLR 对该库完全失效**。

**符号地址全部可预测**：一旦某库的 prelink 基址固定，该库内所有 `.got`、`.got.plt`、函数地址全部可在攻击前计算。攻击者只需知道目标系统的 prelink 配置（通常从同版本发行版安装即可得知），无需任何信息泄漏漏洞就能构造 ROP 链。

**检测 prelink 是否影响某个库**：

```bash
# 方法一：直接查询 prelink 状态（需安装 prelink 工具）
prelink -p /usr/lib/x86_64-linux-gnu/libc.so.6
# 输出示例（有 prelink 信息时）：
# /usr/lib/x86_64-linux-gnu/libc.so.6: 0x7f3b4a200000 [prelinked]
# 输出示例（无 prelink 时）：
# /usr/lib/x86_64-linux-gnu/libc.so.6: not prelinked

# 方法二：readelf 查看是否有 prelink ELF NOTE
readelf -n /usr/lib/x86_64-linux-gnu/libc.so.6 | grep -i prelink

# 方法三：通过 /proc/<pid>/maps 观察同一库多次运行的地址是否变化
# 若 prelink 生效且 MAP_FIXED，库地址每次相同
# 若 ASLR 有效（prelink 未生效），库地址每次不同
for i in 1 2 3; do
    awk '/libc/{print $1}' /proc/$$/maps | head -1
    sleep 0
done
# 输出若三次相同 → prelink 固定了基址；每次不同 → ASLR 正常工作

# 方法四：检查 /etc/prelink.cache 是否存在（prelink 的全局缓存）
ls -la /etc/prelink.cache 2>/dev/null && echo "prelink cache exists"
```

> [!warning] 示例输出为示意
> 上述命令在未安装 `prelink` 的现代 Linux 发行版上会直接报"command not found"，这本身就是最好的结果——说明该系统没有 prelink，ASLR 正常工作。`/etc/prelink.cache` 存在则需警惕。

**彻底消除 prelink 影响**：

```bash
# 对全系统所有库撤销 prelink（需 root）
prelink -u -a

# 删除 prelink 缓存
rm -f /etc/prelink.cache

# 禁止 prelink 定期任务重新运行（若有 cron/systemd 单元）
systemctl disable prelink.timer 2>/dev/null
# 或
rm -f /etc/cron.daily/prelink
```

**现代发行版的现状**：Fedora 18（2013）、Debian/Ubuntu 16.04 LTS 后期均已将 prelink 从默认安装中移除。在 2020 年以后安装的主流 Linux 发行版上，绝大多数情况下 prelink 不存在；但在**老旧嵌入式系统、定制企业 Linux 或从旧系统升级而来的镜像**中，仍可能残留 prelink 配置，安全审计时需要检查。
