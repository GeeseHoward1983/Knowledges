...
### mini loader 补 DT_RELR 处理

上面核心代码里 `⑦` 步骤只处理了 `DT_RELA`（`Elf64_Rela` 数组），跳过了 `DT_RELR`。在现代工具链（binutils 2.36+、glibc 2.35+ 默认开启）生成的共享库中，**`DT_RELR` 已成为相对重定位的主要格式**，mini loader 若不处理它，会让大量 `RELATIVE` 型重定位悄悄被跳过，导致被加载的库内部指针全部偏差 `load_bias`，运行时立即崩溃。

`DT_RELR` 的设计背景见系列第 7 篇 [[07.重定位机制详解]] 的 DT_RELR 专节；本节聚焦在 mini loader 里如何**正确解析并应用 RELR 位图**。

#### RELR 格式解析：地址项与位图项的区分

`DT_RELR` 指向的是一个 `Elf64_Relr`（实为 `uint64_t`）数组，通过一条编码规则区分两种条目：

| 条目值 | 含义 | 处理动作 |
|---|---|---|
| **偶数**（LSB = 0） | **地址项（address entry）**：直接是一个需要重定位的目标地址（虚拟地址，相对 `vaddr_min`） | 对该地址执行 `RELATIVE` 重定位，并将"当前位图基址"设置为该地址之后的 8 字节处 |
| **奇数**（LSB = 1） | **位图项（bitmap entry）**：63 个有效位，每 bit 对应"从当前基址起第 N 个 8 字节对齐字"是否需要重定位 | 逐 bit 扫描，置 1 的位对应地址执行 `RELATIVE` 重定位；整个位图项处理完后，基址前移 63×8 字节 |

`RELATIVE` 重定位的应用与 `DT_RELA` 中 `R_X86_64_RELATIVE` 相同：

```c
*target = load_bias + *target;   // RELR 条目没有 addend，addend 就是槽位原始值
```

（区别于 `Elf64_Rela` 的 `r_addend` 字段——RELR 为了压缩体积，把 addend 内联在槽位本身。）

#### 完整的 RELR 解析代码

```c
/* mini_loader.c — 补充 DT_RELR 处理（插入到 ⑦ 步重定位处理块中） */

/* 从 .dynamic 取 RELR 相关字段 */
typedef uint64_t Elf64_Relr;   /* 若 elf.h 未定义，手动补充 */

Elf64_Relr *relr    = NULL;
size_t      relrsz  = 0;

if ((d = find_dyn(dyn, DT_RELR)))    relr   = (Elf64_Relr *)(d->d_un.d_ptr + load_bias);
if ((d = find_dyn(dyn, DT_RELRSZ)))  relrsz = d->d_un.d_val;
/* DT_RELRENT 是每个条目大小（8 字节），可验证但通常直接用 sizeof(Elf64_Relr） */

/* --- 应用 RELR 重定位 --- */
if (relr && relrsz > 0) {
    size_t nrelr = relrsz / sizeof(Elf64_Relr);
    uint64_t *base = NULL;   /* 当前位图基址（指向进程地址空间中的目标地址） */

    for (size_t i = 0; i < nrelr; i++) {
        Elf64_Relr entry = relr[i];

        if ((entry & 1) == 0) {
            /* ── 地址项：entry 本身是一个虚拟地址 ── */
            base = (uint64_t *)(entry + load_bias);
            /* 对该地址执行 RELATIVE 重定位：slot += load_bias */
            *base += load_bias;
            /* 下一个位图的基址从 base+1 开始（位图项以 base 之后的字开头）*/
            base++;
        } else {
            /* ── 位图项：63 个有效位（bit 0 固定为 1，是奇偶标记，不代表重定位目标） ── */
            uint64_t bitmap = entry >> 1;   /* 去掉 LSB 标记位，得到 63 位有效位图 */
            uint64_t *cursor = base;        /* 从当前基址开始逐 bit 处理 */

            while (bitmap != 0) {
                int bit = __builtin_ctzll(bitmap);  /* 找最低置 1 位的位置 */
                uint64_t *slot = cursor + bit;       /* 对应的目标地址 */
                *slot += load_bias;                  /* RELATIVE 重定位 */
                bitmap &= bitmap - 1;               /* 清除最低置 1 位 */
            }
            /* 位图项处理完后，基址前移 63 个字（一个位图项覆盖 63×8 = 504 字节）*/
            base += 63;
        }
    }
}
...
> [!note] `DT_RELR` vs `DT_RELA` 的重要区别
> - `DT_RELA` 条目包含显式 `r_addend`，槽位值可为任意初始值（甚至 0）。
> - `DT_RELR` 条目**没有 addend 字段**，链接器在输出时把 addend 直接写入槽位（重定位前槽位值 = 目标虚拟地址，加载时 `+= load_bias` 即可）。两者不要混用处理逻辑。

#### 验证：用 `readelf` 观察 DT_RELR

```bash
# 检查 .so 是否使用了 DT_RELR
readelf -d /lib/x86_64-linux-gnu/libc.so.6 | grep -i relr
```

```text
# 示意输出
 0x0000000000000023 (RELR)      0x12a3c0
 0x0000000000000024 (RELRSZ)    0x4ea0 (bytes)
 0x0000000000000025 (RELRENT)   0x8 (bytes)
```

```bash
# 对比 .rela.dyn 的大小（RELR 代替了其中绝大部分 RELATIVE 条目）
readelf -d /lib/x86_64-linux-gnu/libc.so.6 | grep -E 'RELA|RELASZ'
```

```text
# 示意输出：RELR 体积远小于等量 RELA 所需空间
 0x0000000000000007 (RELA)      0x118880
 0x0000000000000008 (RELASZ)    0x1140 (bytes)   ← 只剩少量非 RELATIVE 的 RELA 条目
```

> [!warning] 示例输出为示意
> 具体地址和大小因 glibc 版本、编译选项而异；关键是 `RELR`+`RELRSZ` 对的存在和 `RELASZ` 显著缩小的趋势。

传统 `Elf64_Rela` 每条 24 字节，一个 `RELATIVE` 重定位需要 24 字节；RELR 的地址项 8 字节，位图项可以用 8 字节覆盖 63 个重定位目标——最优情况下压缩比接近 63:1。对于指针密集的 libc，RELR 可以把 `DT_RELA` 段体积削减 60-80%，这直接减少了 `.so` 文件大小和映射内存的冷启动开销。

### IFUNC resolver 调用流程在 mini loader 中的实现

mini loader 的局限性表格里有一项"IFUNC 解析——未处理"，指的是 `R_*_IRELATIVE` 重定位类型。在现代 Linux 发行版的 libc 和 libm 中，**几乎所有性能关键函数**（`memcpy`、`strlen`、`sin`、`cos` 等）都通过 IFUNC 在运行时选择最优实现——mini loader 若不处理 `R_X86_64_IRELATIVE`，加载 libm 后调用 `sin` 会跳到未初始化的地址，立即 `SIGSEGV`。

> IFUNC 机制的完整规范（`STT_GNU_IFUNC`、GNU indirect function ABI）见 [[19.IFUNC间接函数]]；本节聚焦在 mini loader 里如何**识别并调用 IFUNC resolver**，把 GOT 槽位填入真实选定的实现地址。

#### IFUNC 的核心思想

`R_X86_64_IRELATIVE` 重定位的处理比 `R_X86_64_RELATIVE` 多一步：

```
R_X86_64_RELATIVE：
  *slot = load_bias + addend          // 直接是最终地址

R_X86_64_IRELATIVE：
  resolver_fn = (void*(*)())(load_bias + addend)   // addend 是 resolver 函数的虚拟地址
  *slot = resolver_fn()                             // 调用 resolver，返回值才是最终地址
```

`resolver_fn` 是库自己写的一个函数，它查询 CPUID（或其他运行时特征），选择最适合当前硬件的实现地址并返回。`R_X86_64_IRELATIVE` 的处理必须在 **`R_X86_64_RELATIVE` 完成之后**（resolver 函数本身也可能依赖 RELATIVE 重定位才能正确运行），但在 **`.init`/`init_array` 之前**。

#### IRELATIVE 在 mini loader 中的实现

```c
/* mini_loader.c — 补充 R_X86_64_IRELATIVE 处理 */

/* 在 ⑦ 步 RELA 重定位循环中，补充第三个 else if 分支 */

} else if (rtype == R_X86_64_IRELATIVE) {
    /*
     * IRELATIVE：addend 是 resolver 函数相对 load_bias 的虚拟地址。
     * resolver 函数原型：void *resolver(void)
     * 返回值是最终应填入 slot 的函数地址。
     *
     * 注意：IRELATIVE 必须在所有 RELATIVE 处理完之后再处理，
     * 否则 resolver 自身可能还没被正确重定位。
     */
    typedef void *(*ifunc_resolver_t)(void);
    ifunc_resolver_t resolver = (ifunc_resolver_t)(load_bias + r->r_addend);
    *target = (uint64_t)resolver();   /* 调用 resolver，取得最优实现地址 */
}
```

由于顺序依赖，建议将 `IRELATIVE` 的处理**单独放在 RELA 遍历的第二轮**（先跑一遍处理非 IRELATIVE 的所有条目，再跑一遍处理 IRELATIVE）：

```c
/* ⑦-a：第一遍——处理 RELATIVE + GLOB_DAT（不含 IRELATIVE） */
for (size_t i = 0; i < nrela; i++) {
    Elf64_Rela *r = &rela[i];
    int rtype = ELF64_R_TYPE(r->r_info);
    uint64_t *slot = (uint64_t *)(r->r_offset + load_bias);

    if (rtype == R_X86_64_RELATIVE) {
        *slot = load_bias + r->r_addend;
    } else if (rtype == R_X86_64_GLOB_DAT) {
        /* ... 同前 ... */
    }
    /* IRELATIVE 暂跳过 */
...
/* ⑦-c：第二遍——处理 IRELATIVE（此时 RELATIVE 已全部完成） */
for (size_t i = 0; i < nrela; i++) {
    Elf64_Rela *r = &rela[i];
    if (ELF64_R_TYPE(r->r_info) != R_X86_64_IRELATIVE) continue;

    uint64_t *slot = (uint64_t *)(r->r_offset + load_bias);
    typedef void *(*ifunc_resolver_t)(void);
    ifunc_resolver_t resolver = (ifunc_resolver_t)(load_bias + r->r_addend);
    *slot = (uint64_t)resolver();
}
```

#### 用 `readelf` 找出 IRELATIVE 条目

```bash
# 查看 libc 或 libm 中有多少 IRELATIVE 条目
readelf -r /lib/x86_64-linux-gnu/libm.so.6 | grep IRELATIVE | head -10
```

```text
# 示意输出
000000029f30  000000000025 R_X86_64_IRELATIVE                    1e1c0
000000029f38  000000000025 R_X86_64_IRELATIVE                    1e210
000000029f40  000000000025 R_X86_64_IRELATIVE                    1e260
...
```

```bash
# 对 addend（即 resolver 函数地址）反汇编，验证其 CPUID 查询逻辑
# 示意：addend = 0x1e1c0，加上实际 load_bias 后查看
objdump -d /lib/x86_64-linux-gnu/libm.so.6 | awk '/^.*1e1c0.*:/{flag=1} flag{print; if(/ret/)exit}'
```

```text
# 示意输出（resolver 函数通常以 cpuid 指令开头）
   1e1c0:  55                      push   %rbp
   1e1c1:  48 89 e5                mov    %rsp,%rbp
   1e1c4:  e8 ...                  call   <__get_cpu_features>
   1e1c9:  ...                     test   (AVX/SSE 特征位)
   1e1cd:  75 ...                  jne    <use_avx_impl>
   1e1cf:  48 8d 05 ...            lea    <sse2_sin>(%rip),%rax
   1e1d3:  c3                      ret
   ...
<use_avx_impl>:
   1e1e0:  48 8d 05 ...            lea    <avx_sin>(%rip),%rax
   1e1e3:  c3                      ret
```

> [!warning] 示例输出为示意
> 实际 resolver 的反汇编随 glibc/libm 版本而变；关键特征是"根据 CPU 特征（CPUID）在两到多个实现地址之间选择并 `ret`"的模式。

#### IFUNC 与 mini loader 的安全边界

调用 resolver 是真实的**用户态函数调用**——mini loader 必须确保在此之前：

1. **被加载库的代码段已映射并有可执行权限**（`PROT_EXEC`）：否则 resolver 的机器码无法执行。
2. **resolver 依赖的所有 `RELATIVE` 重定位已完成**：resolver 通常会访问 `.rodata` 中的常量表或调用辅助函数，这些都依赖 `load_bias` 加成之后的正确地址。
3. **不要在 `mprotect` 上锁（RELRO）之后再处理 IRELATIVE**：若提前把 `.got` 变为只读，IRELATIVE 的回填 `*slot = resolver()` 会 `SIGSEGV`。正确顺序：映射 → RELATIVE → RELR → IRELATIVE → mprotect RELRO → 调用 init_array。
