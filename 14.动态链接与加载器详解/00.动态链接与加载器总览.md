...
> 作者原创系列，共 **16** 篇。本系列承接 [[11.ELF文件详解/00.ELF文件详解总览|ELF]]、[[12.Mach-O文件详解/00.Mach-O文件详解总览|Mach-O]]、[[13.PE文件详解/00-合集总览-PE文件详解|PE]] 三部曲——三部曲讲"可执行文件躺在磁盘上的**静态结构**"，本系列讲它"如何被**加载进内存、链接、重定位、最终跑起来**"。全程贯穿 Linux `ld.so` / macOS `dyld` / Windows Loader 三平台对照。
