...
Android 13（API 33）在 v3 基础上引入了 **v3.1**，通过独立的 Block ID `0x1b93ad61` 区分于 v3 的 `0xf05368c0`，主要解决了 v3 密钥轮换（rotation）在低版本系统上的兼容问题。

**设计动机**：v3 要求系统同时理解 lineage 链中的所有证书，但 Android 9–12 的实现存在一些边界处理问题，当 lineage 链超过两级时某些系统版本的兼容性不一致。v3.1 的策略是：

- **Android 13+**：使用 v3.1 块（Block ID `0x1b93ad61`），其中的轮换（rotation）信息只对 API 33+ 生效
- **Android 9–12**：回退到 v3 块（Block ID `0xf05368c0`），保持与旧系统的兼容行为
- 实际效果是同一 APK 的 Signing Block 可以**同时包含 v3 和 v3.1** 两个 ID-value 对

**rotation 处理细节**：v3.1 的 lineage 条目中新增了 `minSdkVersion` 字段，允许开发者指定"此次轮换仅对 API X 及以上版本的系统生效"，使得旧系统继续使用旧密钥验证，新系统使用新密钥验证——解决了跨版本密钥轮换的部署难题。

```bash
# 使用 apksigner 生成含 v3.1 的签名（需 build-tools 33.0+）
apksigner sign \
  --ks new_key.jks \
  --ks-key-alias new_alias \
  --lineage lineage.bin \
  --rotation-min-sdk-version 33 \   # v3.1 对 API 33+ 生效
  --v3-signing-enabled true \
...
逆向时，若发现 APK Signing Block 中同时存在两个不认识的 ID，可通过查表确认：`0x7109871a`（v2）、`0xf05368c0`（v3）、`0x1b93ad61`（v3.1）、`0x42726f6e`（v4 idsig 元数据占位）。

---

### 签名算法安全与 SigningBlock 滥用

**DSA 弱随机数（k 复用）攻击**：

v1 签名支持 DSA 算法（后缀 `.DSA` 的 `CERT.DSA` 文件）。DSA 签名的安全性依赖随机数 `k`：若同一私钥对两条不同消息使用了相同的 `k`，攻击者可从两个签名中解出私钥。历史上多个 Android App 因使用不安全的随机数生成器（如 `java.util.Random` 而非 `SecureRandom`）导致 DSA `k` 复用而私钥泄露。

```python
# k 复用检测示意（研究目的）：若两次 DSA 签名 r 值相同，则 k 相同
# r = (g^k mod p) mod q
# 相同 r → 相同 k → 可解出私钥 x = (s1-s2)^{-1}*(k*(s1-s2)-(z1-z2)) mod q
```

> [!warning] 示例输出为示意

现代 `apksigner` 默认使用 RSA 或 ECDSA（不再推荐 DSA），且强制使用 `SecureRandom`，但旧版工具链生成的历史 APK 仍需审计。

**RSA 密钥长度安全基线**：

| 密钥长度 | 当前安全评估 | 说明 |
|---|---|---|
| RSA-1024 | 已不安全 | 可被因式分解（2010 年以后不推荐） |
| RSA-2048 | 当前安全基线 | Google Play 最低要求 |
| RSA-4096 | 高安全 | 签名速度较慢，但对移动端可接受 |
| ECDSA P-256 | 等效 RSA-3072，推荐 | 更小密钥，更快签名 |

逆向时查看证书密钥长度：

```bash
apksigner verify --print-certs app.apk | grep "key size"
# 示例输出：Signer #1 key size (bits): 2048
```

> [!warning] 示例输出为示意

**Signing Block 滥用：厂商自定义 ID**：

APK Signing Block 的 ID-value 对结构允许任意第三方 ID（只要不与 Google 已注册的 ID 冲突）。商业加固（壳）厂商利用这一特性**在 Signing Block 中嵌入私有 ID-value 数据**：

| 厂商 | 典型自定义 ID | 用途 |
|---|---|---|
| 360 加固 | `0x71777777` 等私有 ID | 存储加固元数据、壳配置 |
| 腾讯乐固 | 私有 ID | 存储加固版本、解密参数 |
| 通用壳方案 | 私有 ID | 在 APK 安装前预校验 Signing Block 中的自定义值 |

运行时校验流程：壳的 native 代码在 `JNI_OnLoad` 中调用 `getPackageCodePath()` 找到 APK 文件路径，然后 `mmap` + 解析 Signing Block，读取自定义 ID-value 条目中的校验值（例如加密的 Merkle 根或厂商密钥签名）。若该值缺失或被篡改，壳拒绝解密保护的 DEX，App 无法启动。

逆向对抗策略：找到壳读取 Signing Block 的函数（通常通过搜索 `APK Sig Block 42` 魔数或 `0x71` 相关常量），hook 返回值使其始终通过校验；或直接静态解析 Signing Block 结构，提取自定义条目内容。

---

### Google Play App Signing 与历史漏洞

**Google Play App Signing（GPAS）架构**：

自 2017 年起，Google Play 提供 **App Signing by Google Play** 服务，采用**双密钥**架构：
...
上传密钥 (upload key)      应用签名密钥 (app signing key)
     ↓                            ↓
用于签名上传到 Play 的 APK   用于签名分发给用户的 APK
```

关键含义：用户从 Play Store 下载的 APK 签名与开发者上传的 APK 签名**不同**。这对逆向分析有以下影响：

1. **运行时签名自校验必须使用应用签名密钥的指纹**，而非上传密钥
2. 若 App 的签名自校验代码硬编码了上传密钥指纹，在用户侧会因校验失败而崩溃（常见 Bug）
3. 逆向时从 `apksigner verify --print-certs` 看到的是应用签名密钥（Google 管理的那个）

**查询 GPAS 签名密钥**（App 开发者）：

在 Play Console → 设置 → 应用签名 → 可以查看应用签名密钥证书的 SHA-256/MD5 指纹，用于 App 内签名自校验的硬编码值。

**历史漏洞**：

| 漏洞 | CVE | 影响版本 | 原理 |
|---|---|---|---|
| **Master Key** | CVE-2013-4787 | Android 1.0–4.2 | ZIP 允许同名文件时，`PackageManager` 用第一个条目校验签名，但 Dalvik 执行最后一个条目——两者内容不同，签名验证"通过"但执行了未签名代码 |
| **FakeID** | CVE-2014-3315 | Android 2.1–4.4 | `PackageManager` 校验证书链时存在逻辑漏洞，攻击者可伪造中间证书使恶意 App 具备 Adobe Reader 插件等特权证书的权限 |
| **Janus** | CVE-2017-13156 | Android 5.0–8.0 v1-only | 见本篇 v1 章节 |
| **StrandHogg 2.0** | CVE-2020-0096 | Android 6–10 | 通过 `Activity` 任务劫持伪造签名应用的 UI（与签名机制本身关系间接） |

**Master Key（CVE-2013-4787）深度说明**：

ZIP 规范本身允许文件中存在两个同名条目。Android 的 `ZipFile` 实现（读取签名）取第一个同名条目，而 `ZipInputStream`（执行 DEX）取最后一个。攻击者将恶意 `classes.dex` 作为第二个同名条目追加，签名校验读第一个（合法 DEX）通过，但执行时读第二个（恶意 DEX）。Android 4.2 以后修复了这一差异（统一选取最后一个条目）。
