...
Android 8.0（API 26）引入 DEX 版本 038，Android 9.0（API 28）引入 039，扩展了 Java 8 特性和方法句柄支持所需的字节码指令。

#### invoke-polymorphic（方法句柄多态调用）

`invoke-polymorphic` 对应 Java 的 `MethodHandle.invoke()` / `MethodHandle.invokeExact()`，允许在运行时以任意签名调用方法句柄：
...
#                  ↑ 两个引用 ID：方法引用 + proto 引用（区别于 invoke-virtual 仅一个）
move-result v2
```

指令格式（`45cc`）：与 `invoke-virtual` 的 `35c` 格式不同，`invoke-polymorphic` 携带**两个**常量池引用：method_ref（固定为 `MethodHandle.invoke` 或 `invokeExact`）和 proto_ref（实际调用签名）。

**逆向影响**：老版 baksmali（< 2.5.0）不认 `invoke-polymorphic`，遇到会报错或输出乱码。若逆向目标 minSdk ≥ 26，需确保工具版本足够新。

#### invoke-custom（invokedynamic / Kotlin Lambda）

`invoke-custom` 是 Java invokedynamic 字节码在 DEX 层的对应实现，Kotlin Lambda、方法引用（`::method`）、Java 8 流操作都会生成此指令：

```smali
# Kotlin Lambda（示意）：list.forEach { println(it) }
# 编译器生成的 invoke-custom 调用 bootstrap method

invoke-custom {v0}, call_site_0@0
# call_site_0 是 DEX 038+ 新增的 call_site_id_item，
# 指向一个 CallSite（包含 bootstrap method + 静态参数）
```

`call_site_id_item` 在 DEX map_list 中类型为 `0x0007`，对应一个 `encoded_array_item`：

```
call_site 结构（encoded_array）：
  [0] bootstrap_method_handle  ← MethodHandle 引用（指向 LambdaMetafactory::metafactory 等）
  [1] method_name              ← 字符串（被调用的接口方法名，如 "apply"）
  [2] method_type              ← MethodType（接口方法签名）
  [3..] extra args             ← bootstrap 额外参数（实际 Lambda 实现的 MethodHandle 等）
```

**逆向含义**：jadx 可以反编译 `invoke-custom` 为接近原始 Lambda 的 Java 代码；但老版工具处理不当会反编译失败或显示为 `/* synthetic */` 方法引用，此时直接看 smali 更可靠。

#### const-method-handle / const-method-type（DEX 039）

这两条指令加载方法句柄/类型为常量，对应 Java 9 的常量池 `CONSTANT_MethodHandle_info`：

```smali
# DEX 039：加载 MethodHandle 常量到寄存器
const-method-handle v0, invoke-virtual@Ljava/lang/String;->length()I

# 加载 MethodType 常量
const-method-type v1, (Ljava/lang/String;)I
```

这两条指令在现有 Android 代码中较少见（Java 9+ 源码特性），但 Kotlin 编译器某些版本会生成；逆向时遇到需确认 baksmali ≥ 2.5.2。

---

### switch 表结构

Dalvik 提供两种 switch 实现，以 payload 数据嵌在字节码流末尾的方式存储跳转表。

#### packed-switch payload 格式

适用于 case 值连续（如 `1,2,3,4,5`）的 switch：

```
packed-switch 指令：
  packed-switch vA, +BBBB   ← BBBB 是相对当前 PC 的 code unit 偏移（指向 payload）

packed-switch payload 格式（存在字节码流中的某处，通常方法末尾）：
  uint16_t  ident           = 0x0100   ← 魔数，标识这是 packed-switch payload
  uint16_t  size            = N        ← case 数量
  int32_t   first_key       = K        ← 第一个 case 值（之后连续递增）
  int32_t   targets[N]      ← 每个 case 对应的相对跳转偏移（code unit，相对 switch 指令）
```

例：`switch(x) { case 10: ...; case 11: ...; case 12: ... }`，`first_key=10`，`size=3`，`targets=[offset_case10, offset_case11, offset_case12]`。

#### sparse-switch payload 格式

适用于 case 值稀疏（如 `1, 100, 10000`）的 switch：

```
sparse-switch 指令：
  sparse-switch vA, +BBBB

sparse-switch payload：
  uint16_t  ident           = 0x0200   ← 魔数，区别于 packed-switch 的 0x0100
  uint16_t  size            = N        ← case 数量
  int32_t   keys[N]         ← 所有 case 值（升序排列）
  int32_t   targets[N]      ← 对应的相对跳转偏移（与 keys 一一对应）
```

**smali 中的表示**（baksmali 自动生成，人类可读）：
...
:pswitch_data_0
.packed-switch 0x1     # first_key = 1
    :pswitch_0         # case 1 → label
    :pswitch_1         # case 2 → label
.end packed-switch
```

#### 加固壳伪造条目迷惑静态分析

加固壳常用 packed-switch / sparse-switch payload 作为混淆载体：

1. **插入大量无效 case**：`size` 声明 1000 个 case，但实际只有 3 个有效目标，其余 `targets` 填写指向方法末尾或非法地址的偏移——jadx 在尝试还原 switch-case 时会产生大量死分支或崩溃。

2. **payload 嵌套在伪指令中**：把 switch payload 的 `ident` 字节（`0x0100` 或 `0x0200`）改为不合法值，使静态分析工具错误解析后续字节为指令，扰乱控制流图。

3. **goto 跳过 payload 区**：payload 本身不是可执行指令，正常代码通过 `goto` 跳过它；但若把某些混淆代码伪装成 payload（或反之），工具的"从 payload 逆推 case 数量"会被误导。

识别技巧：用 `dexdump -d` 检查 `packed-switch`/`sparse-switch` 指令后的 payload 魔数（`01 00` 或 `02 00`），核实 `size` 与实际有效 label 数量是否一致。

---

### VerifyError 与 smali 动态调试

#### ART 字节码验证器规则

ART 字节码验证器（`verifier::MethodVerifier`）在类加载时对每个方法做静态分析，主要规则：

**类型流（Type Flow）分析**：验证器模拟每条指令的执行，跟踪每个寄存器的类型状态。违规示例：

```smali
# 错误：将 int 赋给 Object 引用寄存器
const/4 v0, 0x1            # v0 = int
check-cast v0, Ljava/lang/String;  # VerifyError: v0 is int, not reference
```

**φ 节点类型合并**：控制流汇合处，各分支对同一寄存器的类型必须兼容：

```smali
    if-eqz v1, :cond_0
    const-string v0, "hello"   # v0 : String
    goto :merge
:cond_0
    const/4 v0, 0x1            # v0 : int  ← 与 String 不兼容
:merge
    # VerifyError: v0 type conflict at merge point
    invoke-virtual {v0}, Ljava/lang/Object;->toString()Ljava/lang/String;
```

修正：在 `:cond_0` 分支也用 `const-string` 或保持类型一致。

**异常路径一致性**：try 块内每条指令出现异常时跳到 catch handler，验证器要求 catch handler 入口处的寄存器类型与 try 块内一致；`move-exception vX` 后 `vX` 必须是 `Throwable` 子类。

**其他常见 VerifyError 原因**：

| 原因 | 示例 |
|---|---|
| `.registers` 数量与实际指令使用不符 | 声明 `.registers 2`，但指令用了 `v3` |
| `invoke-virtual` 对 `null` 类型 | 类型流分析认定某寄存器可能为 null 时调用非 null 方法 |
| 方法引用签名与实际不符 | 调用不存在的方法/字段 |
| `return` 类型与方法声明不符 | `void` 方法执行 `return v0` |
| 跳转目标超出方法边界 | 修改 smali 时 goto/if 标签未同步调整 |

#### smali 层 JDWP 断点调试

ART 支持 JDWP（Java Debug Wire Protocol），允许对 smali 级别的字节码设置断点。前提：App 的 `android:debuggable="true"`（或使用 Magisk/`adb shell am set-debug-app -w com.example.app`）。

**用 Android Studio 附加调试（smali 断点）**：

1. apktool 解包得到 smali 源码
2. Android Studio 安装 Smalidea 插件（或直接打开 smali 文件）
3. adb 转发 JDWP 端口：
...
4. Android Studio → Run → Attach to Process → 选择 `8700` 端口
5. 在 smali 文件中打断点（`if-eqz`/`invoke-virtual` 等行）
6. 触发 App 执行到该位置时断点命中，可查看所有寄存器（v0–vN）和对象内容

**用 jadx 内置调试器**（jadx >= 1.4.0）：

```bash
jadx-gui app.apk
# 菜单 → Run → Attach debugger
# 输入 JDWP 地址（localhost:8700）
# 在反编译的 Java 视图中设断点，自动映射到字节码层
```
...
> threads
# 在类方法设断点
> stop in com.example.License.checkKey
# 运行到断点
> run
# 查看本地变量
> locals
...
smali 级调试的核心价值：在不修改 APK（无需重签名）的情况下，实时观察方法参数（p0–pN）、局部寄存器（v0–vN）的类型和值，以及执行路径——尤其对于静态分析难以确定的混淆逻辑，动态断点是最高效的验证手段。
