...
`ArtMethod` 结构体的字段偏移随 Android 版本和编译配置变化，逆向时不能硬编码偏移，须动态确定。

#### 从 /proc/\<pid\>/maps 定位 libart.so

```bash
# 找目标进程 PID
adb shell pidof com.example.app

# 读取内存映射，找 libart.so 基址
adb shell cat /proc/$(pidof com.example.app)/maps | grep libart.so
# 输出示例：
# 76a0000000-76b2000000 r-xp 00000000 fd:04 1234 /apex/com.android.art/lib64/libart.so
# ↑ 76a0000000 即 libart.so 在该进程的基址
```

#### Frida 读取 ArtMethod 布局

Frida 提供 `Process.androidVersion` 和 `Java.vm.impl.members` 等接口，可直接查询当前设备的 ArtMethod 布局：

```javascript
// 方法一：通过已知方法获取 ArtMethod 指针，再推断偏移（示意）
Java.perform(function() {
    var String = Java.use('java.lang.String');
    // 获取 String.length() 的 ArtMethod 指针
    var lengthMethod = String.class.getMethod('length');
    // Java.vm.impl 可获取底层 ArtMethod 地址
    var artMethodPtr = Java.vm.impl.jniEnvHandle
        ? Java.vm.impl.methodDescToMethodId(lengthMethod)
        : null;

    if (artMethodPtr) {
        console.log('ArtMethod ptr: ' + artMethodPtr);
        // 读取各偏移，与 AOSP art_method.h 对照
        console.log('[+0x00] declaring_class: ' + artMethodPtr.readPointer());
        console.log('[+0x08] access_flags:    ' + artMethodPtr.add(8).readU32());
        console.log('[+0x0C] dex_method_idx:  ' + artMethodPtr.add(12).readU32());
    }
});

// 方法二：利用 Frida art-binding（更高级，直接查询 ArtMethod 布局）
// 示例：artMethodOffset() 返回各字段相对 ArtMethod 基址的偏移
```

> [!warning] 示例输出为示意；实际偏移须以目标设备版本为准。

#### 对照 AOSP art_method.h 版本

`ArtMethod` 结构体定义在 AOSP `art/runtime/art_method.h`。不同版本变化关键点：

| Android 版本 | 变化 |
|---|---|
| Android 5–6 | 无 `hotness_count_`；`entry_point_from_interpreter_` 存在 |
| Android 7 | 引入 `hotness_count_`（JIT 热度）；`entry_point_from_interpreter_` 仍存 |
| Android 8–9 | 指针压缩可能影响 GcRoot 大小 |
| Android 10 | CompressedReference 变化；部分编译选项影响字段对齐 |
| Android 12+ | Hidden API 收紧；`access_flags_` 新增 bit 控制 |

**实践方法**：已知 Android 版本后，在 AOSP 对应 tag 的 `art/runtime/art_method.h` 中搜索字段定义顺序，结合 `sizeof(GcRoot<mirror::Class>)` 等计算实际偏移（arm64 无压缩指针时 GcRoot 为 8 字节）。

```bash
# 在 AOSP 源码中查找 ArtMethod 字段偏移
# （以 Android 12 为例）
curl -s "https://android.googlesource.com/platform/art/+/android-12.0.0_r1/runtime/art_method.h?format=TEXT" | base64 -d | grep -n "declaring_class_\|access_flags_\|entry_point"
```

主流 hook 框架（Frida、SandHook、Epic）内置版本偏移表，覆盖 Android 5–14，直接复用可节省大量调试时间。

---

### oat 内部查找与 profile

#### OatHeader → OatDexFile → OatClass → OatMethodOffsets（oatdump 原理）

`.oat` 文件内部的导航链路如下：

```
OatHeader（位于 ELF .rodata 节开头）
  magic = "oat\n"
  version = "199\0"（随 Android 版本变化）
  key-value store → compiler-filter, debuggable, ...
  dex_file_count
  → OatDexFile[dex_file_count]
      dex_file_location_size / dex_file_location_data  ← DEX 路径字符串
      dex_file_checksum
      dex_file_offset（→ vdex 中嵌入的 DEX 或 .oat 内嵌 DEX）
      class_offsets_offset（→ OatClass 数组）
      → OatClass[class_defs_size]
          status（已编译/部分编译/未编译）
          type（kAllCompiled / kSomeCompiled / kNoneCompiled）
          bitmap（标注哪些方法已编译，仅 kSomeCompiled 时存在）
          → OatMethodOffsets[compiled_method_count]
              code_offset（机器码在 .oat 内的偏移）
```

`oatdump` 工具遍历这条链路输出所有方法的机器码偏移、对应的 DEX 方法 idx 等信息：

```bash
# adb 上直接运行（需 root 或 shell 权限）
adb shell oatdump \
    --oat-file=/data/app/com.example.app-xxxxx/oat/arm64/base.odex \
    --output=/data/local/tmp/dump.txt
...
# version: 199
# ...
# DEX FILE #0: classes.dex
#   CLASS #0 (Lcom/example/MainActivity;):
#     METHOD #0 (onCreate):
#       CODE: (code_offset=0x1234, size=128)
#       ... ARM64 disassembly ...
```

> [!warning] 示例输出为示意

逆向时使用 oatdump 的核心价值：将 IDA/Ghidra 看到的机器码地址与 Java 方法名精确对应，解决"机器码偏移 → Java 方法"的映射问题。

#### profile 格式与 cloud profile 对抗

**profile 文件格式**（`primary.prof`）是二进制格式，记录热点方法列表：

```
profile header：
  magic = 0x00 + "pro\n"  （Android 9+ 格式）
  version = "010P\0"（版本随 Android 变）
  number_of_dex_files
  uncompressed_data_size
  compressed_data_size
  compressed_data（zlib 压缩）
    → ProfileData：
        每个 DEX 文件的 checksum + 热点方法 index 集合（bitset 或 list）
```

```bash
# 查看 profile 内容（需 root）
adb shell ls /data/misc/profiles/cur/0/com.example.app/
# primary.prof  ref.prof

# profman 工具（AOSP 自带）解码 profile
adb shell profman --profile-file=/data/misc/profiles/cur/0/com.example.app/primary.prof \
    --apk=/data/app/com.example.app-xxx/base.apk \
    --dex-location=classes.dex \
    --dump-classes-and-methods > /data/local/tmp/profile_dump.txt
adb pull /data/local/tmp/profile_dump.txt .
```

> [!warning] 示例输出为示意

**cloud profile 与逆向对抗**：Google Play 向设备下发的 cloud profile（`baseline.prof` 或 Play Store 后台下发的 profile）会提前 AOT 编译热点路径，导致逆向者在 cold path（未被 profile 覆盖的方法）看到解释器执行，而 hot path 直接是机器码——两者行为差异可能导致 hook 时机不一致。

对抗手段：
1. 删除或清空 profile 文件，强制 App 从解释器启动（所有方法走同一路径）
2. 用 `adb shell cmd package compile -m speed com.example.app` 强制全量 AOT
3. 分析 `.oat` 中的 `OatMethodOffsets.code_offset == 0`（未编译方法）与非零（已 AOT 编译）的分布
...
Android 9 引入 Hidden API 限制（非公开 SDK API 的访问控制），Android 12 进一步收紧，影响逆向中常用的反射访问：

| API 类别 | Android 9 限制 | Android 12+ 变化 |
|---|---|---|
| blacklist（现称 blocked） | 阻止所有访问，抛 NoSuchMethodException | 覆盖更多类，包括部分 ART 内部类 |
| greylist-max-o（max-target-o） | targetSdk ≤ 27 可访问 | 标准 App 完全不可访问 |
| `getDeclaredMethods()` 返回 | 过滤 hidden 方法 | 过滤范围扩大 |

**影响 Frida `Java.use` 的场景**：当尝试 hook 系统内部类（如 `android.app.ActivityManagerService` 内部方法或 `libcore` 私有类）时，`Java.use()` 底层通过反射获取方法时会被 Hidden API 机制拦截，导致 `NoSuchMethodException`：

```javascript
// 受 Hidden API 限制的 hook（示意）
Java.perform(function() {
    // 以下可能在 Android 12+ 抛出 NoSuchMethodException
    var VMRuntime = Java.use('dalvik.system.VMRuntime');
    // VMRuntime 是 maxTargetSdk restricted API

    // 绕过方案 1：通过 JNI 调用（绕过 Java 层反射检查）
    var getRuntime = new NativeFunction(
        Module.findExportByName('libart.so', '_ZN3art9JNIEnvExt...'),
        'pointer', ['pointer']
    );
    // 直接操作 ArtMethod 指针，无需 Java 反射

    // 绕过方案 2：使用 Frida 内置的 Java.vm.impl（利用 JVMTI 绕过限制）
});
```

**系统级绕过**：在 root 设备上，可通过 Magisk 模块或 `adb shell settings put global hidden_api_policy 1` 将策略调至最宽松（仅警告不拦截）：

```bash
# 临时关闭 Hidden API 限制（需 adb root）
adb shell settings put global hidden_api_policy 1  # 只警告
# 或
adb shell am set-hidden-api-checks-enabled false    # Android 9–11
```

#### JVMTI 变化与 Frida Java.use

JVMTI（JVM Tool Interface）是 ART 提供给调试/代理工具的标准接口，Frida 依赖它实现 `Java.use()` hook：

| 特性 | Android 9–11 | Android 12+ |
|---|---|---|
| JVMTI 可用性 | `debuggable=true` 可用；root 下可强制 | 收紧：非 debuggable App 通过 JVMTI 访问受限 |
| `RetransformClasses` | 可重定义已加载类的方法体 | 限制增强，部分系统类不可重定义 |
| `ClassFileLoadHook` | 可用于脱壳（捕获 DEX 加载） | 对 boot classpath 类的 hook 受限 |

**实际影响**：在 Android 12+ 非 debuggable App 上，Frida `Java.use()` 对 **App 自身代码**通常仍然有效（通过 `--attach-device-no-pause` 等变通）；但对**系统服务类**（如 AMS、PMS 的方法）的 hook 成功率下降，需改用 native hook（直接修改 `ArtMethod.entry_point`）或通过 Xposed/LSPosed 框架（hook 在系统进程，不受 App 的 JVMTI 限制）。
