...
### .debug_frame 与 .eh_frame 何时各自生成

#### 生成时机与编译开关的对应关系

GCC/Clang 对两个 CFI 节的生成逻辑完全独立，由不同的编译选项控制，理解这一点对分析二进制文件至关重要：

| 节 | 默认行为 | 生成开关 | 抑制开关 |
|----|---------|---------|---------|
| `.eh_frame` | **默认生成**（x86-64 System V ABI 要求，无论是否 `-g`）| `-fasynchronous-unwind-tables` | `-fno-asynchronous-unwind-tables` |
| `.debug_frame` | **默认不生成**（即使 `-g` 也不生成） | 需要 `-g` + `-fasynchronous-unwind-tables` 同时生效，或使用汇编 `.cfi_sections .debug_frame` | — |

**`.eh_frame` 为何是默认行为**：x86-64 psABI 要求所有函数必须有正确的 CFI 记录，以便信号处理程序、`pthread_cancel`、`backtrace()` 等无需调试信息即可正确展开栈帧。Linux 内核调度和 `setjmp/longjmp` 等机制也依赖此能力。

**`.debug_frame` 为何默认不生成**：调试器（GDB/LLDB）在 `.eh_frame` 存在时优先使用它；`.debug_frame` 包含的展开规则内容与 `.eh_frame` 高度重复，双重存储会使调试信息体积增大，故 GCC 默认省略 `.debug_frame`。
...
# 只生成 .eh_frame（默认，无 -g）
gcc -O2 foo.c -o foo
# 结果：有 .eh_frame，无 .debug_frame

# -g 默认也只生成 .eh_frame（plus .debug_* 调试节）
gcc -g foo.c -o foo
# 结果：有 .eh_frame + .debug_info/line/...，无 .debug_frame

# 同时生成 .debug_frame（需要显式要求）
gcc -g -fasynchronous-unwind-tables foo.c -o foo
# 注意：-fasynchronous-unwind-tables 在 x86-64 上已默认开启，
# 但它控制的是 .eh_frame 的生成；要同时生成 .debug_frame，
# 需要在汇编层面用 .cfi_sections .debug_frame 或 .cfi_sections .eh_frame,.debug_frame

# 汇编伪指令强制同时生成两节
# 在 .s 文件头部添加：
#   .cfi_sections .eh_frame, .debug_frame

# 禁用 .eh_frame（C 代码，不推荐用于生产）
gcc -fno-asynchronous-unwind-tables foo.c -o foo
# 结果：无 .eh_frame，backtrace() 依赖帧指针（需 -fno-omit-frame-pointer）
```

#### 调试器读取优先级

当两个节同时存在时，主流调试器的优先策略：

| 调试器 | 优先读取 | 降级策略 |
|--------|---------|---------|
| **GDB** | `.debug_frame`（若存在） | 降级到 `.eh_frame` |
| **LLDB** | `.eh_frame` | 降级到 `.debug_frame` |
| **libunwind（GNU）** | `.eh_frame` | 无 `.eh_frame` 则尝试 `.debug_frame` |
| **libunwind（LLVM）** | `.eh_frame` / `.eh_frame_hdr` | 无则尝试 `.debug_frame` |
| **perf** | `.eh_frame`（via libunwind） | 若两者都无，使用帧指针链 |

GDB 优先 `.debug_frame` 是历史设计决策——`.debug_frame` 使用绝对地址编码，解析更简单，且对 DWARF 调试节更"权威"；而 `.eh_frame` 的 pcrel 编码在某些情况下需要运行时地址修正。

#### stripped 二进制的降级行为

`strip`/`strip --strip-debug` 对两个节的影响截然不同：

```bash
# 查看 strip 前的节列表
readelf -S program | grep -E 'debug_frame|\.eh_frame'

# strip --strip-debug：移除所有 .debug_* 节，保留 .eh_frame
strip --strip-debug program
# 结果：.debug_frame 被删除，.eh_frame 保留

# strip（不带 --strip-debug）：与 strip --strip-all 等效
strip program
# 结果：.debug_frame 被删除，.eh_frame 保留（因为 SHF_ALLOC 节不被 strip 删除）
# 注意：.eh_frame 有 SHF_ALLOC 标志，strip 默认不删除运行时必需节

# 验证 strip 后的状态
readelf -S program_stripped | grep -E 'eh_frame|debug_frame'
# 预期：只剩 .eh_frame（及 .eh_frame_hdr），无 .debug_frame
```

**stripped 后的调试降级**：调试器在 `.debug_frame` 缺失的情况下自动降级到 `.eh_frame`：

```text
降级链（从优到劣）：
  .debug_frame（精确，含调试语义）
    ↓ 不存在时
  .eh_frame（精确，但部分信息如异常 augmentation 噪声较多）
    ↓ 不存在时（-fno-asynchronous-unwind-tables 编译）
  帧指针链（rbp 链表遍历，需 -fno-omit-frame-pointer）
    ↓ 帧指针也被优化掉时
  启发式猜测（不可靠，GDB 的 "Cannot access memory" 来源之一）
```

**实际影响举例**：生产环境中常见 `.debug_frame` 被 strip 后，GDB 的 `backtrace` 仍然工作（通过 `.eh_frame`），但某些高级功能（如 GDB 的 `info frame` 显示完整寄存器状态、`finish` 命令的准确返回点）可能依赖 `.debug_frame` 提供的额外信息。

```bash
# 检查当前二进制包含哪些展开信息节
readelf -S program | grep -E '\.debug_frame|\.eh_frame'

# 如果只想要 .debug_frame 用于调试但不想要 .eh_frame 占运行时内存（极少见用途）
gcc -g -fno-asynchronous-unwind-tables foo.c -o foo
# 再手动加 .cfi_sections .debug_frame 到汇编
```
