...
#### load_elf_binary 关键路径

Linux 内核在 `fs/binfmt_elf.c` 中实现 ELF 加载器，入口函数为 `load_elf_binary()`。其关键执行路径如下：

```
execve()
  └─ search_binary_handler()
       └─ load_elf_binary()          ← binfmt_elf.c 注册的 handler
            ├─ 读 ELF 头，验证魔数 / e_machine / e_ident[EI_CLASS]
            ├─ 读 PT_INTERP（若有）→ 记录解释器路径
            ├─ 遍历 PT_LOAD → elf_map()（内部 vm_mmap/mmap_region）
            │    ├─ 对齐到 p_align（通常 PAGE_SIZE 或 HPAGE_SIZE）
            │    ├─ p_flags → PROT_READ / PROT_WRITE / PROT_EXEC
            │    └─ p_memsz > p_filesz → set_brk() 清零 BSS 区域
            ├─ 若静态 ELF：elf_entry = ehdr.e_entry
            ├─ 若动态 ELF：
            │    ├─ 加载 ld-linux.so（再次调用 load_elf_interp）
            │    └─ elf_entry = interp 的 e_entry（而非主程序 e_entry）
            ├─ create_elf_tables()   ← 填充栈顶的 auxv / envp / argv
            └─ start_thread()        ← 设置 sp/ip，切换到用户态入口
```

**PT_LOAD 页对齐细节**：`elf_map()` 在 mmap 前将 `p_vaddr` 向下对齐到页边界，`p_filesz` 对应的文件内容通过 `MAP_PRIVATE` 映射，超出页边界的尾部字节（`p_vaddr + p_filesz` 到下一页边界）内核手动清零，防止信息泄露（利用 `clear_user` / `memset` 清尾页）。

**`set_brk` 清 BSS**：当 `p_memsz > p_filesz` 时，内核调用 `set_brk(elf_bss, elf_brk, bss_prot)` 在 `p_filesz` 之后的内存区域建立匿名 `MAP_ANONYMOUS|MAP_FIXED` 映射，内核保证这段内存全零，对应 `.bss` 节内容。

#### auxv（辅助向量）完整条目

内核在 `create_elf_tables()` 中将辅助向量（auxiliary vector）压入栈顶，位于 `envp[]` 之上，由 `AT_NULL` 结束。辅助向量以 `(key, value)` 对形式传递，动态链接器和 C 运行库在启动时读取：

| AT 类型 | 值 | 含义 | 逆向 / 漏洞意义 |
|---|---|---|---|
| `AT_NULL` | 0 | 辅助向量结束标记 | — |
| `AT_IGNORE` | 1 | 忽略此条目 | — |
| `AT_EXECFD` | 2 | 可执行文件描述符（旧式解释器）| — |
| `AT_PHDR` | 3 | 程序头表在内存中的虚拟地址 | PIE 加载基址 = AT_PHDR − e_phoff |
| `AT_PHENT` | 4 | 程序头表条目大小（`e_phentsize`）| — |
| `AT_PHNUM` | 5 | 程序头表条目数（`e_phnum`）| — |
| `AT_PAGESZ` | 6 | 系统页大小（通常 4096）| heap spray 步长参考 |
| `AT_BASE` | 7 | 动态链接器加载基址 | 泄露 ld.so ASLR 基址 |
| `AT_FLAGS` | 8 | 标志（目前始终为 0）| — |
| `AT_ENTRY` | 9 | 主程序入口点（`e_entry` 加 ASLR 偏移）| 泄露主程序 ASLR 基址 |
| `AT_NOTELF` | 10 | 程序不是 ELF（旧式 a.out 兼容）| — |
| `AT_UID` | 11 | 真实用户 ID | 提权检测 |
| `AT_EUID` | 12 | 有效用户 ID | SUID 检测 |
| `AT_GID` | 13 | 真实组 ID | — |
| `AT_EGID` | 14 | 有效组 ID | — |
| `AT_PLATFORM` | 15 | 平台名字符串地址（如 `"x86_64"`）| 栈地址泄露 |
| `AT_HWCAP` | 16 | 硬件能力位掩码（CPU 特性）| IFUNC resolver 用于选优实现 |
| `AT_CLKTCK` | 17 | `times()` 每秒时钟节拍数 | — |
| `AT_SECURE` | 23 | 安全模式标志（SUID/capabilities）| ld.so 忽略 LD_PRELOAD 的依据 |
| `AT_BASE_PLATFORM` | 24 | 基础平台名（某些架构）| — |
| `AT_RANDOM` | 25 | 16 字节随机值在栈上的地址 | stack canary / cookie 来源；泄露可预测 canary |
| `AT_HWCAP2` | 26 | 扩展硬件能力位掩码 | — |
| `AT_EXECFN` | 31 | 可执行文件路径名地址 | 栈地址泄露 |
| `AT_SYSINFO` | 32 | vDSO 系统调用入口（32 位）| — |
| `AT_SYSINFO_EHDR` | 33 | vDSO ELF 头在内存中的地址 | 泄露 vDSO ASLR 基址 |

**逆向与漏洞利用意义**：在有任意读原语时，通过泄露 `AT_BASE`（ld.so 基址）、`AT_ENTRY`（主程序基址）、`AT_SYSINFO_EHDR`（vDSO 基址）即可计算整个进程地址空间的 ASLR 布局。`AT_RANDOM` 所指的 16 字节被 glibc 用作 stack canary 的种子，若能读取该值则可预测 canary。

可用以下方式在运行时查看 auxv：

```bash
# 通过 /proc 查看当前进程辅助向量
cat /proc/self/auxv | od -t dL
LD_SHOW_AUXV=1 /bin/true        # ld.so 解析后以可读形式打印
```

#### 静态 vs 动态 ELF 的内核路径分叉

内核在扫描程序头表时，通过是否存在 `PT_INTERP` 段来区分静态和动态链接，二者走完全不同的后续路径：

| 分支 | 条件 | 内核行为 |
|---|---|---|
| **静态链接** | 无 `PT_INTERP` | `elf_entry = e_entry`（主程序入口）；内核直接切换到此地址 |
| **动态链接** | 有 `PT_INTERP` | 读取解释器路径 → 再次调用 `load_elf_interp()` 把 ld.so 映射进内存 → `elf_entry = ld.so 的 e_entry`；主程序 `e_entry` 通过 `AT_ENTRY` 传递给 ld.so |

动态路径下，内核把控制权交给 ld.so 而非主程序——这意味着 ld.so 是进程的第一段"用户态代码"，由它完成共享库加载、重定位、初始化，最终通过 `AT_ENTRY` 跳回主程序真正入口 `_start`。

静态路径下没有 ld.so 参与，内核直接进入 `_start`，由 C 运行库（glibc 静态版）完成环境初始化后调用 `main()`。
