...
动态链接器在解析一个未定义符号（`st_shndx = SHN_UNDEF`）时，遵循一套精确的优先级规则，这套规则决定了最终使用哪个共享库中的哪个定义。

#### global scope 与 local scope

动态链接器维护两层符号查找范围：

| 范围 | 描述 | 构成 |
|---|---|---|
| **global scope**（全局作用域）| 整个进程中所有以 `RTLD_GLOBAL` 标志加载的模块的已导出符号集合 | 主可执行文件 + `RTLD_GLOBAL` 的共享库 |
| **local scope**（局部作用域）| 某个 `dlopen` 调用显式加载的模块（及其传递依赖），默认为 `RTLD_LOCAL` | `dlopen(path, RTLD_LOCAL)` 加载的库及其依赖 |

默认情况（无显式 `dlopen`）所有库以 `RTLD_GLOBAL` 加载，符号在全局作用域内可见。

#### 标准解析顺序（广度优先搜索，BFS）

对于 global scope，符号查找按以下顺序进行：

```
1. 主可执行文件（ET_DYN 或 ET_EXEC）的 .dynsym
2. 主可执行文件 DT_NEEDED 列表中的第一个库（按声明顺序）
3. 第一个库的 DT_NEEDED 库（深度扩展）
4. 第二个 DT_NEEDED 库 ...
（类似 BFS，加载顺序决定查找顺序）
```

实际上 glibc ld.so 在初始化时会建立一个全局的共享库加载顺序列表（`link_map` 链）。符号查找时按 `link_map` 链从头到尾扫描，找到第一个匹配的定义即返回。

#### DT_SYMBOLIC 与 DF_1_GLOBAL

| 机制 | 作用 |
|---|---|
| `DT_SYMBOLIC`（旧式，不推荐）| 强制本共享库自身的符号引用优先绑定到本库内部定义，再扩展到全局 scope；相当于内部引用跳过主可执行文件优先级 |
| `DF_SYMBOLIC`（`DT_FLAGS` 中的位 0）| `DT_SYMBOLIC` 的现代等价形式，语义相同但用 flags 字段表示 |
| `DF_1_GLOBAL`（`DT_FLAGS_1` 中的位）| 要求动态链接器将本库的符号放入全局作用域（与 `RTLD_GLOBAL` 效果类似）|

```bash
# 检查库是否设置了 DT_SYMBOLIC 或相关 flags
readelf -d libfoo.so | grep -E "SYMBOLIC|FLAGS"
```

`DT_SYMBOLIC` 会影响 C++ 虚函数的覆盖行为，是导致跨 DSO C++ 对象类型比较失败的常见原因，现代代码应改用 `__attribute__((visibility("protected")))` 实现类似效果。

#### LD_PRELOAD 优先级

`LD_PRELOAD`（环境变量）指定的库在 global scope 中插到所有其他库**之前**（仅次于主可执行文件之后），从而可以抢占任意符号：
...
**安全限制**：当进程设置了 SUID / SGID 位、或 `AT_SECURE=1`（通过 capabilities 等手段），glibc ld.so **忽略** `LD_PRELOAD`、`LD_LIBRARY_PATH` 等影响搜索路径的环境变量，防止权限提升。

```bash
# 示例：用 LD_PRELOAD 抢占 malloc（内存调试）
LD_PRELOAD=/usr/lib/libmimalloc.so.2 ./a.out

# 安全模式下 LD_PRELOAD 被忽略（AT_SECURE=1）
LD_SHOW_AUXV=1 sudo ./a.out | grep AT_SECURE
```

#### RTLD_DEEPBIND

`dlopen(path, RTLD_LOCAL | RTLD_DEEPBIND)` 中的 `RTLD_DEEPBIND` 标志改变该库内部符号引用的解析优先级：

| 标志 | 解析顺序 |
|---|---|
| 默认（无 `RTLD_DEEPBIND`）| 全局 scope 优先，本库自身其次 |
| 含 `RTLD_DEEPBIND` | 本库自身优先，全局 scope 其次 |

效果类似 `DT_SYMBOLIC`，但仅对该特定 `dlopen` 加载的库有效，不影响其他库。`RTLD_DEEPBIND` 常用于：
- 插件系统需要隔离内部 libc / libstdc++ 版本；
- 避免与宿主程序的全局符号产生命名冲突。

**注意**：`RTLD_DEEPBIND` 与 `LD_PRELOAD` 组合时，`RTLD_DEEPBIND` 会使该库优先用自身定义而忽略 `LD_PRELOAD`，是常见的钩子失效原因。

#### STB_GNU_UNIQUE 绑定类型（glibc 2.11+）

`STB_GNU_UNIQUE`（值 10，glibc 2.11 引入）是一种特殊绑定类型，确保进程中同名符号只有**唯一一个实例**——不管该符号来自哪个共享库：

| 属性 | 说明 |
|---|---|
| 值 | 10（与 `STB_LOOS` / `STT_GNU_IFUNC` 高/低4位区分）|
| 引入版本 | glibc 2.11（2009 年）|
| 主要用途 | C++ 模板的静态局部变量、`type_info` 对象在多个 DSO 中保持唯一 |
| 实现机制 | ld.so 维护全局唯一符号哈希表；第一次见到该名字时注册，后续所有引用重定向到第一个注册的地址 |
| 不可抢占 | `STB_GNU_UNIQUE` 符号**不能**被 `LD_PRELOAD` 或其他库覆盖（打破了普通 `STB_GLOBAL` 可抢占的规则）|

```c
// GCC 自动为以下情形生成 STB_GNU_UNIQUE 符号：
// C++ 函数内的静态局部变量（标准要求线程安全初始化）
void foo() {
    static MyClass instance;  // → 可能为 GNU_UNIQUE
}
```

```bash
# 查看 GNU_UNIQUE 符号
readelf --dyn-syms libstdc++.so.6 | grep UNIQUE
nm -D libstdc++.so.6 | grep ' u '   # nm 用小写 u 表示 GNU_UNIQUE
```

`STB_GNU_UNIQUE` 引入的"不可抢占"特性是一把双刃剑：在解决 C++ ODR 问题的同时，破坏了某些依赖 `LD_PRELOAD` 的 mock/interpose 方案，调试时需特别注意。
