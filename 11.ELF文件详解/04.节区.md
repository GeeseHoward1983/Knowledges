...
`SHT_GROUP`（值 17）节是 ELF 对 COMDAT（Common Data）语义的实现，解决 C++ 模板实例化、内联函数在多个翻译单元重复生成时的代码/数据去重问题。

#### 数据结构

```c
/* SHT_GROUP 节的内容：uint32_t 数组 */
Elf32_Word flags;         /* 第0项：组标志，目前仅 GRP_COMDAT = 1 有效 */
Elf32_Word member_idx[];  /* 后续项：属于本组的节的节头表索引 */
```

节头字段的含义（对 SHT_GROUP 节）：

| 字段 | 含义 |
|---|---|
| `sh_link` | 关联符号表（`.symtab`）的节索引 |
| `sh_info` | 组签名符号（group signature symbol）在该符号表中的索引 |
| `sh_entsize` | 始终为 4（每个成员索引占 4 字节）|
| `sh_flags` | 通常为 0；组内成员节须设 `SHF_GROUP`（0x200）|

**组签名符号**：`sh_info` 指向的符号名就是该 COMDAT 组的"名字"（signature）。对于 C++ 实例化，签名通常是模板实例的 mangled name（如 `_ZN3FooIiE3barEv`）。链接器用签名来识别并合并同名组。

#### GRP_COMDAT 标志

当 `flags == GRP_COMDAT`（0x1）时，该组具有 COMDAT 语义：链接时若发现多个目标文件含有签名相同的 COMDAT 组，**保留其中一份，丢弃其余所有**。这与 Microsoft COFF 的 `/COMDAT` section 语义等价。

```bash
# 查看目标文件中的 SHT_GROUP 节
readelf -g foo.o          # 列出所有节组及其成员
readelf -S foo.o | grep GROUP   # 确认节类型

# 示例输出（C++ 模板实例化，代表性，非本机实跑）
# COMDAT group section [   3] `.group' [_ZN3FooIiEC1Ev] contains 2 sections:
#    [Index]    Name
#    [    4]   .text._ZN3FooIiEC1Ev
#    [    5]   .rela.text._ZN3FooIiEC1Ev
```

#### C++ 模板与内联函数的去重机制

编译器为每个模板实例或 `inline` 函数生成一个 COMDAT 组，组内包含：

- 函数的 `.text.<mangled_name>` 节（实际代码）
- 对应的 `.rela.text.<mangled_name>` 节（重定位条目）
- 可能包含的 `.rodata.<mangled_name>`（只读数据，如 typeinfo）

同一个模板实例可能出现在多个翻译单元（.o 文件）中：

```
foo.o: Foo<int>::bar()   ← COMDAT 组，签名 _ZN3FooIiE3barEv
bar.o: Foo<int>::bar()   ← 同签名 COMDAT 组（重复）
baz.o: Foo<int>::bar()   ← 同签名 COMDAT 组（重复）
```

链接器将三份保留一份，其余两份连同组内全部成员节一起丢弃。

#### 链接器对 COMDAT 的消重流程

```
链接器收集所有目标文件的 SHT_GROUP 节
  │
  ▼
按签名（sh_info 指向的符号名）建立哈希表
  │
  ▼
对每个签名：首次出现的组 → 标记"保留"；后续同签名 → 标记"丢弃"
  │
  ▼
丢弃组的所有成员节（包括代码节、重定位节、数据节）不纳入输出
  │
  ▼
其他节对丢弃成员节的引用 → 重定向到保留的那份
```

#### 与 `--gc-sections` / ICF 的交互

| 机制 | 工作层次 | 与 COMDAT 的关系 |
|---|---|---|
| **COMDAT 去重** | 链接开始前，按签名去重 | 先于 `--gc-sections` 执行，决定哪些节参与后续分析 |
| **`--gc-sections`**（Dead code elimination）| 以节为单位，删除无引用节 | 作用于 COMDAT 去重后的节集合；被保留的 COMDAT 节若无引用仍可被 GC 删除 |
| **ICF**（Identical Code Folding，LLD `--icf=all`）| 对比节内容，合并内容完全相同的节 | 作用于 GC 后；COMDAT 节若内容与其他节完全相同也可被 ICF 合并 |

三者执行顺序：`COMDAT 去重 → --gc-sections → ICF`，逐步精简输出体积。

```bash
# 编译时生成 COMDAT 节（-ffunction-sections 让每个函数单独成节）
g++ -c -ffunction-sections foo.cpp -o foo.o

# GNU ld：启用 GC
ld --gc-sections -o a.out foo.o bar.o

# LLD：启用 GC + ICF
ld.lld --gc-sections --icf=all -o a.out foo.o bar.o
```
