...
#### _Unwind_Backtrace 自定义栈回溯

`_Unwind_Backtrace` 是 libgcc/libunwind 提供的底层栈帧枚举接口，通过逐帧调用回调函数实现调用栈遍历，内部完全依赖 `.eh_frame`（及 `.eh_frame_hdr` 加速索引）进行帧展开：
...
trace_callback(struct _Unwind_Context *ctx, void *arg) {
    int *count = (int *)arg;
    /* 获取当前帧的 PC（返回地址 - 1，指向调用指令）*/
    uintptr_t pc = _Unwind_GetIP(ctx);
    if (pc == 0)
        return _URC_END_OF_STACK;

    /* 用 dladdr 将地址转为符号名（依赖 .dynsym / .symtab）*/
    Dl_info info;
    if (dladdr((void *)pc, &info) && info.dli_sname) {
        printf("#%-2d 0x%016lx  %s+%ld  (%s)\n",
               *count, pc,
               info.dli_sname,
...
void print_backtrace(void) {
    int count = 0;
    _Unwind_Backtrace(trace_callback, &count);
}

/* 编译：gcc -g -rdynamic backtrace.c -o bt -ldl
   -rdynamic 使符号表导出给 dladdr 使用 */
```

`glibc` 的 `backtrace(3)` 函数内部即调用 `_Unwind_Backtrace`，因此凡是能运行 `backtrace()` 的环境都需要有效的 `.eh_frame`。

#### dl_iterate_phdr 辅助遍历加载对象

当需要对所有已加载共享库的地址范围做统计（如自建符号查找、地址过滤）时，结合 `dl_iterate_phdr` 枚举 `PT_PHDR` 信息：

```c
#include <link.h>
#include <stdio.h>

/* 回调：对每个已加载的 DSO（含主程序）调用一次 */
static int
phdr_callback(struct dl_phdr_info *info, size_t size, void *data) {
    printf("对象: %s  加载基址: 0x%lx\n",
           info->dlpi_name[0] ? info->dlpi_name : "(main)",
           info->dlpi_addr);

    /* 遍历程序头，找 PT_GNU_EH_FRAME 段（.eh_frame_hdr 位置）*/
    for (int i = 0; i < info->dlpi_phnum; i++) {
        const ElfW(Phdr) *phdr = &info->dlpi_phdr[i];
        if (phdr->p_type == PT_GNU_EH_FRAME) {
            printf("  .eh_frame_hdr 在 VA 0x%lx\n",
                   info->dlpi_addr + phdr->p_vaddr);
        }
    }
    return 0;  /* 返回非 0 时终止迭代 */
}
...
/* 编译：gcc -g iterate.c -o iter -ldl */
```

这是 libunwind（LLVM 实现）查找各 DSO `.eh_frame_hdr` 的实际机制：`dl_iterate_phdr` 枚举所有 DSO，找到每个 DSO 的 `PT_GNU_EH_FRAME` 段，建立地址区间 → `.eh_frame_hdr` 的映射，再通过二分查找找到目标 PC 对应的 FDE。

#### -fno-exceptions 对 .eh_frame 的影响

```bash
# C++ 编译：默认开启异常
g++ -g foo.cpp -o foo
# .eh_frame 存在，augmentation 含 "zPLR"（personality + LSDA + pcrel）
# .gcc_except_table 存在

# 禁用异常
g++ -g -fno-exceptions foo.cpp -o foo
# .eh_frame 依然存在（backtrace/信号处理仍需要 CFI）
# 但 CIE augmentation 降为 "zR"（无 personality P，无 LSDA L）
# .gcc_except_table 不生成（无 try/catch 表达数据）

# 完全禁止 CFI 表（极少用，会破坏 backtrace 和信号处理）
gcc -fno-asynchronous-unwind-tables -fno-exceptions foo.c -o foo
# .eh_frame 不生成（但 backtrace() 将依赖帧指针链，需 -fno-omit-frame-pointer）
```

**`-fno-exceptions` 的精确效果**：

| 特性 | `-fno-exceptions` 前 | `-fno-exceptions` 后 |
|------|---------------------|---------------------|
| `.eh_frame` 存在 | 是（augmentation "zPLR"）| 是（augmentation "zR"）|
| CIE personality 指针 | 存在（指向 `__gxx_personality_v0`）| 不存在 |
| FDE LSDA 指针 | 存在（指向 `.gcc_except_table`） | 不存在 |
| `.gcc_except_table` | 生成 | 不生成 |
| `throw` 语句 | 合法 | 编译错误（`-fno-exceptions` 下 throw 非法）|
| `try/catch` | 合法 | catch 降级为 `if(false)`（实现相关）|
| `noexcept` 检查 | 运行时 terminate | 同上（static 信息保留）|

#### AArch64 Compact Unwind 对比

Apple 平台（iOS/macOS）的 AArch64 ABI 使用**Compact Unwind** 格式替代 DWARF CFI，与 Linux 的 `.eh_frame` 差异显著：

| 对比项 | Linux AArch64 `.eh_frame` | Apple Compact Unwind |
|--------|--------------------------|---------------------|
| 存储节 | `.eh_frame` + `.eh_frame_hdr` | `__unwind_info` + `__eh_frame`（退回） |
| 编码粒度 | 每条 CFI 指令精确到每条汇编指令 | 每个函数一个 32 位压缩描述符（4字节） |
| 典型大小 | 函数平均 ~64 字节 FDE | 4 字节（简单函数） |
| 支持的场景 | 任意复杂 prologue/epilogue | 只支持标准 AArch64 ABI 的规范 prologue |
| 降级机制 | 无（`-fasynchronous-unwind-tables` 必须全覆盖）| 复杂函数降级到 `__eh_frame` 中的 DWARF FDE |
| 查找结构 | 二分查找 `.eh_frame_hdr` 中的 FDE PC 表 | 多级索引（Page table → second level）|

AArch64 Compact Unwind 的 32 位描述符编码示例：

```
位 [31:24]  = 编码模式（0x04 = 仅保存 lr/fp 到栈，0x03 = 无帧）
位 [23:16]  = 保存的寄存器列表（x19-x28 的位掩码）
位 [15: 0]  = 栈帧大小 × 16（最大 65520 字节）
```

Linux AArch64 不采用 Compact Unwind，坚持 DWARF `.eh_frame` 路线，因为 Linux 内核与 GCC 工具链的标准化以 DWARF 为核心，且 AArch64 的 prologue 有时需要 DWARF 表达式才能精确描述（如动态栈分配 `alloca`）。

```bash
# Linux AArch64：查看 .eh_frame
aarch64-linux-gnu-readelf --debug-dump=frames program

# Apple AArch64 macOS：查看 compact unwind
xcrun dyldinfo -unwind /usr/bin/ls
otool -lv /usr/bin/ls | grep -A20 "__unwind_info"
```
