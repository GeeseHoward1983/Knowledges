...
eBPF 程序以标准 ELF 可重定位目标文件（`ET_REL`，`e_machine = EM_BPF`）的形式存在，其特殊性集中在三个维度：一是**节名即语义**——`SEC("xdp")`、`SEC("kprobe/…")` 等节名直接决定程序类型，`libbpf` 通过前缀匹配表将节名映射到 `BPF_PROG_TYPE_*`；二是**BTF 驱动类型安全**——`.BTF` 节提供内核感知的类型信息，`.BTF.ext` 中的 `core_relos` 使 CO-RE 跨内核版本重定位成为可能；三是**运行时 fd 重定位**——BPF ELF 的 `R_BPF_64_64` 重定位将 map 文件描述符写入 `ld_imm64` 指令，这是区别于普通 ELF 的根本差异。掌握这套机制，是进行 eBPF 开发调试、逆向分析和安全审计的基础。
