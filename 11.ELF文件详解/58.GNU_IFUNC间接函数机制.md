...
GNU IFUNC 通过 `STT_GNU_IFUNC` 符号类型和 `R_X86_64_IRELATIVE` 重定位，在程序加载时一次性调用 resolver 函数选出最优实现并写入 GOT，后续调用等价于直接调用，无额外开销。动态链接器必须在所有普通符号绑定完成后才处理 IRELATIVE，以保证 resolver 可以安全调用已解析的辅助函数。glibc 大量使用 IFUNC 实现 `memcpy`/`strlen` 等函数的 CPU 特性分发。逆向分析时，静态工具看到的 IFUNC 符号 `st_value` 指向 resolver 而非最终函数，需通过 `LD_DEBUG=bindings` 或在 `main` 断点后读取 GOT 来获取实际跳转目标。

## 参考: `STT_GNU_IFUNC` (GNU 扩展，非 System V 标准)
