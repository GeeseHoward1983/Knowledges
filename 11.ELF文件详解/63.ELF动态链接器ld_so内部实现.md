...
`ld.so` 是动态链接的核心引擎，其实现精妙之处在于三个层次：**自举**（`_dl_start` 在无任何外部支持的条件下完成自身重定位）、**BFS 依赖加载**（以 `DT_NEEDED` 驱动广度优先遍历，生成决定符号查找顺序的 `link_map` 链表）、**延迟绑定**（PLT stub 首次调用时触发 `_dl_runtime_resolve`，通过 `.gnu.hash` O(1) 查找真实函数地址并回填 GOT）。`LD_PRELOAD` 的 hook 能力、`RTLD_NEXT` 链式调用，以及 `LD_DEBUG=all` 的全程透明化，都建立在对这套机制的深入理解之上。安全方面，GOT 覆写、RPATH 注入、`/etc/ld.so.preload` 持久化是主要攻击面，Full RELRO（`-z,relro,-z,now`）是最有效的防御手段。
