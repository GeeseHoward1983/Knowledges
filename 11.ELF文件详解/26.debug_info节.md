...
DWARF 2/3 用 `DW_AT_low_pc` + `DW_AT_high_pc` 描述函数的连续地址区间，但现代编译优化会产生**地址不连续**的函数代码片段：

| 优化场景 | 碎片化原因 |
|----------|-----------|
| **LTO（链接时优化）** | 链接器在全局视角重排代码，将热路径/冷路径物理分离（`-ffunction-sections` + 链接器排序），同一函数的 hot/cold 分区可能相差数 MB |
| **PGO（基于性能的优化）** | 借助性能数据（`perf.data`/`gcda`）将冷代码（异常处理路径、rarely-called 分支）移至 `.text.cold` 等独立节，使 i-cache 局部性最优 |
| **代码生成优化** | `-fbasic-block-sections`（LLVM/GCC）将每个基本块放入独立节，链接器据 profile 重排 |
| **内联展开** | `DW_TAG_inlined_subroutine` 的各实例散落各处，原始函数本体与所有内联实例地址集合不连续 |

当函数代码段分裂为多个不连续区间时，单对 `low_pc/high_pc` 无法描述，必须使用 `DW_AT_ranges` 指向一个地址范围列表。

#### DW_AT_ranges 的工作机制

`DW_AT_ranges` 属性值（`DW_FORM_sec_offset` 或 DWARF 5 的 `DW_FORM_rnglistx`）指向 `.debug_ranges`（DWARF 3/4）或 `.debug_rnglists`（DWARF 5）节中的一条范围记录，替代 `low_pc/high_pc`：

```
DIE（DW_TAG_subprogram）的典型结构对比：

── 连续函数（DWARF 4）──
DW_AT_low_pc  : 0x401000
DW_AT_high_pc : 0x401080   （或 DW_FORM_data8: 0x80，表示长度）

── 非连续函数（DWARF 4）──
DW_AT_ranges  : 0x3c        （.debug_ranges 节内偏移）

── 非连续函数（DWARF 5）──
DW_AT_ranges  : DW_FORM_rnglistx 0x2  （.debug_rnglists 中的索引）
```

#### .debug_ranges 格式（DWARF 3/4）

`.debug_ranges` 由若干**范围列表**（Range List）组成，每个列表以终止条目结尾：

```
范围列表条目（每条 2 × address_size 字节）：
  [起始地址, 结束地址)  — 半开区间，结束地址 = 最后一条指令之后
  
特殊条目：
  [0, 0]              — 终止符（两个全零地址）
  [0xFFFF...FF, base] — 基址选择条目（base address selection entry）
                        下一个非终止条目的起止地址均加上 base
```

**基址选择条目**是一种压缩优化：当多个范围条目相对于同一基址时，存储差值而非绝对地址，减少重定位数量（但 DWARF 4 仍需重定位基址）。

```bash
# DWARF 4 示例：函数 foo 分裂为两个片段
# .debug_ranges 偏移 0x3c 处：
#   0x401000 - 0x401050   片段 1（hot path）
#   0x402000 - 0x402020   片段 2（cold path，异常处理）
#   0x000000 - 0x000000   终止符
```

#### .debug_rnglists 格式（DWARF 5）

DWARF 5 替换为更紧凑的 `.debug_rnglists`，使用变长编码（`DW_RLE_*`）：

| 条目类型 | 说明 | 与 DWARF4 的改进 |
|----------|------|-----------------|
| `DW_RLE_offset_pair` | 两个 ULEB128 相对于基址的偏移 | 省去重定位，小偏移 1~2 字节即可 |
| `DW_RLE_startx_endx` | 两个 `.debug_addr` 索引 | Split DWARF 中 .dwo 无需重定位 |
| `DW_RLE_base_addressx` | `.debug_addr` 索引设为基址 | 配合 offset_pair 极度压缩 |
| `DW_RLE_start_length` | 绝对起始 + ULEB128 长度 | 长度通常比地址小，节省字节 |
| `DW_RLE_end_of_list` | 终止符（1 字节） | DWARF4 终止符需要 16 字节 |

#### readelf 实战：查看范围信息

```bash
# 查看 .debug_ranges（DWARF 4）
readelf --debug-dump=Ranges program

# 查看 .debug_rnglists（DWARF 5）
readelf --debug-dump=rnglists program

# 综合：同时 dump info 和 ranges，找到函数的非连续片段
readelf --debug-dump=info program | grep -A5 "DW_AT_ranges"
readelf --debug-dump=Ranges program

# llvm-dwarfdump（更友好）
llvm-dwarfdump --debug-rnglists program
llvm-dwarfdump --debug-ranges program
```
...
Contents of the .debug_ranges section:
    ...
    Offset   Begin    End
    00000000 0000000000401000 0000000000401050
    00000000 0000000000402000 0000000000402020
    00000000 <End of list>
    00000030 0000000000401080 00000000004010c0
    00000030 <End of list>
```

```text
# 结合 debug_info 查看哪个函数使用了哪条 range list：
$ readelf --debug-dump=info a.out | grep -B2 -A1 "DW_AT_ranges"
 <1><3c>: Abbrev Number: 2 (DW_TAG_subprogram)
    DW_AT_name     : foo
    DW_AT_ranges   : 0x0    ← 指向偏移 0x0 的范围列表（两段非连续代码）
```

#### 调试器与性能分析工具的行为

调试器（GDB/LLDB）解析 `DW_AT_ranges` 后，将函数地址映射视为多个区间的并集：

- **断点设置**：`break foo` 在 `foo` 的所有代码片段首条指令均设断点
- **栈回溯**：`backtrace` 时按 PC 查找所属 FDE 和 DIE，ranges 多段均可命中
- **perf/profiler**：`perf report` 解析符号时，若使用 DWARF 模式（`--call-graph dwarf`），同样需要处理非连续范围
