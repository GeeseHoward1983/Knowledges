...
静态库（`.a`）是 POSIX `ar` 格式的存档，内嵌完整的 ELF 可重定位目标文件，并通过 `/` 特殊成员存储符号索引加速链接器查找。链接器的核心算法是**单趟从左到右按需提取**：只有当库成员定义了当前未定义符号集合中的符号，该成员才被提取——这直接导致库顺序敏感性，是"undefined reference"错误的最常见根源。`--whole-archive` 强制全量提取（代价是可能引入多重定义），`--start-group`/`--end-group` 则通过多趟固定点迭代解决库间循环依赖。Thin archive 以路径代替内容存档，适合构建系统内部的快速增量构建，但不能脱离源 `.o` 独立分发。理解这一机制，是排查链接错误、合理组织库依赖和保障供应链安全的关键。
