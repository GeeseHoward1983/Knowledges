...
## 10.8.4 CPM.cmake：基于 FetchContent 的社区封装

`CPM.cmake`（C++ Package Manager）是基于 `FetchContent` 构建的**单文件社区封装层**，旨在以更简洁的 API 完成"声明→获取→可用"的全流程，同时增加了**版本缓存**、**去重**、**优先系统版本**等原生 `FetchContent` 不提供的便利特性。

### 工作原理

CPM 本质是一段 CMake 脚本，下载后用 `include()` 引入即可——无需安装、无需全局配置。它对每个依赖调用 `FetchContent_Declare`/`FetchContent_MakeAvailable`，但额外做了：

- **本地磁盘缓存**：通过环境变量 `CPM_SOURCE_CACHE` 指定缓存根目录，相同的库+版本组合在全机器范围内只下载一次，多个项目共享同一份源码，再配置时直接读磁盘无需联网。
- **`find_package` 优先**：设置 `CPM_USE_LOCAL_PACKAGES=ON` 时，`CPMAddPackage` 会先尝试 `find_package`，找到系统版本即直接使用，找不到才下载——与 `FIND_PACKAGE_ARGS`（§10.8.3）的语义吻合。
- **去重**：多个子项目都声明同一个依赖时，CPM 自动合并，只构建一次。

### 快速上手

```cmake
# 1) 下载 CPM.cmake（通常放进 cmake/ 目录并提交进版本控制）
#    或在 CMakeLists.txt 里用 file(DOWNLOAD ...) 按需拉取最新版
cmake_minimum_required(VERSION 3.24)
project(demo CXX)

include(cmake/CPM.cmake)          # include 这个单文件即完成"安装"

# 2) CPMAddPackage：声明并立即可用
CPMAddPackage(
  NAME           fmt
  GITHUB_REPOSITORY fmtlib/fmt
  GIT_TAG        11.0.2            # 固定版本，可复现
  OPTIONS        "FMT_INSTALL OFF" # 覆盖子项目的 CMake 选项（无需手写 set_before_avail）
)
...
| 参数 | 作用 |
| --- | --- |
| `NAME` | 包名（对应 `FetchContent_Declare` 的 `<name>`） |
| `GITHUB_REPOSITORY` / `GIT_REPOSITORY` | 来源仓库（前者是 `github.com/<org>/<repo>` 的简写） |
| `GIT_TAG` | 版本标签或提交 SHA，**强烈建议写具体值** |
| `VERSION` | 语义版本号，配合 `CPM_USE_LOCAL_PACKAGES` 触发 `find_package` 版本检查 |
| `OPTIONS` | 以空格分隔的 `key value` 对，通过 `set(key value CACHE ...)` 覆盖子项目选项 |
| `PATCHES` | 要应用的 patch 文件列表（基于 `git apply`） |
| `EXCLUDE_FROM_ALL` | 同 `FetchContent_Declare` 的 `EXCLUDE_FROM_ALL`（3.28+），目标不进 `all` |
| `DOWNLOAD_ONLY` | 只下载源码，不 `add_subdirectory`，留给手动处理 |

### 适用场景与取舍

CPM.cmake 在如下场景有明显优势：

- **团队项目**：统一缓存目录后，`clone` 新机器第一次构建会从缓存命中，大幅缩短 CI 冷启动。
- **频繁换依赖版本**：`OPTIONS` 能在声明处覆盖子项目选项，免去"先 `set(key val)` 后 `MakeAvailable`"的顺序问题。
- **轻量嵌入**：只需把 `CPM.cmake` 单文件提交进项目，零外部依赖。

与原生 `FetchContent` 相比，CPM 是**社区库而非 CMake 内置**，引入了对该文件的依赖；对于需要与 vcpkg/Conan 的**依赖提供者**（§10.11）协作的大型项目，原生 `FIND_PACKAGE_ARGS` 路径更透明。两者选其一，按团队偏好决定。

---

## 10.8.5 `FetchContent_GetProperties` 与手动 `FetchContent_Populate`

通常 `FetchContent_MakeAvailable` 是推荐的一步到位方式，但有时你需要在"源码就位"之后、`add_subdirectory` 执行**之前**插入自定义操作——例如修改子项目的 CMake 缓存选项、对源码打补丁、或有条件地跳过某个子项目的编译。这时就需要手动拆分 Populate 过程。

### 旧方式（3.30 前）：`FetchContent_Populate` 手动调用

```cmake
include(FetchContent)

FetchContent_Declare(some_lib
  GIT_REPOSITORY https://github.com/example/some_lib.git
  GIT_TAG        v2.1.0
)

# 先获取属性，判断是否已就位（避免重复 populate）
FetchContent_GetProperties(some_lib)
# some_lib_POPULATED：是否已填充（布尔）
# some_lib_SOURCE_DIR：源码目录
# some_lib_BINARY_DIR：二进制目录
if(NOT some_lib_POPULATED)
  # 下载/克隆源码，但不调用 add_subdirectory
  FetchContent_Populate(some_lib)

  # === 在这里做 add_subdirectory 前的定制 ===
  # 例：关掉子项目的示例和测试
  set(SOME_LIB_BUILD_EXAMPLES OFF CACHE BOOL "" FORCE)
  set(SOME_LIB_BUILD_TESTS    OFF CACHE BOOL "" FORCE)
  # 例：打补丁（若源码有需要修改的地方）
  # execute_process(COMMAND git apply ${CMAKE_CURRENT_SOURCE_DIR}/patches/some_lib.patch
  #                 WORKING_DIRECTORY ${some_lib_SOURCE_DIR})

  # 手动 add_subdirectory（SOURCE_DIR / BINARY_DIR 由 GetProperties 拿到）
  add_subdirectory(${some_lib_SOURCE_DIR} ${some_lib_BINARY_DIR} EXCLUDE_FROM_ALL)
endif()
```

> **CMake 3.30 起：`FetchContent_Populate(<name>)` 的"手动"形式已被弃用（deprecated）。**  
> 调用时会打印 `CMake Deprecation Warning`，在 CMake 4.x 中该警告会更明显。

### 新方式（3.28+）：`FetchContent_MakeAvailable` + `SOURCE_SUBDIR` / `EXCLUDE_FROM_ALL`

CMake 3.28 在 `FetchContent_Declare` 中新增了 `SOURCE_SUBDIR` 和 `EXCLUDE_FROM_ALL` 选项，覆盖了手动 Populate 最常见的两类需求，同时保持 `MakeAvailable` 的简洁调用。
...
FetchContent_Declare(some_lib
  GIT_REPOSITORY https://github.com/example/some_lib.git
  GIT_TAG        v2.1.0
  EXCLUDE_FROM_ALL                    # 3.28+：其目标不进 all，等价于 add_subdirectory(... EXCLUDE_FROM_ALL)
  SOURCE_SUBDIR  lib                  # 3.28+：只 add_subdirectory 源码树下的 lib/ 子目录
                                      #  适合"仓库根目录不是 CMake 项目、真正的 CMakeLists.txt 在子目录"时
)

# 在 MakeAvailable 前仍可设定选项（这会影响子项目配置）
set(SOME_LIB_BUILD_EXAMPLES OFF CACHE BOOL "" FORCE)

FetchContent_MakeAvailable(some_lib)  # 下载 + add_subdirectory（指向 lib/），不再需要手动 Populate
```

### 仍需手动 Populate 的场景

即使在 3.30+ 中，`FetchContent_Populate` 的**新签名**（传入完整下载选项而非包名）并未被弃用，适合"不想声明 + MakeAvailable，只想把某个 URL/git 仓库的内容下载到指定路径"的纯下载场景：

```cmake
# 新签名（非弃用）：直接传下载选项，不与 Declare/MakeAvailable 联动
FetchContent_Populate(
  QUIET
  SUBBUILD_DIR ${CMAKE_CURRENT_BINARY_DIR}/3rdparty/some_lib-subbuild
  SOURCE_DIR   ${CMAKE_CURRENT_BINARY_DIR}/3rdparty/some_lib-src
  BINARY_DIR   ${CMAKE_CURRENT_BINARY_DIR}/3rdparty/some_lib-build
  GIT_REPOSITORY https://github.com/example/some_lib.git
  GIT_TAG        v2.1.0
)
# 不自动 add_subdirectory，适合"只要源码、自己决定怎么用"
```
...
| CMake 版本 | 推荐方式 |
| --- | --- |
| **< 3.28** | `Declare` + `GetProperties` + `FetchContent_Populate(<name>)` + 手动 `add_subdirectory` |
| **3.28 ~ 3.29** | 优先 `Declare(... SOURCE_SUBDIR / EXCLUDE_FROM_ALL)` + `MakeAvailable`；需打补丁时仍可用旧方式（会有 deprecation 警告） |
| **3.30+** | **`MakeAvailable` + `SOURCE_SUBDIR`/`EXCLUDE_FROM_ALL`** 是首选；选项覆盖用 `set(... CACHE ... FORCE)` 在 `MakeAvailable` 前设置；打补丁用 `PATCH_COMMAND`（`ExternalProject` 的选项，也可在 `Declare` 里透传） |
