...
> 作者原创系列，共 **13** 篇。系统拆解调试器的底层机制（`ptrace`/断点/单步）、调试信息（DWARF/PDB/栈展开）、主流工具（GDB/LLDB/WinDbg/x64dbg），并落到反调试对抗与**手写一个 mini 调试器**。本系列**反向激活** [[11.ELF文件详解/24.debug节|ELF 的 .debug_* (DWARF) 一族]]，并与 [[14.动态链接与加载器详解/00.动态链接与加载器总览|动态链接与加载器]]、[[32.hooks/00.总览|Hook 技术]] 互补。
