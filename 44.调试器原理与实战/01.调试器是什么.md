...
传统"实时调试"只能**向前**推进执行：`next`、`step`、`continue`，每个操作都把程序状态往后推，一旦过了某个关键时刻就必须重新运行、重新复现。这一局限在以下场景尤为痛苦：

- **偶发性 bug / Heisenbug**：调试器介入改变了时序，bug 不再复现。
- **竞态条件**：需要精确捕捉某一时刻的多线程交错，单步会破坏这个时序。
- **崩溃根因倒推**：程序在 `T+1000ms` 崩溃，但真正的"坏数据写入"发生在 `T+0ms`，实时调试无法回头。

时间旅行调试的**录制-回放（Record and Replay）范式**解决了这个问题：

```
录制阶段（Record）：          回放阶段（Replay）：
程序正常运行                   从 trace 加载历史状态
  + 所有非确定性事件被捕获       随意跳转到任意时刻
    · 内存写入序列               向前单步  →  next / step
    · 系统调用及返回值           向后单步  ←  reverse-next / reverse-step
    · 信号/中断到达时序           设断点，反向执行到上一次命中
    · I/O 读取结果                  g-（WinDbg）/ rr replay 的 reverse-continue
```

**`rr` 的关键命令**（Linux，详见 [[16.rr时间旅行调试]]）：
...
# GDB 反向命令
(gdb) reverse-next          # 反向单步越过（上一条语句）
(gdb) reverse-step          # 反向单步进入
(gdb) reverse-continue      # 反向执行到上一个断点
(gdb) reverse-finish        # 反向执行到上一个函数入口
```

**WinDbg TTD 的关键命令**（Windows，详见 [[10.WinDbg与x64dbg]] 的 TTD 节及 [[16.rr时间旅行调试]]）：
...
p-             ; 反向单步
g-             ; 反向执行到上一个断点
dx @$cursession.TTD.Calls("ntdll!NtWriteFile")  ; 查询所有 WriteFile 调用历史
```

**录制开销**：`rr` 依赖 Linux `perf_event` 的硬件 CPU 周期计数器，录制开销约 1.5–2x；WinDbg TTD 采用动态二进制插桩，开销通常 5–20x。两者都会产生较大的 trace 文件（长时间录制可达数 GB）。

> [!tip] 深入阅读
> `rr` 的确定性回放原理、Linux perf_event 硬件计数器机制、WinDbg TTD 插桩实现，以及与 UndoDB/QEMU replay 的对照，详见 [[16.rr时间旅行调试]]。
