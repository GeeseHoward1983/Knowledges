...
TTD 的核心优势不仅是"能向后走"，更在于通过 **`dx` 数据模型**将整条 trace 作为可查询的数据库：执行了哪些函数、哪些内存地址被写入过、何时写入、写入了什么值——这些问题都可以用类 LINQ 语法一条命令回答，无需手动反向单步。

#### 时间坐标系

TTD 用**时间戳（Time Position）**标识 trace 中的每个时刻，格式为 `<sequence>:<step>`：

- `sequence`：粗粒度的"时钟周期号"，每次内存写入/系统调用推进一次。
- `step`：同一 sequence 内的细粒度步数。

```text
; 查看当前时刻
!tt                             ; 显示当前时间位置（如 24F:0）
!positions                      ; 所有线程的当前时间位置

; 导航
!tt 0                           ; 跳到 trace 起点
!tt 100                         ; 跳到 trace 终点（100% 处）
!tt 24F:0                       ; 跳到指定时间戳
!tt 50.0                        ; 跳到 50% 位置（百分比语法）

; 反向执行
p-                              ; 反向单步（step backward，等价 reverse-next）
g-                              ; 反向 continue（到上一个断点）
t-                              ; 反向 trace into（等价 reverse-step）
```

#### dx 数据模型查询

`dx @$cursession.TTD` 命名空间暴露整条 trace 的结构化数据，可用 LINQ 风格操作：

```text
; 查询所有 CreateFileW 调用（函数名 + 调用时刻 + 参数）
dx @$cursession.TTD.Calls("kernelbase!CreateFileW")

; 只看第一个参数（文件路径）
dx @$cursession.TTD.Calls("kernelbase!CreateFileW").Select(c => c.Parameters[0])

; 查询对某内存范围的所有写操作历史（addr 范围 + 写入时刻）
...
; 查询某线程的调用树
dx @$cursession.TTD.Calls("ntdll!*").Where(c => c.ThreadId == 0x1234)

; 找到特定返回值的调用（如失败的 VirtualAlloc）
dx @$cursession.TTD.Calls("kernelbase!VirtualAlloc").Where(c => c.ReturnValue == 0)
```

> [!warning] 示例输出为示意
> `dx` 查询需要符号（`.sympath` 已配置）；`Calls()` 的函数名支持通配符（`ntdll!*`）。首次查询会对 trace 建索引，可能需要几秒到几分钟。
...
| 文件 | 内容 |
|---|---|
| `myapp.run` | 主 trace 文件：指令序列 + 内存写入 + 系统调用日志（二进制压缩格式） |
| `myapp.idx` | 索引文件：`dx` 查询的加速索引，可由 `.run` 重建（`!index` 命令） |
...
#### WinDbg TTD 与 Linux rr 对照

两者都解决"传统调试只能向前"的问题，但录制机制截然不同：

| 维度 | WinDbg TTD（Windows） | rr（Linux） |
|---|---|---|
| 录制机制 | 动态二进制插桩（`TTDRecorder.dll` 注入） | `perf_event` 硬件计数器 + 确定性重放 |
| 非确定性捕获 | 插桩记录每条指令的副作用（寄存器/内存变化） | 只记录非确定性事件（信号、I/O）；指令本身确定性重执行 |
| 录制开销 | 5–20x（每条指令均插桩） | 1.5–2x（仅捕获非确定性边界） |
| trace 大小 | 较大（含全量指令副作用）| 较小（仅非确定性事件 + 计数器快照）|
| 多线程 | 支持，线程调度序列也被录制 | 支持，通过硬件计数器确保线程调度可重现 |
| 查询能力 | `dx TTD.Calls/Memory`（强，类 SQL） | GDB `reverse-*` 命令（基础）；`rr sources` 辅助分析 |
| 集成 | WinDbg Preview 原生 | GDB（`rr replay` 启动后 GDB 接管）|
| 内核调试 | 不支持（仅用户态进程） | 不支持（仅用户态进程）|
| 开源 | 否（微软专有） | 是（Mozilla，开源）|

> [!tip] 深入阅读
> `rr` 的确定性回放原理（Linux `perf_event`、硬件计数器如何捕获非确定性事件）及与 WinDbg TTD 的深度对比详见 [[16.rr时间旅行调试]]。
