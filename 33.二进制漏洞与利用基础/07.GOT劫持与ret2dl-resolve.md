...
### Partial vs Full RELRO 的快速判断（checksec 与手动确认）

#### checksec 输出含义速查

`checksec` 工具通过解析 ELF 动态节（`PT_DYNAMIC`）中的 `DT_FLAGS`/`DT_FLAGS_1` 字段以及节权限位来判断 RELRO 级别：
...
| checksec 输出 | 含义 | `.got.plt` 可写？ | lazy binding 开启？ |
|---|---|---|---|
| `No RELRO` | 未传 `-z relro`；连 `.got` 也可写 | 是 | 是 |
| `Partial RELRO` | 传了 `-z relro`，但**未**传 `-z now`；`.got` 只读，`.got.plt` 可写 | **是** | 是 |
| `Full RELRO` | 传了 `-z relro -z now`；启动时解析全部符号，随后 `.got.plt` 被 `mprotect` 改为只读 | **否** | **否** |

> [!warning] 最常见误判：看到 `Partial RELRO` 认为 GOT 受保护
> `Partial RELRO` 下函数 GOT（`.got.plt`，`JUMP_SLOT` 类型重定位）**仍然可写**，仍是 GOT 改写的有效攻击面。只有 `Full RELRO` 才真正保护函数 GOT。
...
# 1. 是否传了 -z relro（设置了 GNU_RELRO 段）
readelf -l target | grep GNU_RELRO
# 有输出 → 至少是 Partial RELRO；无输出 → No RELRO

# 2. 是否传了 -z now（设置了 BIND_NOW 标志位）
readelf -d target | grep -E 'BIND_NOW|FLAGS'
# 示意输出：
# 0x000000000000001e (FLAGS)       BIND_NOW   ← BIND_NOW 位被置位 = Full RELRO
# 0x000000006ffffffb (FLAGS_1)     Flags: NOW

# 3. 运行时确认 .got.plt 是否只读（Full RELRO 的最终验证）
# 在 GDB 中，main 断点命中后（加载完成）：
# (gdb) info proc mappings
# 找 .got.plt 所在虚拟地址范围，权限应为 r-- 而非 rw-
```

#### BIND_NOW 语义：启动时解析 vs 延迟解析

`BIND_NOW`（对应链接参数 `-z now`，ELF 字段 `DT_FLAGS` 中的 `DF_BIND_NOW` 位）告诉加载器：

> "在 `dlopen`/进程启动时立即解析此 ELF 中**所有** `JUMP_SLOT` 重定位，不要延迟到首次调用。"

其直接效果：

1. `_dl_runtime_resolve` 在进程启动阶段被调用（对每个函数各一次），解析并填写全部 GOT 槽位。
2. 填写完毕后，加载器调用 `mprotect` 将 `GNU_RELRO` 覆盖的内存区域（含 `.got.plt`）设为只读（`PROT_READ`，去除 `PROT_WRITE`）。
3. 之后任何对 `.got.plt` 的写操作都会触发 `SIGSEGV`——GOT 改写攻击在内核级别被阻止。

这一机制的代价是**启动时延**：所有外部函数在 `main` 执行前被一次性解析，对于引用大量外部符号的程序（如 Firefox、Python 解释器）可能增加数十毫秒启动时间。对于普通服务端程序，这个代价通常可以接受。

```bash
# 演示对比：有无 BIND_NOW 的启动行为差异（教学示意）
# 无 BIND_NOW（Partial RELRO）：首次 puts 调用时才触发 _dl_runtime_resolve
strace -e trace=mmap,mprotect,write ./target_partial 2>&1 | head -20

# 有 BIND_NOW（Full RELRO）：main 前就完成全部解析；.got.plt 区域有额外 mprotect 调用
strace -e trace=mmap,mprotect,write ./target_full 2>&1 | head -30
# 可观察到额外的 mprotect(..., PROT_READ) 调用，对应 GNU_RELRO 段变只读
```

### 现代 glibc（2.35+）对 ret2dl-resolve 的额外校验

随着 ret2dl-resolve 在 CTF 和真实利用中的广泛出现，glibc 2.29 起逐步在动态链接器内部加入了若干针对性的合法性检查，到 glibc 2.35 时这些检查已相当完备。这些校验使**即使在 Partial RELRO 下**伪造链的构造也更加困难，但如前所述，根本缓解仍是 Full RELRO。

#### `_dl_fixup` 中的 reloc_index 边界检查

在早期 glibc（2.28 及以前）中，`_dl_fixup` 拿到 `reloc_index` 后直接做指针算术：

```c
/* 简化版旧逻辑（glibc 2.28 及以前）*/
const Elf64_Rela *reloc = (void *)(D_PTR(l, l_info[DT_JMPREL])
                                    + reloc_index * sizeof(*reloc));
```

没有越界检查——`reloc_index` 可以是任意值，使 `reloc` 指向 `.rela.plt` 之外的任意可读内存（即攻击者在 `.bss` 准备的 `fake_rela`）。

glibc 2.29 开始引入 `DT_PLTRELSZ` 长度限制，2.35 进一步收紧为：

```c
/* 现代 glibc（2.35+）简化示意 */
if (reloc_offset >= D_PTR(l, l_info[DT_PLTRELSZ]))
    _dl_signal_error(0, l->l_name, NULL, "invalid relocation offset");
```

`DT_PLTRELSZ` 是 `.rela.plt` 节的**字节大小**（链接时固定），只有 `0 <= reloc_offset < PLTRELSZ` 的值才被接受。

**对 ret2dl-resolve 的影响**：

- 直接使用"指向 `.bss` 的越界 index"的传统 32 位手法在 glibc 2.35+ 上直接报错终止。
- 64 位 glibc 2.35+ 下，`reloc_index` 必须是合法范围内的值（指向真实 `.rela.plt` 条目），无法简单地用大值越界到 `.bss`。
- 现代 ret2dl-resolve PoC 需要通过**先行修改 `.rela.plt` 内容**（需要可写的 `.rela.plt`，即无 RELRO 或 Partial RELRO 下 `.rela.plt` 本身可写）或借助更复杂的 link_map 伪造路径绕过。

#### `_dl_lookup_symbol_x` 中的 link_map 完整性检查

`_dl_fixup` 解析符号名后，调用 `_dl_lookup_symbol_x` 在已加载的所有 DSO 中搜索。现代 glibc 在此处增加了对 `link_map` 结构体的合法性验证：

```c
/* glibc 2.35+ 简化示意：_dl_fixup 内对 l（link_map）的几项校验 */
// 1. l 必须是合法的 link_map 指针（非空、地址对齐）
assert(l != NULL);

// 2. l->l_info[DT_SYMTAB] 和 l->l_info[DT_STRTAB] 必须非空
//    → 若攻击者伪造 link_map 把这两个字段清零，此处触发 abort

// 3. l->l_ns 命名空间索引必须合法（不得越界进入 _dl_ns 数组外）
if ((Lmid_t)l->l_ns < 0 || (size_t)l->l_ns >= GL(dl_nns))
    _dl_signal_error(...);
```

攻击者若通过 ROP 直接调用 `_dl_runtime_resolve` 并传入伪造的 `link_map`，需要使 `l_info`、`l_ns` 等字段全部合法，难度大幅上升。

#### `versym` 版本符号检查

glibc 在符号解析路径中还会检查 `DT_VERSYM`（版本符号表）：若 `.gnu.version` 节存在，`_dl_lookup_symbol_x` 会核对目标符号的版本标签是否与候选 DSO 提供的版本匹配：

```c
/* 简化示意：_dl_lookup_symbol_x 的版本检查 */
if (l->l_info[VERSYMIDX(DT_VERSYM)] != NULL) {
    const ElfW(Half) *verstab = D_PTR(l, l_info[VERSYMIDX(DT_VERSYM)]);
    ElfW(Half) ndx = verstab[sym_index];
    // 若伪造的 fake_sym 对应版本索引不合法，跳过该 DSO
}
```

在拥有合法版本信息的 ELF（现代 Ubuntu 包编译的程序通常含 `DT_VERSYM`）中，伪造的符号若版本字段不匹配，解析器可能找不到目标函数（返回 NULL）或解析到错误符号，导致利用失败。

#### 综合影响与 CTF 策略

| glibc 版本 | 主要新增校验 | 对传统 ret2dl-resolve 的影响 |
|---|---|---|
| ≤ 2.28 | 无边界检查 | 经典手法可用 |
| 2.29–2.31 | `reloc_offset` 边界初步检查 | 需使 index 指向合法范围内条目（需改 `.rela.plt`） |
| 2.32–2.34 | 增加 link_map 字段校验 | 伪造 link_map 需精心构造多个字段 |
| 2.35+ | 全面收紧：边界 + link_map + versym | 传统方式基本失效；仍可通过修改真实 `.rela.plt` 内容（需无/Partial RELRO）或 partial overwrite 实现变体 |

> [!note] 核心结论
> glibc 2.35+ 的这些校验是**深度防御层**（defense in depth），使无 RELRO 或 Partial RELRO 下的 ret2dl-resolve 利用复杂度大幅上升——但它们不是"完全不可绕过"，只是让攻击者需要更多前提条件和更精准的内存布局。唯一可靠的根治仍是 Full RELRO：消除运行期 `_dl_runtime_resolve` 调用路径，使所有这些校验都不再有被绕过的机会。
