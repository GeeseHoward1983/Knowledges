...
本节从原理层面拆解"构造 fake vtable → 让对象 `__vptr` 指向它 → 虚函数调用跳到可控地址"的完整链路，帮助理解为什么 CFI / CET 要从这里入手防御。

#### 3.1 vtable 的内存布局回顾

每个含虚函数的 C++ 类在编译期生成一张只读的 **vtable 数组**，布局如下：

```
[类 vtable（.rodata 段）]
偏移 0x00 → 虚函数 #0 的地址（如 Dog::speak）
偏移 0x08 → 虚函数 #1 的地址（如 Dog::eat）
偏移 0x10 → 虚函数 #2 的地址 …

[Dog 对象（堆/栈）]
偏移 0x00 → __vptr（指向上面的 vtable）← 攻击者的目标
偏移 0x08 → age（int 字段）
偏移 0x0C → tricks（int 字段）
```

调用 `obj->speak()` 时，编译器生成的指令序列等价于：

```asm
mov  rax, [obj]        ; 取 __vptr（对象首地址处 8 字节）
mov  rax, [rax + 0x00] ; 取 vtable[0]（speak 的函数指针）
call rax               ; 间接调用
```

只要能改写 `__vptr` 或 vtable 中的条目，`call rax` 就能跳到任意地址。

#### 3.2 攻击前提：结合堆 UAF 或类型混淆

vtable 伪造通常需要以下至少一种原始能力：

| 前提条件 | 来源 |
|---------|------|
| 堆上**可写区域**能放置伪造数据 | 堆溢出、UAF 后的 chunk 复用（见 [[10.堆漏洞入门]]） |
| 能**改写某对象的 `__vptr`** | 堆溢出越界覆盖相邻对象首 8 字节；或 UAF 后对旧 chunk 写入新内容 |
| 已知**目标跳转地址** | 信息泄露打破 ASLR（见 [[13.现代利用与信息泄露]]）后得到 system/one_gadget 地址 |

在**类型混淆**场景下，不需要直接改写内存——只需让程序把一个已有对象当成另一种类型来解读，若这两种类型在"首字段偏移"上的语义不同，`__vptr` 就已经"错位"指向攻击者可控的数据。

#### 3.3 构造 fake vtable 的四步链

**步骤一：在可写内存中布置 fake vtable 数组**

攻击者通过堆喷射（heap spray）或可控缓冲区，在堆某地址 `FAKE_VTBL` 处写入：

```
FAKE_VTBL + 0x00 → target_addr  ← 如 system("/bin/sh") 的地址
FAKE_VTBL + 0x08 → target_addr  ← 多个虚函数槽都指向同一目标（保险做法）
FAKE_VTBL + 0x10 → target_addr
```

**步骤二：让受害对象的 `__vptr` 指向 `FAKE_VTBL`**

方式 A（堆溢出）：溢出写越过 chunk 边界，直接覆盖相邻对象的首 8 字节（即 `__vptr`）：

```c
// 示意：buf 溢出写到相邻的 obj.__vptr
memcpy(buf, attacker_data, OVERFLOW_LEN);
// attacker_data 末尾 8 字节 = FAKE_VTBL 地址
```

方式 B（UAF + tcache 重用）：`free(obj)` 后，再次 `malloc(sizeof(*obj))` 从 tcache 取到同一地址，此时向其写入 `FAKE_VTBL` 地址即改写了原 `__vptr`，之后程序仍持有旧指针并以为对象还"活着"。

方式 C（类型混淆偏移叠加）：程序把对象 A 当类型 B 解读，B 的 `__vptr` 字段偏移恰好落在 A 的某个整数字段上——攻击者通过整数漏洞控制该字段值，值即为 `FAKE_VTBL` 地址。
...
```
__vptr → FAKE_VTBL → target_addr → call target_addr
```

此时 `target_addr` 即为攻击者选定的任意地址（如 `system`、`one_gadget`、ROP 链起点）。

**步骤四：传参（可选）**

如果目标是 `system("/bin/sh")`，需要在调用前让 `rdi = &"/bin/sh"` 字符串地址。利用"vtable 调用时 `rdi = this` 指针"这一 x86-64 调用约定：在对象起始处或附近布置 `/bin/sh\x00` 字符串，或借助 gadget 调整寄存器——具体取决于场景，但核心链路已完整。

#### 3.4 CTF 视角：识别 vtable 伪造题型的特征

在 CTF 堆题中，以下组合通常预示 vtable 伪造路线：

1. **题目给了 C++ 多态对象**（含虚函数），且提供了任意堆写能力（溢出/UAF）。
2. **`checksec` 显示 Full RELRO**——GOT 已封，转向 vtable 是自然选择。
3. **gdb 断在 `call [rax]` 且 `rax` 指向堆地址**——即 vtable 已被劫持的典型现场。
4. 目标二进制**没有开启 CFI**（`checksec` 无 CFI/CET 字段）——开启后 vtable 伪造会在调用前被检测。

调试辅助命令（Linux gdb-peda/pwndbg 环境）：

```bash
# 查看对象内存，确认 __vptr 当前指向
x/4gx $对象地址

# 查看 vtable 内容（真实 vtable 通常在 .rodata 段）
x/8gx *($对象地址)

# 确认 fake vtable 布置是否到位
x/4gx $FAKE_VTBL_地址
```

### 四、`reinterpret_cast` / C 风格强转在类型混淆中的触发条件

类型混淆要"发生"，首先需要代码中有一处"不安全的类型断言"——`reinterpret_cast` 和 C 风格强转是最常见的两个触发点。本节梳理它们各自在哪些条件下导致可利用的类型混淆。

#### 4.1 `reinterpret_cast` 的语义与危险

`reinterpret_cast<T*>(p)` 的作用是**位模式不变，仅改变编译器对指针的类型解读**——不做任何偏移调整、不调用转换构造函数、不触发 RTTI 校验。其合法使用场景极窄（如 `char*`/`uint8_t*` 与任意类型互转做序列化、硬件寄存器映射），一旦用于不相关的类层次之间，就直接引入类型混淆。

**触发条件一：不相关类型之间的强制转换**

```cpp
class NetworkPacket { public: uint32_t type; uint64_t payload; };
class AuthToken    { public: uint64_t secret; void verify(); };

// 攻击者控制 pkt->type，网络层按 type 字段决定"这是 AuthToken"
void dispatch(NetworkPacket *pkt) {
    if (pkt->type == TYPE_AUTH) {
        // ❌ reinterpret_cast：把网络包内存直接当 AuthToken 解读
        AuthToken *tok = reinterpret_cast<AuthToken *>(pkt);
        tok->verify();  // verify() 读 tok->secret → 实际读的是 pkt->payload
                        // 若 payload 是攻击者填的任意值，secret 即被污染
    }
}
```

关键在于：`pkt->payload`（8 字节）在内存中的位置与 `tok->secret` 完全重合，攻击者通过构造网络包的 `payload` 字段即可污染 `secret`。

**触发条件二：void\* 中转引起的类型信息丢失**

```cpp
// ❌ 常见于 C 风格回调注册：把对象存为 void*，取回时强转
void register_handler(void *ctx, callback_fn fn);

// 注册时存的是 TypeA*
register_handler(new TypeA(), handler_a);

// 取回时误传了 TypeB 的回调
void handler_b(void *ctx) {
    TypeB *b = reinterpret_cast<TypeB *>(ctx); // ← ctx 实际是 TypeA*
    b->do_something();  // 按 TypeB 字段偏移读 TypeA 的内存
}
```

此模式在 C 风格插件系统、libevent/libuv 回调、JNI 的 `jlong` 指针存储中频繁出现。

**触发条件三：序列化/反序列化还原对象**

```cpp
// ❌ 从文件/网络读回字节流，直接 reinterpret_cast 为对象
uint8_t buf[sizeof(Config)];
fread(buf, 1, sizeof(Config), fp);
Config *cfg = reinterpret_cast<Config *>(buf); // ← buf 内容由文件控制
cfg->callback();  // callback 字段是函数指针，文件可伪造任意地址
```

攻击者构造恶意文件，将 `callback` 字段偏移处的 8 字节设为目标函数地址，`cfg->callback()` 即跳转到任意位置。

#### 4.2 C 风格强转 `(T*)p` 的额外陷阱

C 风格强转在 C++ 中按优先级依次尝试 `const_cast → static_cast → static_cast+const_cast → reinterpret_cast → reinterpret_cast+const_cast`，最终一定会成功——这意味着编译器**绝不会拒绝**，所有错误都推迟到运行时。与 `reinterpret_cast` 的主要区别：

| 对比维度 | `(T*)p`（C 风格）| `reinterpret_cast<T*>(p)` |
|---------|----------------|--------------------------|
| 编译期拒绝 | 从不拒绝 | 从不拒绝（均无运行时检查） |
| 代码搜索可见性 | 难以 grep 区分 | 可精确 `grep reinterpret_cast` |
| 能否做 downcast | 能（无 RTTI） | 能（无 RTTI） |
| 是否可被 clang-tidy 检测 | 可（`cppcoreguidelines-pro-type-cstyle-cast`） | 可（单独规则） |

**C 风格强转在不安全向下转型中的经典触发路径**：

```cpp
Base *b = factory();  // 工厂函数可能返回 Base 也可能返回 Derived
Derived *d = (Derived *)b;  // C 风格，无检查
d->derived_only_method();   // 若 b 实际是 Base，读 Derived 独有字段 → 类型混淆
```

#### 4.3 union 误用导致的隐式类型双关

除了显式强转，`union` 误用同样会在没有任何 cast 语句的情况下引发类型混淆：
...
Payload p;
p.as_int = attacker_controlled_int;  // 攻击者填入一个整数
func_expecting_ptr(p.as_ptr);        // 直接把整数当指针用 → 任意地址读/崩溃
```

C++ 标准（不同于 C99）对通过非激活成员读取 `union` 的行为定义为**未定义行为**，编译器可以（也经常会）基于此假设进行激进优化，使得运行时实际读到的值与直觉不符。

**反序列化还原对象的完整危险模式**（结合 union + switch）：

```cpp
// 危险：从网络流按 type 字段直接 union 还原
struct Message {
    uint8_t type;
    union Body {
        UserRecord  user;   // type == 1
        AdminRecord admin;  // type == 2，有 is_admin 字段
    } body;
};

Message msg;
recv(sock, &msg, sizeof(msg), 0);  // 攻击者控制全部字节
if (msg.type == 1) {
    process_user(msg.body.user);
}
// 漏洞：攻击者把 type 设为 1，但 body 按 AdminRecord 填充
// process_user 按 UserRecord 布局读字段，读到的实际是 AdminRecord 的字节
// 若 UserRecord::callback 字段偏移与 AdminRecord::is_admin 字段偏移重合 → 权限混淆
```
...
| 危险写法 | 触发类型混淆的条件 | 安全替代 |
|---------|-----------------|---------|
| `(Derived*)base` | base 实际类型非 Derived | `dynamic_cast<Derived*>(base)` + 空指针检查 |
| `reinterpret_cast<T*>(buf)` | buf 内容外部可控 | 先校验长度和 magic，再逐字段反序列化；或使用 Protocol Buffers/FlatBuffers |
| union 非激活成员读 | 多成员大小/语义不同 | `std::bit_cast<T>` (C++20) 或 `memcpy` 做 type punning |
| `void*` 回调 ctx 强转 | 多处注册不同类型 | 使用模板 + `std::function` / 虚函数 + RTTI，避免裸 `void*` |
| 反序列化直接内存映射 | 输入文件/流外部可控 | 使用带类型标签校验的反序列化框架（如 protobuf、nlohmann/json） |
