...
### tcache key 伪造与 double free 绕过（glibc 2.29+）

glibc 2.29 为 `tcache_entry` 结构增加了 `key` 字段，作为 double free 检测机制。在现代堆利用中，绕过此检测是实施 tcache poisoning 的前置条件之一。

#### tcache_entry 的 `key` 字段机制

```c
/* glibc 2.29+ tcache_entry 结构 */
typedef struct tcache_entry {
    struct tcache_entry *next;               /* fd：下一个空闲 chunk（safe-linking 混淆） */
    struct tcache_perthread_struct *key;     /* double free 检测：指向本线程 tcache 头 */
} tcache_entry;
```

**free 时的写入**：chunk 进入 tcache 时，glibc 将 chunk 用户数据区的第二个 `size_t` 字段（偏移 `+0x10` 处，即 `key` 字段的位置）写为当前线程 `tcache_perthread_struct` 的地址（一个堆地址，每次运行不同但在本次运行内固定）。

**再次 free 时的检测**：若 `key` 字段等于当前线程的 tcache 头指针，判定 double free，立即调用 `abort()`：

```c
/* glibc tcache_put / __libc_free 中的检测逻辑（简化示意） */
if (e->key == tcache) {
    /* 遍历对应 bin 链确认是否真正 double free */
    abort();
}
e->key = tcache;   /* 标记为"已在 tcache 中" */
```

#### 绕过方式一：UAF 清除 `key` 字段

最直接的绕过方式：在第一次 free 后，通过 UAF 写将 `key` 字段清零（或写为任意非 tcache 头的值），再执行第二次 free 时检测不到标记，double free 绕过成功：

```c
/* 概念示意（教学） */
char *p = malloc(0x30);
free(p);              /* chunk 进 tcache，p+0x10 = &tcache_perthread */
*(size_t *)(p + 0x10) = 0;   /* UAF 写：清零 key 字段 */
free(p);              /* 再次 free：key == 0 != tcache，不触发检测 */
/* tcache 链中现有两份 p：bin -> p -> p -> ...（环） */
```

**前提**：攻击者需要对 freed chunk 有 UAF 写能力（偏移 `+0x10` 处），这通常已是"堆利用的标准起点"，所以清 key 字段在大多数场景下不构成额外障碍。

#### 绕过方式二：错位分配（Off-by-One + 大小布局）

另一种思路是通过**堆布局操纵**，使 glibc 的 key 检测"读到错误位置"从而失效。典型手法是利用堆溢出（off-by-one 或 off-by-null）修改相邻 chunk 的 `size` 字段，使一个 chunk 的 key 字段区域被另一个 chunk 的用户数据覆盖，或制造两个"逻辑上不同但物理上有重叠"的 chunk，让检测读取的内存地址不是实际 key 字段所在位置：

```
布局示意（off-by-null 修改 chunk B 的 size）：
  chunk A（用户可控） → chunk B（size 被改小）
  free(B) → 进入较小 size 的 tcache bin，key 字段写在 B+0x10
  free(B) 再次（用原始 B 指针） → glibc 用原始 size 计算 tcache bin，
          key 字段位于原位置，但此时那里可能已被堆其他内容覆盖而非 tcache 头
```

这类手法高度依赖具体 glibc 版本和 chunk 大小布局，实际 CTF 中更常见的是直接 UAF 清 key。

#### tcache poisoning 在有 key 保护时的完整流程

综合 key 伪造 + safe-linking（glibc 2.32+），现代 tcache poisoning 的最小步骤如下：

```
1. [泄露堆地址]    通过 UAF 读获取 tcache chunk 的 fd 或 key 字段值，
                   得到堆地址 → 计算 safe-linking 密钥（heap_key = leak >> 12）
2. [清 key 字段]   通过 UAF 写将目标 chunk 的 key 字段清零，
                   准备执行第二次 free
3. [构造链表环]    二次 free 目标 chunk → tcache 链出现环
4. [修改 fd]       通过 malloc 拿到 chunk，写入混淆后的目标地址：
                   stored_fd = heap_key XOR evil_addr
                   （evil_addr 须 16 字节对齐）
5. [连续 malloc]   第一次弹出正常 chunk；第二次弹出 evil_addr → 任意写原语
```

> [!note] FSOP：glibc 2.34+ 的主流替代
> glibc 2.34 移除了 `__malloc_hook`/`__free_hook`，现代 tcache poisoning 的"写什么"目标发生了变化。最主流的后续手法是伪造 `_IO_FILE` 结构配合 FSOP，详见 `[[18.FSOP与IO_FILE劫持]]`。
