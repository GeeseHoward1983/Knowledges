...
### 减轮变体 Salsa20/8 与 Salsa20/12

Salsa20 原始设计支持三个轮次版本，均经过 eSTREAM 评选团队评估：

| 变体 | 轮数 | 相当于双轮数 | 性能（粗估，x86-64） | eSTREAM 状态 |
|---|---|---|---|---|
| Salsa20/8 | 8 轮 | 4 双轮 | ~最高（约 Salsa20/20 的 2.5 倍速） | 未推荐（存在密钥恢复攻击） |
| Salsa20/12 | 12 轮 | 6 双轮 | ~中等（约 Salsa20/20 的 1.7 倍速） | **eSTREAM 最终推荐** |
| Salsa20/20 | 20 轮 | 10 双轮 | 基准 | eSTREAM 最终推荐 |

**eSTREAM 为何推荐 Salsa20/12 而非 Salsa20/20**

eSTREAM（2004–2008 年欧洲流密码评选项目，目标是征集适合软件和硬件的流密码）在最终阶段同时推荐了 Salsa20/12 和 Salsa20/20，给出的理由如下：

1. **安全裕度仍充足**：到 2008 年评选结束，针对 Salsa20 最强的学术攻击打到约 7–8 轮（区分器），12 轮距最强攻击有 4–5 轮的缓冲，安全裕度被评估团队认为"合理（reasonable margin）"而非"宽裕（conservative margin）"。
2. **性能优势明显**：12 轮比 20 轮快约 1.5–1.7 倍，在受限硬件（8 位/16 位 MCU）上差距更大。eSTREAM 的 Profile 2（面向硬件约束设备）尤其重视吞吐/面积比，Salsa20/12 在此指标上具竞争力。
3. **设计者明确建议**：Bernstein 在提交文档中明确标注 Salsa20/12 为"推荐的性能/安全平衡点"，而 Salsa20/20 作为"保守选择"。

**Salsa20/8 为何不推荐**

2007 年 Crowley 发表了针对 Salsa20/8 的攻击（已知明文 `2^{251}`，虽不实用，但证明 8 轮不再有学术"完整"安全性）。此后 Aumasson 等人改进至约 `2^{249}`，进一步缩小理论安全裕度。评选团队因此将 Salsa20/8 排除在最终推荐之外。

**与 ChaCha 的对应**

Salsa20/8→12→20 的减轮逻辑同样适用于 ChaCha 家族，对应 ChaCha8/12/20。ChaCha12（12 轮）比 ChaCha20 快约 1.5 倍，在 eSTREAM 框架之外的现代应用中有时被考虑（如 libsodium 的 `crypto_stream_chacha20` 提供了 12 轮接口）。RFC 8439 标准化版本仅包含 ChaCha20（20 轮），生产环境应以此为准。

### eSTREAM 评选背景与竞品对比

**eSTREAM 项目简介**

eSTREAM（European Stream Cipher Project）是欧盟 ECRYPT 网络于 2004 年发起的流密码评选计划，目标是填补 2000 年代初流密码标准的空白（当时 RC4 已显现弱点，AES-CTR 尚未广泛应用）。评选分两个 Profile：

- **Profile 1（软件优化）**：面向通用 CPU，目标是高吞吐率。
- **Profile 2（硬件约束）**：面向 RFID、传感器、嵌入式设备，目标是小面积/低功耗。

**最终入选算法（2008 年）**

| 算法 | Profile | 特点 |
|---|---|---|
| **Salsa20/12** | 软件（P1） | ARX，无 S 盒，天然恒时，本篇主角 |
| **HC-128** | 软件（P1） | 大查找表（2 × 512 个 32 位字），速度极快但内存消耗高（4 KB 状态） |
| **Rabbit** | 软件（P1） | 基于耦合非线性迭代器，较高吞吐但无随机访问 |
| **SOSEMANUK** | 软件（P1） | 结合 SNOW + Serpent，安全性有正式分析，但结构复杂 |
| **Grain v1** | 硬件（P2） | LFSR + NFSR，极低面积，4G 前移动通信使用 |
| **MICKEY v2** | 硬件（P2） | 双寄存器（一 LFSR + 一非线性），面积比 Grain 略大 |
| **Trivium** | 硬件（P2） | 三个移位寄存器组成，结构最简单，已被广泛分析 |

**Salsa20 的相对定位**

```
速度排行（软件 Profile，x86-64，无 SIMD）：
  HC-128    > Salsa20/12 > Rabbit ≈ SOSEMANUK

内存开销：
  HC-128（4 KB 状态） >> Salsa20（64 字节状态） > Rabbit/Trivium

可分析性（正式安全证明/分析丰富度）：
  Salsa20/Trivium（丰富）> HC-128/Grain（中等）> Rabbit/SOSEMANUK（较少）

随机访问支持：
  Salsa20 / Trivium（支持）vs. Rabbit / Grain（不支持）
```

Salsa20 在软件 Profile 中的核心优势不是绝对速度（HC-128 更快），而是**极小状态（64 字节）+ 随机访问 + 恒定时间 + 可证明安全分析**的综合平衡。HC-128 速度虽快，但 4 KB 的内存状态在 IoT/嵌入式场景中是硬伤，且 cache-timing 侧信道风险更高。

**eSTREAM 之后的走向**

Salsa20/ChaCha20 在 eSTREAM 结束后继续演进，最终由 Google 工程师（Adam Langley 等）推动进入 TLS（2013 年），被 RFC 7539/8439 标准化，远远超越了其他 eSTREAM 入选算法的影响力——HC-128/Rabbit/SOSEMANUK 基本停留在学术和库支持层面，未进入主流协议。
