...
**松弛**（relaxation）是链接器在重定位扫描过程中将指令替换为**更短或更高效形式**的优化技术——即"能省则省，能直达则直达"。与普通重定位（只改操作数）不同，松弛可能**删除字节、修改操作码**，因而需要对节进行两遍（或多遍）扫描。

#### 为什么松弛需要两遍扫描

松弛改变了指令长度，从而改变了后续符号的地址，而地址变化又可能触发更多松弛——这是一个迭代收敛过程：

```
第一遍：发现可松弛的重定位 → 计算节内地址变化量 → 更新符号地址
第二遍：用更新后的地址重新判断，可能有更多重定位变得可松弛
……
直到某遍没有新松弛发生（通常 2～3 遍即收敛）
```

对于**字节删除型**松弛（如 RISC-V），链接器还必须维护"碎片"（fragment）表，记录每处删除的位置与字节数，以便后续重定位的 `r_offset` 都能做对应的 delta 修正。

#### x86-64：`R_X86_64_GOTPCRELX` / `R_X86_64_REX_GOTPCRELX`

x86-64 的 GOT 松弛是最常见的松弛场景，由 GNU ld 和 LLD 均默认开启。

当编译器生成通过 GOT 间接访问 `hidden` 或模块内已知符号的指令序列：

```asm
; 原始指令（通过 GOT 间接加载，7 字节）
movq  foo@GOTPCREL(%rip), %rax   ; R_X86_64_REX_GOTPCRELX，addend -4
; 对应重定位：r_type = R_X86_64_REX_GOTPCRELX
```

链接器若确定 `foo` 在本模块内定义且可见性为 `hidden`，可以将其松弛为：

```asm
; 松弛后（直接 PC-relative，7 字节，操作码不变但语义改变）
leaq  foo(%rip), %rax            ; 直接取地址，无 GOT 间接
```

或对 `call *foo@GOTPCREL(%rip)` 形式：
...
这类松弛消除了 GOT 入口，减少了 `.rela.dyn` 条目数量，对热路径函数调用有显著性能收益。相关 GOT 消除逻辑见 [[11.ELF文件详解/17.got节.md]]。

#### RISC-V：`R_RISCV_RELAX` 字节删除

RISC-V 的松弛是 ELF 架构中最复杂的，因为 RISC-V 指令集有标准 4 字节指令和压缩（C）扩展 2 字节指令，两者可混用：

```asm
; 原始：AUIPC + JALR 两条指令共 8 字节
auipc  ra, %pcrel_hi(func)       ; R_RISCV_CALL_PLT + R_RISCV_RELAX
jalr   ra, %pcrel_lo(1b)

; 若目标在 ±1 MiB 范围内，松弛为单条 JAL（4 字节，删除 4 字节）
jal    ra, func
```

`R_RISCV_RELAX` 是一个**配对标记**，附着在主重定位条目旁边，告诉链接器"这个重定位允许被松弛"。链接器在每遍扫描中根据当前已知距离决定是否合并：

- `R_RISCV_CALL` / `R_RISCV_CALL_PLT`（AUIPC+JALR）→ 可松弛为 `JAL`（距离 ≤ ±1 MiB）
- `R_RISCV_HI20` + `R_RISCV_LO12_I`（LUI+ADDI/LW）→ 可松弛为单条 C 扩展指令或消除 LUI（目标在 GP 寄存器的 ±2 KiB 范围内，即"GP 松弛"）

字节删除要求 gas 在汇编时在 `.text` 中嵌入对齐 nop padding（`R_RISCV_ALIGN`），链接器松弛完毕后再消除多余 nop。

#### ARM64（AArch64）：`ADRP` + `ADD/LDR` 合并

ARM64 的地址加载通常需要两条指令：

```asm
adrp  x0, :pg_hi21:symbol    ; 对齐到 4K 页，21 位偏移覆盖 ±4 GiB
add   x0, x0, :lo12:symbol   ; 补全 12 位页内偏移
```

当符号在 PC ±1 MiB 范围内时（`ADR` 单指令覆盖范围），链接器可以把双指令松弛为：

```asm
adr   x0, symbol             ; 单指令，省 4 字节
```

Apple ld64 和 LLD（`--aarch64-merge-exidx-entries`）均支持此合并。此外 LLD 还实现了 `R_AARCH64_CALL26` 的 Thunk 生成：超出 ±128 MiB 时插入跳板，而不是报错——这是松弛的**反向**（扩展而非收缩），同属松弛算法的一部分。

#### 与本篇重定位流程的关系

松弛发生在重定位主循环**之内**，是重定位流程的一个特化分支（见上方"重定位的触发流程"流程图中的 `PLT32 → PC32 松弛` 分支）。完整的链接器松弛体系（含 RISC-V GP 松弛、TLS 松弛等高级场景）见专篇 [[15.链接器松弛]]。
