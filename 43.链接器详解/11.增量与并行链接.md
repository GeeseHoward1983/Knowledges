...
本节从实现层面补齐上文 "mold 为什么快" 的技术细节，包括分片哈希表的设计参数、与 Intel TBB 的关系、异步 I/O 策略、并行符号解析与重定位的具体流程，以及输出文件的 `fallocate` 预分配策略，最后与 lld、gold 做架构对比。

#### 分片哈希表（sharded hash map）消除锁竞争

mold 全局符号表的核心数据结构是 `tbb::concurrent_hash_map` 包装的分片（shard）设计，在 mold 源码中对应 `Symbol` 数组配合固定分片数 `N`：

```cpp
// 概念示意（非 mold 实际代码，反映设计思路）
static constexpr int NUM_SHARDS = 65536; // 2^16
std::array<Shard, NUM_SHARDS> symbol_table;
...
Symbol *get_or_create(std::string_view name) {
    size_t h = hash_fn(name);
    auto &shard = symbol_table[h % NUM_SHARDS];
    std::lock_guard lock(shard.mu);
    return shard.map.emplace(name, nullptr).first->second;
}
```
...
| 参数 | mold 选择 | 原因 |
|---|---|---|
| 分片数 N | 通常 2^16（65536）量级 | 远大于线程数（典型 8–128），使每个线程平均获得不同 shard，锁冲突概率趋近于 `thread_count / N`（约 0.01%–0.2%） |
| hash 函数 | 快速非加密 hash（城市哈希/xxHash 类） | 分布均匀、CPU 指令少，不成为瓶颈 |
| shard 粒度 | 每 shard 独立 `std::mutex` + `std::unordered_map` | 相比全局大锁，锁持有时间极短（一次哈希桶操作）；相比无锁方案，实现简单可验证 |

这与 LLD 的 `llvm::DenseMap` + 细粒度 `SpinLock` 方案相比，mold 分片更细、shard 数更大，在 32 核以上机器上优势更显著。

#### 与 Intel TBB（tbb::parallel_for）的并行任务调度关系

mold 使用 **Intel TBB（Threading Building Blocks）** 作为任务调度后端（`tbb::task_group` / `tbb::parallel_for`），而非自己实现线程池：

```cpp
// mold 并发处理输入文件（概念示意）
tbb::parallel_for_each(ctx.objs.begin(), ctx.objs.end(),
    [&](ObjectFile<E> *file) {
        file->parse(ctx);         // 解析 ELF header + symtab
        file->resolve_symbols(ctx); // 向 shard 注册符号
    });

// 并发重定位写回
tbb::parallel_for_each(ctx.output_sections.begin(), ctx.output_sections.end(),
    [&](OutputSection<E> *osec) {
        osec->copy_buf(ctx);   // 拷贝内容到 mmap 输出缓冲
        osec->apply_reloc_alloc(ctx); // 写入重定位修补
    });
```

**TBB 带来的好处**：

- **工作窃取（work stealing）**：若某线程处理的文件很大（耗时长），其他空闲线程自动"偷取"剩余任务，负载自动均衡，无需手动划分分片大小。
- **嵌套并行**：TBB 支持嵌套任务（task_group 内再起 parallel_for），mold 的节内部并行与文件级并行可以自然嵌套，不会死锁或过订阅（over-subscription）。
- **NUMA 感知**：TBB Arena 可绑定 NUMA 节点，在多路服务器上减少跨 NUMA 内存访问。

LLD 使用 `llvm::parallel::for_each`（内部基于线程池），与 TBB 类似但不具备工作窃取；GNU ld/gold 不使用任何并行框架。

#### Preload 与异步 I/O：提前 mmap 输入文件

mold 在开始任何解析前，先用后台线程将所有输入文件 `mmap` 进地址空间：

```
主线程：     [解析命令行] → [启动 preload 线程] → [开始解析第 1 批已就绪文件] → ...
Preload 线程：               [mmap file_1] [mmap file_2] [mmap file_3] ...（并发）
```

关键实现点：

1. **`mmap` 而非 `read`**：`mmap` 是惰性的——操作系统只在真正访问对应内存页时才触发 page fault 并读磁盘（demand paging）。预先 `mmap` 可以让内核提前发起预读（readahead），CPU 不等 I/O。
2. **文件预排序**：mold 将输入文件按估算大小降序排列，优先预加载大文件，使"大文件先就绪"，让后续解析线程尽早获得最重的工作。
3. **`MADV_SEQUENTIAL` + `MADV_WILLNEED`**：mold 对 `mmap` 区域调用 `madvise`，提示内核"将要顺序访问全部页面"，触发更激进的内核预读。

与 LLD 的差异：LLD 也用 `mmap` 读输入，但不做显式 preload——它在解析符号时才 `mmap` 对应文件，是被动触发；mold 是主动提前全量 preload，属于"I/O 与 CPU 完全流水线化"。
...
阶段 1（纯并行，无锁）：
  ...
  │ 解析 .o[0]  │  │ 解析 .o[1]  │  │ 解析 .o[2]  │
  │ 读 symtab   │  │ 读 symtab   │  │ 读 symtab   │
  │ 建局部符号表│  │ 建局部符号表│  │ 建局部符号表│
  └─────────────┘  └─────────────┘  └─────────────┘
          ↓ tbb::parallel_for_each（无全局锁）

阶段 2（分片锁，低争用）：
  各线程将局部符号推送至全局 sharded symbol table
  同一名字的符号写入同一 shard（最多只有一把 shard 锁被争用）
  弱/强冲突解析在此阶段原子完成
```

**并行重定位（完全无锁）**：

重定位阶段的关键前提：**布局已完成，各输出节的 VMA（虚拟地址）已固定**。此时每条重定位记录 `(offset, type, symbol, addend)` 的计算 `S + A - P` 完全独立，各线程写入输出文件的不同内存页，**零锁冲突**：

```cpp
// 概念示意：完全无锁并发写回
tbb::parallel_for(0, relocs.size(), [&](size_t i) {
    auto &r = relocs[i];
    u64 val = r.sym->get_addr(ctx) + r.addend - (output_base + r.offset);
    write_val(output_buf + r.offset, val, r.type); // 写不同地址，无竞争
});
```

这与 GNU ld 的"单线程逐条写回"形成了极致对比。

#### mmap 写回与 fallocate 预分配输出文件策略

mold 写输出文件的策略分三步：

```
步骤 1：fallocate（预分配磁盘空间）
  fallocate(fd, 0, 0, total_output_size);
  // 告知内核"我要写这么大的文件"，一次性分配磁盘块，
  // 避免后续写入时因磁盘碎片/重分配带来的延迟。

步骤 2：mmap（映射整个输出文件到内存）
  void *buf = mmap(NULL, total_output_size, PROT_READ|PROT_WRITE,
                   MAP_SHARED, fd, 0);
  // 此后所有写操作都是写内存，由内核 dirty page 机制异步落盘。
  // 零额外 write() 系统调用，零额外内核缓冲区拷贝。

步骤 3：并发填充 + munmap 触发落盘
  tbb::parallel_for_each(sections, [&](auto *s) {
      memcpy(buf + s->offset, s->input_buf, s->size); // 并发无锁
  });
  munmap(buf, total_output_size); // 触发 msync，落盘
```

**与 LLD 的对比**：LLD 也用 `mmap` 写输出，但在部分配置下仍会调用 `write()` 补写 ELF header 和 section header table；mold 坚持"全程 mmap，只在 munmap 时才真正 I/O"，减少了额外的系统调用开销。

**`fallocate` 的重要性**：若不调用 `fallocate`，并发多线程向同一文件的不同偏移写入时，内核可能需要多次扩展文件大小，引入全局文件 inode 锁争用；`fallocate` 一次性预分配消除了这个串行点。

#### 与 lld、gold 的架构对比

| 维度 | GNU ld（BFD） | gold | LLD | mold |
|---|---|---|---|---|
| **线程框架** | 无 | `std::thread` 手动管理 | `llvm::parallel`（线程池） | Intel TBB（工作窃取） |
| **符号表并发** | 单线程全局哈希 | 部分并行（读并行，写单线程） | sharded DenseMap + SpinLock | sharded map（2^16 片）+ mutex |
| **I/O 策略** | 顺序 `fread` | 顺序 `mmap`（部分） | 并发 `mmap` | 全量 preload + MADV_WILLNEED |
| **输出写入** | `fwrite` 串行 | `mmap` + 部分并行 | `mmap` 并行 + 部分 `write` | `fallocate` + 全程 `mmap` 并行 |
| **重定位并行** | 无 | 部分 | 节级并行 | 条目级全并行（TBB） |
| **布局并行** | 无 | 无 | 节合并部分并行 | 节合并 + 地址分配并行 |
| **内存分配** | glibc malloc（碎片化） | 部分 Arena | LLVM BumpPtrAllocator | 大块 Arena（减少分配器锁竞争） |
| **串行瓶颈数** | 几乎全程串行 | 符号合并、布局 | 地址布局 | 仅地址布局的最终归约 |

结论：mold 的速度领先不来自单一创新，而是**系统性地消除每一个可识别的串行点**——从 I/O（preload）到符号解析（sharded）到重定位（无锁 mmap）到输出（fallocate）——叠加后形成在 32 核以上机器上接近线性加速的效果。
