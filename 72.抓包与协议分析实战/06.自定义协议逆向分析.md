...
识别技巧：发送不同长度的消息，观察前几个字节是否等于消息体长度。
...
- 差分分析是私有协议逆向的核心方法：控制变量观察字节变化，逐步推断字段含义
- Wireshark Lua dissector 可将任意 TCP/UDP 流量解析为可读协议树，魔数匹配和端口注册是两种注册方式
- Scapy 自定义层提供了 Python 原生的协议构造、发送、解析能力，与 Wireshark 互补
- 010 Editor 模板适合对单个二进制文件做静态解析，binwalk 适合固件中的格式识别
- 协议逆向必须在授权范围内进行，不得对非授权系统执行
