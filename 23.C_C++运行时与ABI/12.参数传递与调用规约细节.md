...
**AAPCS64（Procedure Call Standard for the ARM 64-bit Architecture）** 是 AArch64/ARM64 平台（Linux/macOS/Windows on ARM）的官方调用规约，与 System V AMD64 整体思路相近但寄存器名称和某些细节不同。

#### 寄存器分工总表

| 寄存器 | 别名 | 角色 | 说明 |
|--------|------|------|------|
| X0–X7 | — | **整型/指针参数寄存器** | 依次传第 1–8 个整型/指针参数；X0 亦为返回值寄存器 |
| X8 | XR（Indirect Result Register）| **间接结果寄存器** | 调用者将大对象（sret）的目标地址放入 X8，不占参数槽位（与 SysV AMD64 的 RDI 不同） |
| X9–X15 | — | **临时寄存器（Caller-saved）** | 调用者保存；被调用者可随意使用 |
| X16–X17 | IP0、IP1 | **进程内链接寄存器（Intra-Procedure-call scratch）** | 动态链接器/veneer 用，普通代码也可作临时寄存器 |
| X18 | PR（Platform Register）| **平台寄存器** | 平台保留（iOS/Windows 用于 TEB/TLS；Linux 通常可作通用临时）|
| X19–X28 | — | **被调用者保存寄存器（Callee-saved）** | 被调用者若使用，必须在序言/尾声保存/恢复 |
| X29 | FP | **帧指针** | 可选（`-fomit-frame-pointer` 可省）；若使用，必须保存/恢复 |
| X30 | LR | **链接寄存器（返回地址）** | `BL`/`BLR` 指令把返回地址写入 X30；被调用者保存（若本函数还调其他函数）|
| SP | SP | **栈指针** | 始终 16 字节对齐；只能以 16 字节倍数增减 |
| XZR | — | **零寄存器** | 读取恒为 0；写入丢弃 |

#### 浮点与向量参数寄存器

| 寄存器 | 角色 |
|--------|------|
| V0–V7（Q0–Q7）| 浮点/SIMD 参数（第 1–8 个浮点参数）；V0/Q0 亦为浮点返回值 |
| V8–V15 | **Callee-saved**（低 64 位 D8–D15 必须保存）；高位（128 位部分）caller-saved |
| V16–V31 | Caller-saved 临时 |

> [!note]
> ARM64 的浮点参数用 **V** 寄存器（128 位），但传递 `float`/`double` 时只使用 S（32 位）/D（64 位）别名；整型参数和浮点参数的槽位**互相独立**（和 SysV AMD64 相同），即第一个 `int` 进 X0，第一个 `double` 进 V0，互不影响。
...
|---------|--------|
| 整型 ≤ 64 位 | X0 |
| 整型 128 位 | X0（低 64）+ X1（高 64）|
| float / double | V0（S0 / D0）|
| 两个浮点 / 128 位向量 | V0 + V1 |
| ≤ 16B 简单 struct | 按字段类型拆入 X0/X1 或 V0/V1 |
| 大对象（> 16B 或含非平凡成员）| **X8（间接结果寄存器）**：调用者提前把目标内存指针放入 X8 |

**X8 间接返回（AAPCS64 特有）**：与 SysV AMD64 不同，AAPCS64 用专门的 X8 承载 sret 指针，**不占用 X0 的参数槽位**。这意味着 `BigStruct foo(int a, int b)` 编译后：

```
X0 = a（第1参数，正常放入）
X1 = b（第2参数，正常放入）
X8 = &big_struct_storage（sret 目标，由调用者提前设置）
```

```asm
; ARM64 汇编示意（AT&T/GNU 风格，注意 ARM64 语法用逗号右置）
; BigStruct make(int a, int b) 的调用点
;
; 调用者先准备 X8
sub  sp, sp, #64            // 在栈上为 BigStruct 留空间
mov  x8, sp                 // X8 = sret 指针（目标地址）
mov  w0, #5                 // X0 = a
mov  w1, #10                // X1 = b
bl   _Z4makell              // 调用 make(int,int)
// 返回后 BigStruct 已写入 [sp]；X0 = X8（同 SysV 约定 rax）
```
...
AAPCS64 对**同质浮点聚合（Homogeneous Floating-point Aggregate, HFA）** 和**同质向量聚合（Homogeneous Vector Aggregate, HVA）** 有特殊处理：

- **HFA**：struct 中全部成员为相同浮点类型（`float` 或 `double`），且成员数 ≤ 4。
- **HVA**：struct 中全部成员为相同向量类型（如 `float32x2_t`），且成员数 ≤ 4。

满足条件时，每个成员**独立占用一个 V 寄存器**（V0、V1、V2、V3），而非整体按内存传递：

```c
struct HFA3 { float x, y, z; };   // 3 个 float → V0(S0), V1(S1), V2(S2)
struct HFA4 { double a, b, c, d; }; // 4 个 double → V0(D0)…V3(D3)

// HFA5 个以上 → MEMORY 传递
struct TooLarge { float a, b, c, d, e; };  // 5 个 → 超限，走内存
```

HFA/HVA 规则对 SIMD 运算密集的科学计算/图形库 ABI 设计影响很大，误认为"struct 总是走内存"会导致 FFI 绑定产生 ABI 不兼容。

#### 可变参数（varargs）

AAPCS64 的可变参数处理比 SysV AMD64 简单：

- **无 AL 寄存器约定**：不需要提前告知 XMM 计数（ARM64 没有独立浮点寄存器计数惯例）。
- `va_list` 实现包含 4 个字段：`stack`（下一个栈参数指针）、`gr_top`（整型寄存器保存区顶）、`vr_top`（向量寄存器保存区顶）、`gr_offs`/`vr_offs`（当前整型/向量寄存器偏移）。
- 函数序言中，编译器把 X0–X7 保存到**整型寄存器保存区**，V0–V7 保存到**向量寄存器保存区**，供 `va_arg` 按需读取。

```c
// varargs 调用示例
void log_int_float(const char* fmt, ...);

// 调用点（ARM64）：
// X0 = fmt（字符串指针）
// X1 = 42（int 参数）
// V0(D0) = 3.14（double 参数）
// 无需设置 AL 等特殊寄存器
```

#### 与 SysV AMD64 的对照表

| 特性 | SysV AMD64（x86-64 Linux/macOS）| AAPCS64（ARM64 Linux/macOS/Windows）|
|------|--------------------------------|-------------------------------------|
| 整型参数寄存器 | RDI RSI RDX RCX R8 R9（6 个）| **X0–X7（8 个）** |
| 大对象 sret 指针 | 隐藏首参，放入 **RDI**（占参数槽）| 专用 **X8**（不占参数槽）|
| 浮点参数寄存器 | XMM0–XMM7 | **V0–V7** |
| 整型返回 | RAX（/+RDX）| **X0（/+X1）** |
| 浮点返回 | XMM0 | **V0** |
| Callee-saved 整型 | RBX RBP R12–R15 | **X19–X28 + X29(FP) + X30(LR)** |
| Callee-saved 浮点 | 无（XMM 全 caller-saved）| **V8–V15**（低64位）|
| 红区（red zone）| **128 字节**（RSP 以下）| **无红区** |
| 栈对齐 | call 前 16B 对齐 | 始终 **16B 对齐** |
| varargs AL 约定 | **AL = XMM 计数** | **无此约定** |
| HFA/HVA 规则 | 无（struct 按 eightbyte 分类）| **有**（≤4 个相同浮点/向量成员走V寄存器）|
| 返回地址存储 | 压栈（call 指令）| **X30（LR）**（`BL`/`BLR` 指令）|
| 平台专用寄存器 | 无（FS/GS 为段基址，不同层面）| **X18**（iOS/Windows 保留）|

> [!note] 逆向要点
> ARM64 反汇编中，`BL func` 或 `BLR Xn` 是函数调用（相当于 x86 `call`）；返回地址在 X30（LR），函数若还要调子函数则需 `STP X29, X30, [SP, #-N]!` 在序言保存。看到连续 `MOV W0–W7` / `MOV X0–X7` 装入整型参数，或 `FMOV D0–D7` 装浮点，规律与 SysV AMD64 的 RDI–R9 / XMM0–XMM7 类比即可。X8 出现在 `BL` 前且被 `ADR`/`SUB`/`ADD` 装载地址，几乎可以确认是 sret 大对象返回。
