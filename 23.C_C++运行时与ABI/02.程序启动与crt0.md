...
macOS 使用 **Mach-O** 二进制格式，其启动路径与 Linux ELF 在动态链接器介入方式和入口点指定机制上都有根本差异。

#### LC_MAIN 与旧式 LC_UNIXTHREAD

Mach-O 可执行文件通过 **Load Command** 描述程序入口。随 OS X 10.8 / Xcode 4.x 起，Apple 引入了新的 `LC_MAIN` 命令，逐步取代旧的 `LC_UNIXTHREAD`：

| Load Command | 时代 | 入口描述方式 | 是否需要 `_start`（CRT）|
|---|---|---|---|
| `LC_UNIXTHREAD`（旧）| OS X ≤ 10.7 / iOS ≤ 4 | 嵌入初始线程寄存器状态（含 PC 值），内核直接跳转到该 PC | 是，CRT `_start` 负责设置栈/环境再调 `main` |
| `LC_MAIN`（新）| macOS ≥ 10.8 / iOS ≥ 6 | 只记录一个 `entryoff`（文件偏移，相对于 `__TEXT` 段起始）| 否；`dyld` 直接把控制权交给 `libdyld.dylib` 的 `_dyld_start`，最终调用由 `entryoff` 指定的 C 函数（通常是 CRT 的 `start` 或直接是 `main`） |
...
struct entry_point_command {
    ...
    uint32_t  cmdsize;    // sizeof(struct entry_point_command) = 24
    uint64_t  entryoff;   // 入口点相对于 __TEXT 段起始的文件偏移
    uint64_t  stacksize;  // 主线程栈大小（0 = 使用系统默认）
};
```
...
```bash
# 查看 Mach-O Load Commands（macOS 工具链）
otool -l ./a.out | grep -A 5 LC_MAIN
# 示例输出（为示意）：
# Load command 12
#       cmd LC_MAIN
#   cmdsize 24
#  entryoff 0x3f50      ← __TEXT 段内的入口偏移
# stacksize 0           ← 使用默认栈大小

# 亦可用 llvm-objdump
llvm-objdump --macho -l ./a.out | grep -A 5 LC_MAIN
```

> [!warning] 示例输出为示意

#### dyld → `__dyld_start` → `main` 启动链

macOS 动态链接器是 **`dyld`（dynamic linker/loader）**，其最新版本 `dyld4`（macOS 12+）以进程内缓存（`dyld_shared_cache`）为核心。启动链如下：

```mermaid
graph TD
    KERNEL["内核 execve()\n映射 Mach-O 段\n找 LC_DYLINKER → /usr/lib/dyld"] --> DYLD
    DYLD["dyld（动态链接器）\n_dyld_start（汇编入口）"] --> CACHE["读取 dyld_shared_cache\n解析依赖（LC_LOAD_DYLIB）\n处理重定位（chained fixups）"]
    CACHE --> INIT["执行所有依赖库的\n+load 方法（ObjC）\n__attribute__((constructor)) 函数\n.init_array 函数指针"]
    INIT --> LIBDYLD["libdyld.dylib\ndyldSetupForMain()\n设置 argc/argv/envp\n初始化 libSystem（libc）"]
    LIBDYLD --> CRT["CRT start 函数\n（Clang：start / __start）\n或直接 entryoff 指向 main"]
    CRT --> MAIN["main(argc, argv, envp)"]
    MAIN --> EXIT["exit()\natexit 链\n__cxa_finalize"]
```

> [!warning] 流程图中 `+load` / `__attribute__((constructor))` 的执行顺序受依赖链顺序及优先级影响，此为简化示意

关键阶段说明：

| 阶段 | 执行者 | Linux ELF 类比 |
|------|--------|---------------|
| 内核映射 ELF/Mach-O | 内核 | 内核 `execve()` |
| 动态链接器入口 | `__dyld_start`（汇编）| `ld.so` 的 `_dl_start` |
| 加载依赖库 | `dyld`：`LC_LOAD_DYLIB` | `ld.so`：`DT_NEEDED` |
| 重定位 | `dyld`：chained fixups（`LC_DYLD_CHAINED_FIXUPS`）| `ld.so`：`.rela.plt`/`.rela.dyn` |
| 库初始化回调 | `dyld` 调用 `__attribute__((constructor))` | `ld.so` 调用 DSO 的 `.init_array` |
| C 库初始化 | `libSystem.B.dylib`（包含 `libdyld`/`libc`/`libpthread`）| `glibc` 的 `__libc_start_main` |
| 用户入口 | `main()`（由 `entryoff` 指定或 CRT 调用）| `main()`（由 `__libc_start_main` 调用）|

#### 与 Linux `_start`→`__libc_start_main` 的关键差异

| 维度 | Linux ELF（glibc）| macOS Mach-O（dyld4）|
|------|------------------|--------------------|
| 入口 Load Command | `e_entry` 字段（ELF 头）| `LC_MAIN.entryoff` |
| 动态链接器介入 | `PT_INTERP` 段，指向 `ld.so` | `LC_DYLINKER`，指向 `/usr/lib/dyld` |
| 启动汇编符号 | `_start`（`crt1.o`/`Scrt1.o`）| `__dyld_start`（dyld 内部）|
| C 库 | `glibc`（`libc.so.6`）| `libSystem.B.dylib`（融合了 libc/pthread/dyld API）|
| 标准库 DSO 缓存 | 无；每次从磁盘 mmap | **`dyld_shared_cache`**：所有系统库预链接成一个大文件，`dyld` 直接 mmap 共享内存 |
| ObjC `+load` 调用 | 不适用（ELF 无 ObjC runtime 钩子）| **在 `main` 前**由 `dyld` 调用每个类的 `+load` |
| `.init_array` 遍历 | `__libc_start_main` 调用 | `dyld` 在调 `main` 前遍历各 dylib 的 `__mod_init_func` 节（等价于 `.init_array`）|
| stripped 二进制入口识别 | `e_entry` → `_start` 特征指令 | `LC_MAIN.entryoff` 直接给出；或用 `otool -l` 查看 |

#### 实战：otool / lldb 查看启动链

```bash
# 查看 Mach-O 头和所有 Load Commands
otool -l ./a.out

# 只看 LC_MAIN 入口偏移
otool -l ./a.out | awk '/LC_MAIN/{found=1} found && /entryoff/{print; found=0}'

# lldb 从最早的用户代码开始（类比 gdb starti）
lldb ./a.out
(lldb) process launch --stop-at-entry    # 停在 dyld 入口
(lldb) run                               # 如需直接断在 main：breakpoint set -n main

# 查看 __TEXT 段布局（验证 entryoff）
otool -l ./a.out | grep -A 10 'segname __TEXT'
```
